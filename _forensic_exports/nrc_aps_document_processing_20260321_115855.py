...
def _process_pdf(
    *,
    ...
    weak_page_count_document_total = 0
    ...
    for chunk_start in range(0, total_pages, chunk_size):
        ...
        for page_index in range(chunk_start, chunk_end):
            ...
            if native_quality["quality_status"] in {APS_QUALITY_STATUS_WEAK, APS_QUALITY_STATUS_UNUSABLE} or is_advanced_doc or (is_low_info and not has_significant_image):
                ...
                # Track truly weak/unusable native pages
                if fallback_triggered_by_quality:
                    weak_page_count += 1
                if bool(config.get("ocr_enabled", True)):
                    weak_page_count_document_total += 1 if fallback_triggered_by_quality else 0
                    page_ocr_paths_taken.append("fallback")
                    ocr_attempted = True
            ...
            if images and (ocr_available or is_advanced_doc) and bool(config.get("ocr_enabled", True)):
                    ...
                    try:
                        ...
                        if ocr_text:
                            ...
                            if len(ocr_words - native_words) > 5:
                                page_ocr_paths_taken.append("hybrid")
                                page_units.append({
                                    "page_number": page_number,
            ...
            page_summaries.append({
                ...
                "page_native_quality_is_weak": fallback_triggered_by_quality,
                "ocr_paths_taken": page_ocr_paths_taken.copy() if page_ocr_paths_taken else [],
            })
    ...
    final_result = {
        **detection,
        ...
        "weak_page_count_document_total": weak_page_count_document_total,
        "ocr_max_pages_config_value": config.get("ocr_max_pages"),
        "ocr_max_pages_deprecated": None,
        "normalized_text": normalized_text,
        "normalized_text_sha256": hashlib.sha256(normalized_text.encode("utf-8")).hexdigest(),
        "normalized_char_count": len(normalized_text),
    }
