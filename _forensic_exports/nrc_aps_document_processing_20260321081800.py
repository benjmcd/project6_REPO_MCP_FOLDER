    ...
    *,
    ...
    for chunk_start in range(0, total_pages, chunk_size):
        ...
        for page_index in range(chunk_start, chunk_end):
            ...
            visual_features = _provisional_visual_class(page, image_candidates, native_text)
            ...
            if ocr_attempted or ocr_attempted_fallback or ocr_attempted_hybrid:
                ocr_attempted = True

            ocr_paths_taken: list[str] = []
            if fallback_entered:
                ocr_paths_taken.append("fallback")
            if hybrid_entered:
                ocr_paths_taken.append("hybrid")

            page_native_quality_is_weak = pre_branch_native_quality_status in {
                APS_QUALITY_STATUS_WEAK,
                APS_QUALITY_STATUS_UNUSABLE,
            }
...
def _extract_embedded_image_candidates(
    page: fitz.Page,
    text_dict: Any,
) -> dict[str, Any]:
    ...
    for block in text_dict.get("blocks", []):
        ...
        if not bbox:
            continue
        r = fitz.Rect(bbox)
        key = f"{r.irect[0]},{r.irect[1]},{r.irect[2]},{r.irect[3]}"
    ...
    for img_info in page.get_image_info(xrefs=True):
        ...
        samples = img_info.get("samples", b"")
        is_raster_only = bool(smask) or (
            xref == 0
            and bpc == 1
            and not samples.startswith(b"\xff\xd8")
            and not samples.startswith(b"\x89PNG")
            and not samples.startswith(b"\x00\x00\x00\x0c")
        )
        candidates.append({
    ...
    for img in page.get_images():
        ...
        if w < 1 or h < 1:
            continue
        try:
            bbox = fitz.Rect(*page.get_image_bbox(xref)).irect
    ...
    total_area = sum(round(c["width"] * c["height"], 2) for c in candidates)
    max_w = max((c["width"] for c in candidates), default=0.0)
    max_h = max((c["height"] for c in candidates), default=0.0)
    raster_only = sum(1 for c in candidates if c.get("is_raster_only"))
    max_single_ratio = max((c["width"] * c["height"] / page_area for c in candidates), default=0.0) if page_area > 0 else 0.0
    max_single_ratio = min(max_single_ratio, 1.0)

    return {
        "candidates": candidates,
        "candidate_count_approx": len(candidates),
        "total_image_area_approx": round(total_area, 2),
        "max_image_width_approx": round(max_w, 2),
        "max_image_height_approx": round(max_h, 2),
        "raster_only_count_approx": raster_only,
        "max_single_candidate_area_ratio_approx": round(max_single_ratio, 4),
    }


def _provisional_visual_class(
    page: fitz.Page,
    ...
    candidate_count = image_candidates.get("candidate_count_approx", 0)
    total_image_area = image_candidates.get("total_image_area_approx", 0.0)
    total_area_ratio = min((total_image_area / page_area), 1.0) if page_area > 0 else 0.0
    max_single_ratio = image_candidates.get("max_single_candidate_area_ratio_approx", 0.0)

    text_block_ratio_sum = image_candidates.get("text_block_bbox_area_ratio_sum_approx", 0.0)
    text_significant = text_block_ratio_sum > 0.05 or len(native_text.strip()) > 200
    ...
    return {
        ...
        "max_single_candidate_area_ratio_approx": round(max_single_ratio, 4),
        "text_block_bbox_area_ratio_sum_approx": text_block_ratio_sum,
    }
...
def _extract_native_pdf_units(
    ...
    image_candidates = _extract_embedded_image_candidates(page, text_dict)

    page_rect = fitz.Rect(page.rect)
    page_area = page_rect.width * page_rect.height
    text_block_bbox_area_sum = 0.0
    ...
    for block in text_dict.get("blocks", []):
        ...
        text_block_bbox_area_sum += block_rect.width * block_rect.height
    ...
    text_block_bbox_area_ratio_sum_approx = round(
        text_block_bbox_area_sum / page_area, 6
    ) if page_area > 0 else 0.0
    image_candidates["text_block_bbox_area_ratio_sum_approx"] = text_block_bbox_area_ratio_sum_approx
