...
def _seed_report_index_rows(db, *, reports_dir: Path, run_id: str) -> None:
    ...
    refs_a = _write_refs(reports_dir, f"{run_id}_ctx_refs_a", "alpha alpha beta alpha gamma")
    refs_b = _write_refs(reports_dir, f"{run_id}_ctx_refs_b", "alpha delta")
...
def test_persisted_context_packet_is_immutable_and_updates_run_refs(monkeypatch, tmp_path: Path):
    ...
    try:
        ...
        preview = nrc_aps_context_packet.assemble_context_packet(
            db,
            request_payload={
        ...
        first = nrc_aps_context_packet.assemble_context_packet(
            db,
            request_payload={
                "evidence_report_export_package_id": package["evidence_report_export_package_id"],
        ...
        second = nrc_aps_context_packet.assemble_context_packet(
            db,
            request_payload={
                "evidence_report_export_package_ref": package["evidence_report_export_package_ref"],
                "persist_context_packet": True,
            },
        )

        assert first["context_packet_id"] == second["context_packet_id"]
        assert first["context_packet_ref"] == second["context_packet_ref"]
        assert first["context_packet_checksum"] == second["context_packet_checksum"]
        assert first["context_packet_checksum"] == preview["context_packet_checksum"]
        assert first["source_family"] == contract.APS_CONTEXT_PACKET_SOURCE_FAMILY_PACKAGE
        assert Path(first["context_packet_ref"]).exists()

        persisted = nrc_aps_context_packet.get_persisted_context_packet(
            context_packet_id=first["context_packet_id"]
        )
        assert persisted["persisted"] is True
        assert persisted["context_packet_id"] == first["context_packet_id"]

        run = db.get(ConnectorRun, run_id)
        refs = dict((run.query_plan_json or {}).get("aps_context_packet_report_refs") or {})
        assert refs.get("aps_context_packets") == [first["context_packet_ref"]]
        assert refs.get("aps_context_packet_failures") == []
        summaries = list((run.query_plan_json or {}).get("aps_context_packet_summaries") or [])
        assert len(summaries) == 1
        assert summaries[0]["context_packet_id"] == first["context_packet_id"]
    finally:
        db.close()
        engine.dispose()


def test_context_packet_accepts_report_export_and_package_source_families(monkeypatch, tmp_path: Path):
    engine = create_engine("sqlite:///:memory:", connect_args={"check_same_thread": False})
    Session = sessionmaker(autocommit=False, autoflush=False, bind=engine)
    Base.metadata.create_all(bind=engine)
    db = Session()
    try:
        ...
        export_packet = nrc_aps_context_packet.assemble_context_packet(
            db,
            request_payload={
                "evidence_report_export_id": pipeline_a["export"]["evidence_report_export_id"],
                "persist_context_packet": False,
            },
        )
        ...
        assert report_packet["source_family"] == contract.APS_CONTEXT_PACKET_SOURCE_FAMILY_REPORT
        assert export_packet["source_family"] == contract.APS_CONTEXT_PACKET_SOURCE_FAMILY_EXPORT
        assert package_packet["source_family"] == contract.APS_CONTEXT_PACKET_SOURCE_FAMILY_PACKAGE
        assert report_packet["facts"][0]["fact_type"] == "report_summary"
        assert export_packet["facts"][0]["fact_type"] == "export_summary"
        assert package_packet["facts"][0]["fact_type"] == "package_summary"
    finally:
        db.close()
        engine.dispose()


def test_persisted_same_id_drifted_context_packet_fails_closed(monkeypatch, tmp_path: Path):
    engine = create_engine("sqlite:///:memory:", connect_args={"check_same_thread": False})
    Session = sessionmaker(autocommit=False, autoflush=False, bind=engine)
    Base.metadata.create_all(bind=engine)
    db = Session()
    try:
        ...
        run_id = "run-context-service-3"
        _seed_report_index_rows(db, reports_dir=tmp_path, run_id=run_id)
        report = _persisted_artifacts(db, run_id=run_id, query="alpha")["report"]

        packet = nrc_aps_context_packet.assemble_context_packet(
            db,
            request_payload={
                "evidence_report_id": report["evidence_report_id"],
                "persist_context_packet": True,
            },
        )
        ...
        packet_path.write_text(tampered_text, encoding="utf-8")

        with pytest.raises(nrc_aps_context_packet.ContextPacketError) as exc_info:
            nrc_aps_context_packet.assemble_context_packet(
                db,
                request_payload={
                    "evidence_report_id": report["evidence_report_id"],
                    "persist_context_packet": True,
        ...
        assert packet_path.read_text(encoding="utf-8") == tampered_text
    finally:
        db.close()
        engine.dispose()
