...
def _seed_report_index_rows(db, *, reports_dir: Path, run_id: str = "run-evidence-report-1") -> None:
    run = ConnectorRun(
    ...
    target_a = ConnectorRunTarget(
        connector_run_target_id="target-evidence-report-1",
        connector_run_id=run_id,
    ...
    target_b = ConnectorRunTarget(
        connector_run_target_id="target-evidence-report-2",
        connector_run_id=run_id,
        artifact_surface="documents",
        status="recommended",
        ordinal=1,
    ...
    refs_a = _write_refs(reports_dir, "report_refs_a", "alpha alpha beta alpha gamma")
    refs_b = _write_refs(reports_dir, "report_refs_b", "alpha delta")

    db.add(
        ApsContentDocument(
            content_id="content-report-1",
            ...
            normalized_text_sha256="norm-report-1",
    ...
    db.add(
        ApsContentChunk(
            ...
            chunk_text_sha256="sha-report-1",
        )
    )
    db.add(
        ApsContentChunk(
            content_id="content-report-1",
            chunk_id="chunk-report-2",
            content_contract_id=bundle_contract.APS_CONTENT_CONTRACT_ID,
            chunking_contract_id=bundle_contract.APS_CHUNKING_CONTRACT_ID,
            chunk_ordinal=1,
            start_char=17,
            ...
            chunk_text_sha256="sha-report-2",
        )
    )
    db.add(
        ApsContentDocument(
            content_id="content-report-2",
            content_contract_id=bundle_contract.APS_CONTENT_CONTRACT_ID,
            chunking_contract_id=bundle_contract.APS_CHUNKING_CONTRACT_ID,
            normalization_contract_id=bundle_contract.APS_NORMALIZATION_CONTRACT_ID,
            normalized_text_sha256="norm-report-2",
    ...
    db.add(
        ApsContentChunk(
            ...
            chunk_text_sha256="sha-report-3",
        )
    )
    db.add(
        ApsContentLinkage(
            content_id="content-report-1",
            run_id=run_id,
            target_id="target-evidence-report-1",
            accession_number="ML-REPORT-1",
            content_contract_id=bundle_contract.APS_CONTENT_CONTRACT_ID,
            chunking_contract_id=bundle_contract.APS_CHUNKING_CONTRACT_ID,
    ...
    db.add(
        ApsContentLinkage(
            content_id="content-report-2",
            run_id=run_id,
            target_id="target-evidence-report-2",
...
def test_persisted_evidence_report_is_immutable_and_updates_run_refs(monkeypatch, tmp_path: Path):
    ...
    try:
        ...
        bundle = nrc_aps_evidence_bundle.assemble_evidence_bundle(
            db,
            request_payload={"run_id": "run-evidence-report-1", "query": "alpha", "persist_bundle": True},
        )
        ...
        preview = nrc_aps_evidence_report.assemble_evidence_report(
            db,
            request_payload={"citation_pack_id": citation_pack["citation_pack_id"], "persist_report": False},
        )
        first = nrc_aps_evidence_report.assemble_evidence_report(
            db,
            request_payload={"citation_pack_id": citation_pack["citation_pack_id"], "persist_report": True},
        )
        second = nrc_aps_evidence_report.assemble_evidence_report(
            db,
            request_payload={"citation_pack_ref": citation_pack["citation_pack_ref"], "persist_report": True, "limit": 1, "offset": 1},
        )

        assert first["evidence_report_id"] == second["evidence_report_id"]
        assert first["evidence_report_ref"] == second["evidence_report_ref"]
        assert first["evidence_report_checksum"] == second["evidence_report_checksum"] == preview["evidence_report_checksum"]
        assert Path(first["evidence_report_ref"]).exists()
        assert first["total_sections"] == 2
        assert first["total_citations"] == 3
        assert first["sections"][0]["title"] == "Accession ML-REPORT-1 / Content content-report-1"

        persisted = nrc_aps_evidence_report.get_persisted_evidence_report_page(
            evidence_report_id=first["evidence_report_id"],
            limit=1,
            offset=1,
        )
        assert persisted["evidence_report_id"] == first["evidence_report_id"]
        assert persisted["persisted"] is True
        assert persisted["sections"][0]["section_ordinal"] == 2
        assert persisted["sections"][0]["title"] == "Content content-report-2"

        run = db.get(ConnectorRun, "run-evidence-report-1")
        refs = dict((run.query_plan_json or {}).get("aps_evidence_report_report_refs") or {})
        assert refs.get("aps_evidence_reports") == [first["evidence_report_ref"]]
        assert refs.get("aps_evidence_report_failures") == []
        summaries = list((run.query_plan_json or {}).get("aps_evidence_report_summaries") or [])
        assert len(summaries) == 1
        assert summaries[0]["evidence_report_id"] == first["evidence_report_id"]
    finally:
        db.close()
        engine.dispose()


def test_persisted_same_id_invalid_artifact_fails_closed_without_overwrite(monkeypatch, tmp_path: Path):
    engine = create_engine("sqlite:///:memory:", connect_args={"check_same_thread": False})
    Session = sessionmaker(autocommit=False, autoflush=False, bind=engine)
    Base.metadata.create_all(bind=engine)
    db = Session()
    try:
        ...
        report_path = Path(first["evidence_report_ref"])
        tampered = json.loads(report_path.read_text(encoding="utf-8"))
        tampered["source_citation_pack"]["source_bundle"]["query"] = "tampered without checksum update"
        tampered_text = json.dumps(tampered, indent=2, sort_keys=True)
        report_path.write_text(tampered_text, encoding="utf-8")
        ...
        assert report_path.read_text(encoding="utf-8") == tampered_text
...
def test_persisted_same_id_valid_but_drifted_artifact_fails_closed_without_overwrite(monkeypatch, tmp_path: Path):
    engine = create_engine("sqlite:///:memory:", connect_args={"check_same_thread": False})
    Session = sessionmaker(autocommit=False, autoflush=False, bind=engine)
    Base.metadata.create_all(bind=engine)
    db = Session()
    try:
        ...
        report_path = Path(first["evidence_report_ref"])
        tampered = json.loads(report_path.read_text(encoding="utf-8"))
        tampered["source_citation_pack"]["source_bundle"]["query"] = "tampered with checksum update"
        tampered["evidence_report_checksum"] = contract.compute_evidence_report_checksum(tampered)
        tampered_text = json.dumps(tampered, indent=2, sort_keys=True)
        report_path.write_text(tampered_text, encoding="utf-8")

        validated_payload, _validated_path = nrc_aps_evidence_report.load_persisted_evidence_report_artifact(
            evidence_report_ref=report_path,
        )
        assert validated_payload["source_citation_pack"]["source_bundle"]["query"] == "tampered with checksum update"
...
def test_persisted_failure_artifact_is_source_pack_scoped(monkeypatch, tmp_path: Path):
    engine = create_engine("sqlite:///:memory:", connect_args={"check_same_thread": False})
    Session = sessionmaker(autocommit=False, autoflush=False, bind=engine)
    Base.metadata.create_all(bind=engine)
    db = Session()
    try:
        ...
        monkeypatch.setattr(nrc_aps_evidence_citation_pack, "settings", _Settings())
        monkeypatch.setattr(nrc_aps_evidence_report, "settings", _Settings())
        _seed_report_index_rows(db, reports_dir=tmp_path, run_id="run-evidence-report-failure")

        bundle = nrc_aps_evidence_bundle.assemble_evidence_bundle(
            db,
            request_payload={"run_id": "run-evidence-report-failure", "query": "alpha", "persist_bundle": True},
        )
        citation_pack = nrc_aps_evidence_citation_pack.assemble_evidence_citation_pack(
            db,
            request_payload={"bundle_id": bundle["bundle_id"], "persist_pack": True},
        )
        pack_path = Path(citation_pack["citation_pack_ref"])
        tampered = json.loads(pack_path.read_text(encoding="utf-8"))
        tampered["citations"][0]["citation_label"] = "APS-CIT-99999"
        tampered["citation_pack_checksum"] = nrc_aps_evidence_citation_pack.contract.compute_citation_pack_checksum(tampered)
        pack_path.write_text(json.dumps(tampered, indent=2, sort_keys=True), encoding="utf-8")
        ...
        assert exc_info.value.code == contract.APS_RUNTIME_FAILURE_SOURCE_CITATION_PACK_INVALID

        run = db.get(ConnectorRun, "run-evidence-report-failure")
        refs = dict((run.query_plan_json or {}).get("aps_evidence_report_report_refs") or {})
        assert refs.get("aps_evidence_reports") == []
        failure_refs = list(refs.get("aps_evidence_report_failures") or [])
        assert len(failure_refs) == 1
        failure_payload = json.loads(Path(failure_refs[0]).read_text(encoding="utf-8"))
        assert failure_payload["schema_id"] == contract.APS_EVIDENCE_REPORT_FAILURE_SCHEMA_ID
        assert failure_payload["error_code"] == contract.APS_RUNTIME_FAILURE_SOURCE_CITATION_PACK_INVALID
