...
from app.services import nrc_aps_evidence_report_export_package_gate  # noqa: E402
...
def _seed_report_index_rows(db, *, reports_dir: Path, run_id: str) -> None:
    ...
    refs_a = _write_refs(reports_dir, f"{run_id}_gate_refs_a", "alpha alpha beta alpha gamma")
    refs_b = _write_refs(reports_dir, f"{run_id}_gate_refs_b", "alpha delta")
...
def test_evidence_report_export_package_gate_pass_and_fail_closed_on_ordered_source_drift(monkeypatch, tmp_path: Path):
    ...
    try:
        ...
        monkeypatch.setattr(nrc_aps_evidence_report_export_package_gate, "settings", _Settings())
        monkeypatch.setattr(
            nrc_aps_evidence_report_export_package_gate,
            "_load_candidate_runs",
            lambda run_ids, limit: [{"run_id": "run-er-export-package-gate"}],
        )

        run_id = "run-er-export-package-gate"
        ...
        pass_report = nrc_aps_evidence_report_export_package_gate.validate_evidence_report_export_package_gate(
            run_ids=[run_id],
            limit=1,
        ...
        package_path.write_text(json.dumps(tampered, indent=2, sort_keys=True), encoding="utf-8-sig")

        fail_report = nrc_aps_evidence_report_export_package_gate.validate_evidence_report_export_package_gate(
            run_ids=[run_id],
            limit=1,
        ...
        assert contract.APS_GATE_FAILURE_ORDERED_DIGEST in reasons or contract.APS_GATE_FAILURE_DERIVATION_DRIFT in reasons
    finally:
        db.close()
        engine.dispose()
