...
- SEC XBRL Layer 3 PRs follow `next_milestone_plans/Layer3_planning_docs/SEC-XBRL-MERGE-GATE-VERIFIER-POLICY.md`, which is canonical. Tier-1 validate-only changes merge on self-verification plus CI; bot silence is not a blocker. Tier-2 changes (Alembic migrations, `models.py`/ORM schema, durable persistence, value-reveal, default-on, or redaction-posture) record the exact Tier-2 surfaces touched and why, include the narrowest meaningful tests plus migration/rollback or containment notes, and seek independent pre-merge review when practical -- required only for concrete risk triggers (redaction-posture change, value reveal, default-on behavior, destructive/irreversible migration, broad operator-workflow change, or authority that remains ambiguous after audit). When independent review is not obtained, the executor records why self-verification is adequate and what would force a follow-up. Merge is blocked only by failing CI checks (author-enforced; `main` has no branch-protection required-status-check gate), unresolved critical/blocking findings, missing rollback/containment notes for schema/persistence changes, unclear authority, or an explicit operator instruction requiring review.

## Agent Identity Rules

- Do not add agent tool names, model names, or AI product branding anywhere in source files, docs, planning files, commit messages, PR titles, or PR bodies. This includes but is not limited to: `Co-Authored-By: Claude ...`, `Co-Authored-By: Codex ...`, `Generated with Claude Code`, `Generated by Copilot`, `Created by GPT`, and equivalent attribution lines from any AI coding tool.
- Commit messages and PR bodies must read as operator-authored. No agent self-identification.
- Do not add `Co-Authored-By` trailers for AI agents in any commit. Human co-authors may use the standard trailer format.
- Planning docs, handoff briefs, and progress records must contain only repo-relevant content. No agent attribution lines, no tool-branding footers.
