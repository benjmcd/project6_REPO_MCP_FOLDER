...
$NrcApsValidateOnlyGatesRefreshPath = Join-Path $RepoRoot "tools\nrc_aps_refresh_validate_only_gates.py"
$NrcApsValidateOnlyGatesGatePath = Join-Path $RepoRoot "tools\nrc_aps_validate_only_gates_gate.py"
...
$DualLiveRunPath = Join-Path $RepoRoot "tools\dual_live_run.py"
...
$NrcApsValidateOnlyGatesValidationReportPath = Join-Path $RepoRoot "tests\reports\nrc_aps_validate_only_gates_validation_report.json"
...
$Tier2StorageDir = Normalize-AbsPath (Join-Path $BackendDirAbs "app\storage_test_runtime")
$Tier3DbPath = Join-Path $BackendDirAbs "attached_eval.db"
$Tier3DatabaseUrl = To-SqliteUrl (Normalize-AbsPath $Tier3DbPath)
$Tier3StorageDir = Normalize-AbsPath (Join-Path $BackendDirAbs "app\storage_eval")

$NrcApsReplaySourceRoot = Normalize-AbsPath $NrcApsReplaySourceRoot
$NrcApsTestStorageDir = $Tier2StorageDir
$NrcApsTestDatabaseUrl = $Tier2DatabaseUrl
$NrcApsLiveReportsDir = Normalize-AbsPath $NrcApsLiveReportsDir
$NrcApsLiveBatchRoot = Normalize-AbsPath $NrcApsLiveBatchRoot
...
function Invoke-WithTier {
    ...
    $hadDatabaseUrl = Test-Path Env:DATABASE_URL
    $hadStorageDir = Test-Path Env:STORAGE_DIR
    $previousDatabaseUrl = if ($hadDatabaseUrl) { $env:DATABASE_URL } else { $null }
    $previousStorageDir = if ($hadStorageDir) { $env:STORAGE_DIR } else { $null }
    $locationPushed = $false
    try {
...
switch ($Action) {
    ...
    "initialize-dual-live" {
        if (-not (Test-Path $DualLiveInitializePath)) {
            throw "Dual-live initializer not found: $DualLiveInitializePath"
        }
        Push-Location $RepoRoot
        try {
            & py "-$PythonVersion" $DualLiveInitializePath @ActionArgs
            exit $LASTEXITCODE
        }
        finally {
            Pop-Location
    ...
    "refresh-nrc-aps-validate-only-gates" {
        if (-not (Test-Path $NrcApsValidateOnlyGatesRefreshPath)) {
            throw "NRC APS validate-only refresh script not found: $NrcApsValidateOnlyGatesRefreshPath"
        }
        if ([string]::IsNullOrWhiteSpace($NrcApsRunId)) {
    ...
    "eval-attached" {
        ...
        Invoke-WithTier $Tier3DatabaseUrl $Tier3StorageDir {
            Invoke-Py -Arguments @($AttachedEvalPath) -WorkingDirectory $BackendDirAbs
        }
    }
