...
DEFAULT_REPORT_PATH = Path("tests/reports/nrc_aps_sync_drift_validation_report.json")


class NrcApsSyncDriftGateError(RuntimeError):
    """Raised when local sync-drift gate validation fails."""
...
def _load_candidate_runs(*, run_ids: list[str] | None, limit: int | None) -> list[dict[str, Any]]:
    ...
    if normalized_run_ids:
        ...
        for summary_path in summary_paths:
            ...
            run_id = name.rsplit("_run_summary.json", 1)[0]
            row = _run_row_from_summary(run_id, summary_path)
            if row:
                rows.append(row)
...
def validate_sync_drift_gate(
    *,
    ...
    report = nrc_aps_sync_drift.validate_sync_drift_artifact_presence(run_rows=run_rows, reports_dir=settings.connector_reports_dir)
    ...
    if require_runs and len(run_rows) == 0:
        report["passed"] = False
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate NRC APS sync delta/drift artifacts (fail-closed).")
    parser.add_argument(
        "--run-id",
        action="append",
        default=[],
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Maximum number of latest NRC APS completed runs to validate when --run-id is not used.",
    )
    parser.add_argument(
        "--report",
    ...
    report = validate_sync_drift_gate(
