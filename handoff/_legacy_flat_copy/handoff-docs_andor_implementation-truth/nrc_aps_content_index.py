...
APS_CONTENT_UNITS_SCHEMA_ID = "aps.content_units.v1"
...
APS_CONTENT_CONTRACT_ID = "aps_content_units_v1"
APS_CHUNKING_CONTRACT_ID = "aps_chunking_v1"
...
def chunk_normalized_text(
    *,
    ...
    if text_len <= 0:
        return []
    step = int(chunk_size_chars) - int(chunk_overlap_chars)
    if step <= 0:
        raise ValueError("invalid_chunk_step")
    chunks: list[dict[str, Any]] = []
    ordinal = 0
    start = 0
    while start < text_len:
        end = min(start + int(chunk_size_chars), text_len)
...
def _load_normalized_text(
    *,
    ...
    if normalized_ref:
        ...
        if candidate.exists():
            try:
                ...
                return text, str(candidate), hashlib.sha256(text.encode("utf-8")).hexdigest(), normalization_contract_id
            except OSError:
                pass
    ...
    content_type = nrc_aps_artifact_ingestion.normalize_content_type(download.get("content_type"))
    ...
    return (
        normalized_text,
        str(normalized_blob.get("normalized_text_ref") or ""),
        str(normalized_blob.get("normalized_text_sha256") or ""),
        str(extracted.get("normalization_contract_id") or APS_NORMALIZATION_CONTRACT_ID),
    )
...
def content_units_artifact_path(*, run_id: str, target_id: str, reports_dir: str | Path) -> Path:
    ...
    return Path(reports_dir) / f"{safe_run}_{safe_target}_aps_content_units_v1.json"
...
def build_content_units_payload_from_target_artifact(
    *,
    ...
    if artifact_outcome_status == APS_CONTENT_STATUS_ARTIFACT_NOT_AVAILABLE:
        ...
        normalized_text, normalized_text_ref, normalized_text_sha256, normalization_contract_id = _load_normalized_text(
    ...
    chunks: list[dict[str, Any]] = []
    if len(normalized_text) > 0:
        base_chunks = chunk_normalized_text(
