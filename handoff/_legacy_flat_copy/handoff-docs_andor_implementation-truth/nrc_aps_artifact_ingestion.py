...
APS_TEXT_NORMALIZATION_CONTRACT_ID = "aps_text_normalization_v1"

APS_PDF_EXTRACTOR_ID = "aps_pdf_text_extractor"
APS_PDF_EXTRACTOR_VERSION = "1.0.0"
APS_TEXT_EXTRACTOR_ID = "aps_text_plain_extractor"
APS_TEXT_EXTRACTOR_VERSION = "1.0.0"

APS_SUPPORTED_CONTENT_TYPES = {"application/pdf", "text/plain"}
...
def extract_and_normalize(*, content: bytes, content_type: str) -> dict[str, Any]:
    ...
    if normalized_type == "application/pdf":
        ...
        return {
            "content_type": normalized_type,
            "extractor_id": APS_PDF_EXTRACTOR_ID,
            "extractor_version": APS_PDF_EXTRACTOR_VERSION,
            "normalization_contract_id": APS_TEXT_NORMALIZATION_CONTRACT_ID,
            "normalized_text": normalized_text,
            "normalized_text_sha256": hashlib.sha256(normalized_text.encode("utf-8")).hexdigest(),
            "normalized_char_count": len(normalized_text),
        }
    raise ValueError(f"unsupported_content_type:{normalized_type or 'unknown'}")
