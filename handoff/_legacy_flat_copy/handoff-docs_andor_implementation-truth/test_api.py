...
os.environ['DATABASE_URL'] = 'sqlite:///./test_method_aware.db'
...
SQLALCHEMY_DATABASE_URL = 'sqlite:///./test_method_aware.db'
...
class _FakeNrcClient:
    ...
    def download_artifact(self, url, *, max_redirects, max_file_bytes=None):
        ...
        content = b"%PDF-1.7 fake nrc content"
...
def test_nrc_hydrate_process_emits_normalization_contract(monkeypatch):
    ...
    assert target_artifact["normalization_contract_id"] == "aps_text_normalization_v1"
...
def test_nrc_content_index_artifacts_and_search_route(monkeypatch):
    ...
    assert content_artifact["schema_id"] == "aps.content_units.v1"
    assert content_artifact["content_contract_id"] == "aps_content_units_v1"
    assert content_artifact["chunking_contract_id"] == "aps_chunking_v1"
    ...
    assert first_row["content_contract_id"] == "aps_content_units_v1"
    assert first_row["chunking_contract_id"] == "aps_chunking_v1"
    assert first_row["run_id"] == run_id
    assert first_row["target_id"]

    searched = client.post(
        "/api/v1/connectors/nrc-adams-aps/content-search",
        json={"query": "fake nrc", "run_id": run_id, "limit": 10, "offset": 0},
    )
    assert searched.status_code == 200, searched.text
    searched_payload = searched.json()
    assert searched_payload["query_tokens"] == ["fake", "nrc"]
