...
DEFAULT_REPORT_PATH = Path("tests/reports/nrc_aps_safeguard_validation_report.json")
...
def validate_safeguard_gate(
    *,
    ...
    report = nrc_aps_safeguards.validate_safeguard_artifact_presence(
        run_rows=run_rows,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate NRC APS safeguard artifacts (fail-closed).")
    parser.add_argument(
        "--run-id",
        action="append",
        default=[],
    ...
    report = validate_safeguard_gate(
