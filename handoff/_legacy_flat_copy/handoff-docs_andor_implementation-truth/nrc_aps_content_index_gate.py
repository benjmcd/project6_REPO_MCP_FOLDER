...
DEFAULT_REPORT_PATH = Path("tests/reports/nrc_aps_content_index_validation_report.json")
...
def validate_content_index_gate(
    *,
    ...
    db = SessionLocal()
    try:
        report = nrc_aps_content_index.validate_content_index_artifact_presence(
            run_rows=run_rows,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate NRC APS content indexing artifacts (fail-closed).")
    parser.add_argument(
        "--run-id",
        action="append",
        default=[],
    ...
    report = validate_content_index_gate(
