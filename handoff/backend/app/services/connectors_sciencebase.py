...
from app.services.sciencebase_connector.reporting import report_status as build_report_status
...
def serialize_connector_run(db: Session, run: ConnectorRun) -> dict[str, Any]:
    ...
    for surface in ("files", "distributionLinks", "webLinks"):
        count = (
            db.query(ConnectorRunTarget)
            .filter(
                and_(
...
def serialize_connector_run_reports(connector_run_id: str) -> dict[str, Any]:
    return {
        "connector_run_id": connector_run_id,
        "reports": build_report_refs(connector_run_id),
        "report_status": build_report_status(connector_run_id),
    }
