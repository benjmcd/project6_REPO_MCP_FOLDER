...
def _load_candidate_runs(*, run_ids: list[str] | None, limit: int | None) -> list[dict[str, Any]]:
    ...
    def _extract_run_id(name: str) -> str | None:
        token = "_aps_evidence_citation_pack_v2.json"
        failure_token = "_aps_evidence_citation_pack_failure_v2.json"
    ...
    for path in reports_dir.glob("*_aps_evidence_citation_pack_v2.json"):
        run_id = _extract_run_id(path.name)
        if run_id:
            candidates[run_id] = max(float(path.stat().st_mtime), float(candidates.get(run_id, 0.0)))
    for path in reports_dir.glob("*_aps_evidence_citation_pack_failure_v2.json"):
        run_id = _extract_run_id(path.name)
        if run_id:
            candidates[run_id] = max(float(path.stat().st_mtime), float(candidates.get(run_id, 0.0)))
    ...
    *,
    ...
    for row in run_rows:
        ...
        if not run_id:
            continue
        scope = f"run_{run_id}"
        pack_paths = sorted(Path(settings.connector_reports_dir).glob(f"{scope}_*_aps_evidence_citation_pack_v2.json"))
        failure_paths = sorted(Path(settings.connector_reports_dir).glob(f"{scope}_*_aps_evidence_citation_pack_failure_v2.json"))
        reasons: list[str] = []
