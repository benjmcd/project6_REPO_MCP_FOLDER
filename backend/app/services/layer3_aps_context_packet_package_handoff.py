...
PACKAGE_KIND_APS_CONTEXT_PACKET_PACKAGE_HANDOFF = "aps_context_packet_package_handoff"
APS_CONTEXT_PACKET_PACKAGE_HANDOFF_SCHEMA_ID = "layer3.aps_context_packet_package_handoff.v1"
APS_CONTEXT_PACKET_PACKAGE_HANDOFF_SCHEMA_VERSION = 1
SOURCE_GATE_D_APS_PACKAGE_CONTEXT_FREEZE = "16_GATED_APS_PACKAGE_CONTEXT_FREEZE"
...
class Layer3ApsContextPacketPackageHandoffError(ValueError):
    pass


@dataclass(frozen=True)
class Layer3ApsContextPacketPackageHandoffResult:
...
def _require_existing_ref(ref: str | None, *, label: str) -> str:
    ...
    if not normalized:
        raise Layer3ApsContextPacketPackageHandoffError(f"{label} is missing")
    if not Path(normalized).exists():
        raise Layer3ApsContextPacketPackageHandoffError(f"{label} does not exist: {normalized}")
    return normalized
...
def _load_session_or_raise(db: Session, *, session_id: str) -> L3Session:
    ...
    if session is None:
        raise Layer3ApsContextPacketPackageHandoffError(f"Layer 3 session '{session_id}' was not found")
    if session.status not in TERMINAL_SESSION_STATUSES or session.completed_at is None:
        raise Layer3ApsContextPacketPackageHandoffError(
            f"Layer 3 session '{session_id}' must be terminal before Gate D APS package-derived context handoff"
        )
    return session
...
def _load_source_package_or_raise(
    ...
    *,
    ...
    if PACKAGE_KIND_APS_CONTEXT_PACKET_PACKAGE_HANDOFF in rows_by_kind:
        raise Layer3ApsContextPacketPackageHandoffError(
            f"Layer 3 session '{session_id}' already has an APS package-derived context handoff package"
        )
    source_row = rows_by_kind.get(PACKAGE_KIND_APS_EVIDENCE_REPORT_EXPORT_PACKAGE_HANDOFF)
    if source_row is None:
        raise Layer3ApsContextPacketPackageHandoffError(
            f"Layer 3 session '{session_id}' is missing the APS evidence-report-export-package handoff package required for APS package-derived context handoff"
        )
    if source_row.status not in ACCEPTED_SOURCE_PACKAGE_STATUSES:
        raise Layer3ApsContextPacketPackageHandoffError(
            f"Layer 3 package '{PACKAGE_KIND_APS_EVIDENCE_REPORT_EXPORT_PACKAGE_HANDOFF}' must be complete before APS package-derived context handoff"
        )
    source_summary = dict(source_row.summary_json or {})
    if str(source_summary.get("aps_target_family") or "").strip() != "evidence_report_export_package":
        raise Layer3ApsContextPacketPackageHandoffError(
            f"Layer 3 package '{PACKAGE_KIND_APS_EVIDENCE_REPORT_EXPORT_PACKAGE_HANDOFF}' does not point at the evidence-report-export-package APS family"
        )
    if (
        str(source_summary.get("aps_schema_id") or "").strip()
        != aps_export_package_contract.APS_EVIDENCE_REPORT_EXPORT_PACKAGE_SCHEMA_ID
    ):
        raise Layer3ApsContextPacketPackageHandoffError(
            f"Layer 3 package '{PACKAGE_KIND_APS_EVIDENCE_REPORT_EXPORT_PACKAGE_HANDOFF}' has incompatible APS schema provenance"
        )
    _require_existing_ref(
        source_row.payload_ref,
        label=f"Layer 3 package '{PACKAGE_KIND_APS_EVIDENCE_REPORT_EXPORT_PACKAGE_HANDOFF}' payload ref",
    )
    ...
    if reconciliation is None or reconciliation.session_id != session_id:
        raise Layer3ApsContextPacketPackageHandoffError(
            f"Layer 3 session '{session_id}' is missing reconciliation provenance required for APS package-derived context handoff"
        )
...
def _materialize_context_packet_or_raise(
    ...
    source_package_ref = _require_existing_ref(
        source_row.payload_ref,
        label=f"Layer 3 package '{PACKAGE_KIND_APS_EVIDENCE_REPORT_EXPORT_PACKAGE_HANDOFF}' payload ref",
    )
    try:
        source_package_payload, _source_path = aps_export_package.load_persisted_evidence_report_export_package_artifact(
            evidence_report_export_package_ref=source_package_ref
        )
        context_packet_payload = aps_context_contract.build_context_packet_payload(
            source_family=aps_context_contract.APS_CONTEXT_PACKET_SOURCE_FAMILY_PACKAGE,
            source_payload=source_package_payload,
        ...
        if not owner_run_id:
            raise Layer3ApsContextPacketPackageHandoffError(
                "APS package-derived context handoff requires a source export package with owner run id"
            )
        ...
        raise Layer3ApsContextPacketPackageHandoffError(
            f"APS package-derived context handoff failed ({exc.code}): {exc.message or str(exc)}"
        ) from exc
...
def _summary_json(
    *,
    ...
    source_descriptor = dict(context_packet_payload.get("source_descriptor") or {})
    return {
        "package_kind": PACKAGE_KIND_APS_CONTEXT_PACKET_PACKAGE_HANDOFF,
        "package_status": source_row.status,
        "source_gate": SOURCE_GATE_D_APS_PACKAGE_CONTEXT_FREEZE,
        "schema_id": APS_CONTEXT_PACKET_PACKAGE_HANDOFF_SCHEMA_ID,
        "schema_version": APS_CONTEXT_PACKET_PACKAGE_HANDOFF_SCHEMA_VERSION,
        "aps_target_family": "context_packet_package",
        ...
        "source_package_kinds_json": [PACKAGE_KIND_APS_EVIDENCE_REPORT_EXPORT_PACKAGE_HANDOFF],
        "source_package_refs_json": {
            PACKAGE_KIND_APS_EVIDENCE_REPORT_EXPORT_PACKAGE_HANDOFF: str(source_row.payload_ref or ""),
        },
        "compatibility_notes_json": [
            "aps_context_packet_package_handoff points at a persisted aps.context_packet.v1 artifact",
            "aps_evidence_report_export_package_handoff remains the required source package for this tranche",
            "context-packet contract, checksum, and gate semantics are reused without widening",
            "source_family=evidence_report_export_package remains fixed for this Layer 3 handoff path",
            "connector-run runtime refs remain untouched in this Layer 3 handoff path",
        ],
        "field_map_json": {
            "aps_evidence_report_export_package_handoff.payload_ref": "source_descriptor.source_ref",
        ...
        "handoff_status": {
            "status": "aps_context_packet_package_emitted",
            "aps_evidence_report_export_package_admitted": True,
            "context_packet_package_handoff_admitted": True,
            ...
            "source_evidence_report_export_package_id": str(source_descriptor.get("source_id") or ""),
            "source_evidence_report_export_package_checksum": str(
                source_descriptor.get("source_checksum") or ""
            ),
            "source_evidence_report_export_package_ref": str(
                source_descriptor.get("source_ref") or ""
            ),
            "source_export_count": int(source_descriptor.get("source_export_count") or 0),
            "total_sections": int(source_descriptor.get("total_sections") or 0),
            "total_citations": int(source_descriptor.get("total_citations") or 0),
            "total_groups": int(source_descriptor.get("total_groups") or 0),
        },
    }


def materialize_aps_context_packet_package_handoff(
    db: Session,
    *,
    session_id: str,
) -> Layer3ApsContextPacketPackageHandoffResult:
    ...
    db.flush()
    return Layer3ApsContextPacketPackageHandoffResult(
