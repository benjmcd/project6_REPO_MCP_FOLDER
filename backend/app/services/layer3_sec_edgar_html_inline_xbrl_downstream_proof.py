...
PROOF_STATE = "sec_edgar_html_inline_xbrl_downstream_layer3_e2e_proven"
PROOF_RECEIPT_PREFIX = "sec-edgar-html-inline-xbrl-downstream-proof"

SOURCE_FAMILY = layer3_sec_edgar_html_inline_xbrl_material_bridge.SOURCE_FAMILY
PARSER_FAMILY = layer3_sec_edgar_html_inline_xbrl_material_bridge.PARSER_FAMILY
TYPED_CONTENT_CONTRACT_ID = layer3_sec_edgar_html_inline_xbrl_material_bridge.TYPED_CONTENT_CONTRACT_ID
SOURCE_CLASS = layer3_sec_edgar_html_inline_xbrl_material_bridge.SOURCE_CLASS
BRIDGE_MODE = layer3_sec_edgar_html_inline_xbrl_material_bridge.BRIDGE_MODE
BRIDGE_READY_STATE = layer3_sec_edgar_html_inline_xbrl_material_bridge.READY_STATE
...
def record_sec_edgar_html_inline_xbrl_downstream_layer3_proof(
    ...
    if request.get("operator_confirmation") is not True:
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_operator_confirmation_required",
            "operator_confirmation=true is required before recording SEC EDGAR HTML/iXBRL downstream proof.",
    ...
    material_bridge_receipt_id = _required(request, "material_bridge_receipt_id")
    ...
    bridge = _validate_material_bridge_projection(
        ...
        material_bridge_receipt_id=material_bridge_receipt_id,
    ...
    authority = _authority_hashes(parser_receipt, bridge)
        ...
        **proof_input,
        ...
        "status_projection": {
            ...
            "next_allowed_actions": [
                "use_sec_edgar_html_inline_xbrl_downstream_proof_as_layer3_e2e_evidence",
                "select_sec_edgar_html_inline_xbrl_downstream_operator_status_after_runtime_evidence",
            ],
        },
        "next_allowed_actions": [
            "use this proof as SEC EDGAR HTML/iXBRL downstream Layer 3 evidence",
    ...
    if _contains_forbidden_output_ref(proof):
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_raw_authority_exposed",
            "SEC EDGAR HTML/iXBRL downstream proof would expose raw path, URL, token, or artifact-byte authority.",
            http_status=409,
        )
    return proof
...
def _normalise_request(fields: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if blocked or nested_blocked:
        _blocked(
            ...
            "SEC EDGAR HTML/iXBRL downstream proof does not admit caller paths, URLs, bytes, credentials, connector, model, browser, source-expansion, or frontend authority.",
            blocked_fields=[*blocked, *nested_blocked],
        )
    unknown = sorted(set(request) - ALLOWED_FIELDS)
    if unknown:
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_unknown_field",
            "SEC EDGAR HTML/iXBRL downstream proof fields are intentionally scoped.",
            blocked_fields=unknown,
        )
    schema_id = str(request.get("schema_id") or REQUEST_SCHEMA_ID).strip()
    if schema_id != REQUEST_SCHEMA_ID:
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_schema_not_admitted",
            "SEC EDGAR HTML/iXBRL downstream proof requires the admitted request schema.",
...
def _validate_parser_receipt(*, parser_receipt_id: str, parser_receipt_hash: str) -> dict[str, Any]:
    ...
    if mismatches:
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_parser_receipt_mismatch",
            "SEC EDGAR HTML/iXBRL downstream proof parser authority is missing, stale, or outside the admitted source-family contract.",
            http_status=409,
...
def _validate_material_bridge_projection(
    *,
    ...
    bridge = layer3_sec_edgar_html_inline_xbrl_material_bridge.inspect_sec_edgar_html_inline_xbrl_material_bridge_status(
        material_bridge_receipt_id
    )
    if bridge.get("bridge_state") != BRIDGE_READY_STATE:
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_missing_ready_bridge",
            "SEC EDGAR HTML/iXBRL downstream proof requires a ready material-authority bridge receipt.",
            http_status=409,
            blocked_fields=["material_bridge_receipt_id"],
            next_allowed_actions=["refresh_sec_edgar_html_inline_xbrl_material_bridge"],
        )
    mismatches = []
    for field, expected in {
        "mode": BRIDGE_MODE,
        "bridge_receipt_id": material_bridge_receipt_id,
        "bridge_receipt_hash": material_bridge_receipt_hash,
        "dataset_version_id": dataset_version_id,
        "source_family": SOURCE_FAMILY,
        "parser_family": PARSER_FAMILY,
    ...
    for field in (
        ...
        "arbitrary_url_or_upload_parse_admitted",
    ...
    if mismatches:
        _blocked(
            ...
            "SEC EDGAR HTML/iXBRL downstream proof bridge authority is stale, blocked, or mismatched.",
            http_status=409,
            blocked_fields=sorted(set(mismatches)),
            next_allowed_actions=["refresh_sec_edgar_html_inline_xbrl_material_bridge"],
        )
...
def _validate_gate_b_session(
    ...
    *,
    ...
    if session is None or manifest is None:
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_missing_gate_b_session",
            "SEC EDGAR HTML/iXBRL downstream proof requires an existing committed Gate B session and selection manifest.",
            http_status=404,
    ...
    if session.selection_manifest_id != selection_manifest_id or manifest.session_id != session_id:
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_selection_manifest_mismatch",
            "SEC EDGAR HTML/iXBRL downstream proof session and selection manifest do not match.",
            http_status=409,
            blocked_fields=["session_id", "selection_manifest_id"],
        )
    ...
    if (
        ...
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_gate_b_hash_mismatch",
            "SEC EDGAR HTML/iXBRL downstream proof Gate B decision manifest is missing or stale.",
            http_status=409,
    ...
    if not isinstance(gate_b_record, dict):
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_gate_b_idempotency_missing",
            "SEC EDGAR HTML/iXBRL downstream proof requires the committed Gate B idempotency record.",
            http_status=409,
    ...
    if (
        ...
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_gate_b_payload_mismatch",
            "SEC EDGAR HTML/iXBRL downstream proof Gate B commit does not match the bridge-returned material preview and decision manifest.",
            http_status=409,
...
def _validate_material_snapshot(
    ...
    *,
    ...
    if len(matching) != 1:
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_material_snapshot_mismatch",
            "SEC EDGAR HTML/iXBRL downstream proof requires exactly one committed DatasetVersion material snapshot for the bridged DatasetVersion.",
            http_status=409,
    ...
    if snapshot.payload_hash != material_snapshot_payload_hash:
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_material_snapshot_hash_mismatch",
            "SEC EDGAR HTML/iXBRL downstream proof material snapshot payload hash is stale or mismatched.",
            http_status=409,
    ...
    if _authority_value(snapshot, payload, "source_family") != SOURCE_FAMILY:
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_source_family_mismatch",
            "SEC EDGAR HTML/iXBRL downstream proof material snapshot does not prove sec_edgar_html_inline_xbrl authority.",
            http_status=409,
            blocked_fields=["material_snapshot_payload_hash"],
        )
    if _authority_value(snapshot, payload, "parser_family") != PARSER_FAMILY:
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_parser_family_mismatch",
            "SEC EDGAR HTML/iXBRL downstream proof material snapshot does not prove the admitted HTML/iXBRL parser authority.",
            http_status=409,
            blocked_fields=["material_snapshot_payload_hash"],
        )
    if _authority_value(snapshot, payload, "typed_content_contract_id") != TYPED_CONTENT_CONTRACT_ID:
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_typed_content_contract_mismatch",
            "SEC EDGAR HTML/iXBRL downstream proof material snapshot does not prove the admitted typed-content contract.",
            http_status=409,
            blocked_fields=["material_snapshot_payload_hash"],
        )
...
def _read_material_payload(snapshot: L3MaterialSnapshot) -> dict[str, Any]:
    ...
    if not path.is_file():
        _blocked(
            ...
            "SEC EDGAR HTML/iXBRL downstream proof cannot read the committed material snapshot payload.",
            http_status=409,
            blocked_fields=["material_snapshot_payload_hash"],
        )
    try:
        ...
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_material_snapshot_payload_unreadable",
            "SEC EDGAR HTML/iXBRL downstream proof cannot read the committed material snapshot payload.",
            http_status=409,
            blocked_fields=["material_snapshot_payload_hash"],
        )
    if hashlib.sha256(stable_json_bytes(payload)).hexdigest() != snapshot.payload_hash:
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_material_snapshot_payload_hash_invalid",
            "SEC EDGAR HTML/iXBRL downstream proof material snapshot payload bytes no longer match the committed hash.",
            http_status=409,
            blocked_fields=["material_snapshot_payload_hash"],
        )
    if not isinstance(payload, dict):
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_material_snapshot_payload_invalid",
            "SEC EDGAR HTML/iXBRL downstream proof material snapshot payload is not an object.",
            http_status=409,
            blocked_fields=["material_snapshot_payload_hash"],
        )
    return payload
...
def _validate_coverage_evidence(
    ...
    *,
    ...
    if not isinstance(value, dict):
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_coverage_evidence_missing",
    ...
    if missing:
        _blocked(
            ...
            "SEC EDGAR HTML/iXBRL downstream proof evidence is missing required coverage steps.",
            http_status=409,
    ...
    for step in sorted(REQUIRED_COVERAGE):
        ...
        if not isinstance(item, dict):
            _blocked(
                "sec_edgar_html_inline_xbrl_downstream_proof_coverage_item_invalid",
                "Each SEC EDGAR HTML/iXBRL downstream proof coverage item must be an object.",
                http_status=409,
                blocked_fields=[f"coverage_evidence.{step}"],
            )
        if str(item.get("status") or "").strip() != "proven":
            _blocked(
                "sec_edgar_html_inline_xbrl_downstream_proof_coverage_item_not_proven",
                "Each SEC EDGAR HTML/iXBRL downstream proof coverage item must be marked proven.",
                http_status=409,
        ...
        if _is_forbidden_ref(evidence_ref):
            _blocked(
                "sec_edgar_html_inline_xbrl_downstream_proof_coverage_exposes_forbidden_reference",
                "SEC EDGAR HTML/iXBRL downstream proof coverage cannot expose raw URL, file, or local path references.",
                http_status=409,
...
def _validate_coverage_negative_invariants(step: str, item: Mapping[str, Any]) -> None:
    for field in (
        ...
        if item.get(field) not in (None, False):
            _blocked(
                "sec_edgar_html_inline_xbrl_downstream_proof_coverage_exposes_forbidden_authority",
                "SEC EDGAR HTML/iXBRL downstream proof coverage exposes non-admitted authority.",
                http_status=409,
...
def _validate_coverage_bindings(
    ...
    *,
    ...
    if step in SESSION_BOUND_COVERAGE and str(item.get("session_id") or "") != session_id:
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_coverage_session_mismatch",
            "SEC EDGAR HTML/iXBRL downstream proof coverage must bind to the committed Layer 3 session.",
            http_status=409,
    ...
    for field, expected in expected_by_field.get(step, {}).items():
        if str(item.get(field) or "") != expected:
            _blocked(
                "sec_edgar_html_inline_xbrl_downstream_proof_coverage_authority_mismatch",
                "SEC EDGAR HTML/iXBRL downstream proof coverage is not bound to the expected server-owned authority.",
                http_status=409,
                blocked_fields=[f"coverage_evidence.{step}.{field}"],
            )
    if not any(
        ...
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_coverage_not_bound_to_server_receipt",
            "SEC EDGAR HTML/iXBRL downstream proof coverage must include a server response hash, receipt hash, or receipt id.",
            http_status=409,
            blocked_fields=[f"coverage_evidence.{step}"],
        )


def _authority_hashes(parser_receipt: Mapping[str, Any], bridge: Mapping[str, Any]) -> dict[str, str]:
    ...
    if missing:
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_authority_hash_missing",
            "SEC EDGAR HTML/iXBRL downstream proof requires parser and material bridge SHA-256 authority bindings.",
            http_status=409,
...
def _required(fields: Mapping[str, Any], key: str) -> str:
    ...
    if not value:
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_required_field_missing",
            "A required SEC EDGAR HTML/iXBRL downstream proof field is missing or empty.",
            blocked_fields=[key],
        )
    return value
...
def _required_hash(fields: Mapping[str, Any], key: str) -> str:
    ...
    if not _is_sha256(value):
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_hash_invalid",
            "SEC EDGAR HTML/iXBRL downstream proof hash fields must be SHA-256 hex strings.",
            blocked_fields=[key],
        )
    return value
...
def _required_coverage_hash(step: str, item: Mapping[str, Any]) -> str:
    ...
    if not _is_sha256(value):
        _blocked(
            "sec_edgar_html_inline_xbrl_downstream_proof_coverage_hash_invalid",
            "SEC EDGAR HTML/iXBRL downstream proof coverage evidence_hash values must be SHA-256 hex strings.",
...
def _require_exact(fields: Mapping[str, Any], key: str, expected: str) -> None:
    if _required(fields, key) != expected:
        _blocked(
            f"sec_edgar_html_inline_xbrl_downstream_proof_{key}_not_admitted",
            "SEC EDGAR HTML/iXBRL downstream proof request does not match the admitted runtime contract.",
            blocked_fields=[key],
        )
