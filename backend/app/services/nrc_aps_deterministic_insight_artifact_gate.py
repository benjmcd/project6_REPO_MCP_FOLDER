...
def _owner_run_id_from_artifact_payload(payload: dict[str, Any]) -> str | None:
    ...
    owner_run_id = str(source_context_dossier.get("owner_run_id") or "").strip()
...
def _artifact_scope_for_run_id(run_id: str) -> str:
    return f"run_{insight._safe_scope_token(run_id)}"
    ...
    *,
    ...
    for row in run_rows:
        ...
        for artifact_path in sorted(
            Path(settings.connector_reports_dir).glob(f"{scope}_*_aps_deterministic_insight_artifact_v1.json")
        ):
        ...
        for failure_path in sorted(
            Path(settings.connector_reports_dir).glob(f"{scope}_*_aps_deterministic_insight_artifact_failure_v1.json")
        ):
        ...
        checks.append(
            {
                "run_id": run_id,
                "deterministic_insight_artifact_refs": [str(path) for path, _payload in matched_artifacts],
