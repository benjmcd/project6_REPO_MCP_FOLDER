...
def _load_processed_document(
    *,
        ...
        if candidate.exists():
            try:
                ...
                return {
                    ...
                    "visual_page_refs": [dict(r) for r in (diagnostics_payload.get("visual_page_refs") or extraction.get("visual_page_refs") or []) if isinstance(r, dict)],
                }
            except OSError:
                pass
    ...
    return {
        ...
        "visual_page_refs": [dict(r) for r in (extracted.get("visual_page_refs") or extraction.get("visual_page_refs") or []) if isinstance(r, dict)],
    }
...
def build_content_units_payload_from_target_artifact(
    *,
    ...
    if artifact_outcome_status == APS_CONTENT_STATUS_ARTIFACT_NOT_AVAILABLE:
        ...
        visual_page_refs = [dict(r) for r in (processed.get("visual_page_refs") or []) if isinstance(r, dict)]
...
def _deserialize_visual_page_refs(raw: str | None) -> list[dict[str, Any]]:
    if not raw:
        return []
    try:
