...
from app.services import layer3_connector_promotion as connector_promotion
...
from app.services.layer3_workbench_package_state import (
    ...
    mixed_source_external_export_download_readiness_from_reconciliation as _mixed_source_external_export_download_readiness_from_reconciliation,
...
from app.services.layer3_mockup_activation_readiness import (
    mockup_activation_readiness_contract as _mockup_activation_readiness_contract,
)
...
from app.services.layer3_handoff_contract import (
    ...
    mixed_source_aps_handoff_dispatch_blocked_fields,
    mixed_source_external_export_download_readiness_blocked_fields,
...
EXTERNAL_LOCAL_EXPORT_LIFECYCLE_SCHEMA_ID = "layer3.external_local_export.lifecycle.v1"
...
MIXED_SOURCE_APS_HANDOFF_DISPATCH_SCHEMA_ID = "layer3.mixed_source_aps_handoff_dispatch.v1"
MIXED_SOURCE_APS_HANDOFF_DISPATCH_OPERATOR_DECISION = "dispatch_mixed_source_aps_handoff"
MIXED_SOURCE_APS_HANDOFF_TARGET = "mixed_source_aps_evidence_bundle"
MIXED_SOURCE_APS_HANDOFF_DISPATCH_MODE = "server_side_mixed_source_aps_handoff"
MIXED_SOURCE_APS_EVIDENCE_BUNDLE_SCHEMA_ID = "layer3.mixed_source_aps_evidence_bundle_reference.v1"
MIXED_SOURCE_APS_EVIDENCE_BUNDLE_PACKAGE_KIND = "mixed_source_aps_evidence_bundle_reference"
...
SOURCE_INTAKE_PACKAGE_REVIEW_PREVIEW_DOWNSTREAM_UNAVAILABLE = SOURCE_INTAKE_PACKAGE_CONSTRUCTION_DOWNSTREAM_UNAVAILABLE
...
def bootstrap() -> dict[str, Any]:
    return build_bootstrap_contract(
        ...
        state_action_contract=_workbench_state_action_contract(),
        authority_matrix_contract=_workbench_authority_matrix_contract(),
        mockup_activation_readiness=_mockup_activation_readiness_contract(),
...
def _gate_b_response_from_session(
    *,
    ...
    scoped_nonapproved = not approved_source_classes and summary.get("current_gate") == "gate_b"
    if connector_only_gate_b_admission:
        next_state = "connector_source_intake_gate_b_admitted"
        current_gate = "gate_b"
    elif scoped_nonapproved:
        next_state = "gate_b_decision_recorded"
        current_gate = "gate_b"
    else:
...
def _connector_promotion_workbench_error(
    db: Session,
    exc: connector_promotion.ConnectorPromotionError,
...
def _gate_b_decision_impl(db: Session, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if settings.layer3_connector_promotion_bridge_enabled:
        possible_candidate = connector_promotion.possible_gate_b_promotion_candidate(
            raw_decisions
        )
        if possible_candidate is not None:
            try:
                server_exact = connector_promotion.side_effect_free_server_exact_candidate(
                    db.get_bind(),
                    possible_candidate,
                )
                if server_exact:
                    if not connector_promotion.attestation_precondition_available(
                        possible_candidate
                    ):
                        raise connector_promotion.ConnectorPromotionError(
                            "connector_promotion_bridge_unavailable",
                            "Connector promotion bridge is unavailable.",
                            http_status=503,
                            retryable=True,
                        )
                    promotion_candidate = possible_candidate
    ...
    for raw in raw_decisions:
        ...
        if source_class == CONNECTOR_SOURCE_INTAKE_SOURCE_FAMILY and not (
            promotion_candidate is not None
            and candidate_id == promotion_candidate.candidate_id
        ):
            try:
                validate_connector_intake_gate_b_decision_basis(
                    db,
    ...
    if promotion_candidate is not None:
        try:
            promotion_arbitration = connector_promotion.begin_gate_b_arbitration(
                db,
                promotion_candidate,
            )
        except connector_promotion.ConnectorPromotionError as exc:
    ...
    if promotion_arbitration is not None and promotion_arbitration.receipt is not None:
        winning_origin = promotion_arbitration.winning_origin
        if winning_origin is None:
            exc = connector_promotion.ConnectorPromotionError(
            ...
            raise _connector_promotion_workbench_error(db, exc)
        winning_session, winning_manifest, winning_decision_manifest = winning_origin
        response = _gate_b_response_from_session(
            request_id=request_id,
            status="already_committed",
            session=winning_session,
            manifest=winning_manifest,
            decision_manifest=winning_decision_manifest,
        )
        result = connector_promotion.promotion_result(
            promotion_arbitration.candidate,
            "reused",
            promotion_arbitration.receipt.connector_promotion_receipt_id,
        )
        db.rollback()
        db.info.pop("b1b_promotion_identity_lock", None)
        connector_promotion.consume_gate_b_promotion_result(result)
        return response
    ...
    def complete_existing_promotion_response(response: dict[str, Any]) -> dict[str, Any]:
        if promotion_arbitration is None:
            return response
        if promotion_arbitration.candidate.decision != "approved":
            result = connector_promotion.promotion_result(
                promotion_arbitration.candidate,
                "none",
                None,
    ...
    if existing_idempotency is not None:
        ...
        if (
            ...
            return complete_existing_promotion_response(
    ...
    scoped_nonapproved = promotion_arbitration is not None and not approved
    if not approved and not scoped_nonapproved:
        raise Layer3WorkbenchError(
    ...
    persisted_items = approved if approved else decisions
    manifest_items = []
        ...
        manifest_items.append(
            {
                ...
                "selection_basis": {
                    ...
                    "gate_b_decision": item["decision"],
                },
                "expansion_reason": (
                    "gate_b_approved_material"
                    if item["decision"] == "approved"
                    else "gate_b_nonapproved_material"
                ),
            }
        )
    ...
    current_gate = (
        "gate_b"
        if connector_only_gate_b_admission or scoped_nonapproved
        else "gate_c"
    )
    ...
    for descriptor, item in zip(descriptors, persisted_items, strict=True):
        ...
        _, snapshots = record_retrieval_event(
            ...
            outcome="loaded",
            reason_code=(
                "gate_b_approved_preview_material"
                if item["decision"] == "approved"
                else "gate_b_nonapproved_preview_material"
            ),
            loaded_materials=[
                SnapshotMaterial(
        ...
        if (
            promotion_candidate is not None
            and item["candidate_id"] == promotion_candidate.candidate_id
        ):
            promotion_snapshot = snapshots[0]
    promotion_internal_result = None
    if promotion_arbitration is not None:
        if promotion_snapshot is None:
            raise Layer3WorkbenchError(
                "gate_b_idempotency_state_inconsistent",
                "Gate B promotion snapshot was not staged.",
                status="conflict",
                http_status=409,
            )
        promotion_internal_result = connector_promotion.stage_gate_b_promotion_receipt(
            db,
            ...
            submitted_material_preview_hash=material_preview_hash,
        )
    finalize_session(db, session=session)
    db.commit()
    db.info.pop("b1b_promotion_identity_lock", None)
    if promotion_internal_result is not None:
        connector_promotion.consume_gate_b_promotion_result(promotion_internal_result)
    return _gate_b_response_from_session(
        request_id=request_id,
        status="ok",
        session=session,
...
def gate_b_decision(db: Session, payload: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        if db.info.get("b1b_promotion_identity_lock"):
            db.rollback()
            db.info.pop("b1b_promotion_identity_lock", None)
        raise
...
def _source_intake_aps_dispatch_prepare_state_admitted(
    prepare_state: dict[str, Any],
) -> bool:
    return bool(
        prepare_state.get("pass_type") == PASS_TYPE_SINGLE_ITEM
        and prepare_state.get("pass_scope") == PASS_SCOPE_SOURCE_INTAKE_QUALITATIVE
        and prepare_state.get("method") == SOURCE_INTAKE_EXECUTION_METHOD_NAME
        and prepare_state.get("source_gate") == SOURCE_INTAKE_EXECUTION_START_SOURCE_GATE
        and prepare_state.get("package_construction_source_gate")
        == SOURCE_INTAKE_PACKAGE_CONSTRUCTION_SOURCE_GATE
        and prepare_state.get("source_shape") == SOURCE_INTAKE_SOURCE_FAMILY
        and list(prepare_state.get("source_dataset_version_ids") or []) == []
        and prepare_state.get("package_review_submit_schema_id")
        == SOURCE_INTAKE_PACKAGE_REVIEW_SUBMIT_SCHEMA_ID
        and bool(str(prepare_state.get("source_intake_record_id") or "").strip())
        and bool(str(prepare_state.get("candidate_id") or "").strip())
    ...
    session.summary_json = {
        **_json_clone(session.summary_json or {}),
        "execution_result_review": {
            ...
            "output_schema_id": review_state.get("output_schema_id"),
            ...
            "output_hash": review_state.get("output_hash"),
            "planned_pass_source_gate": review_state.get("planned_pass_source_gate"),
            "unresolved_trace_count": unresolved_trace_count,
            "package_review_enabled": False,
            "handoff_enabled": False,
            "downstream_unavailable": list(EXECUTION_RESULT_REVIEW_DOWNSTREAM_UNAVAILABLE),
...
def _aps_handoff_dispatch_response(
    *,
    ...
    source_intake_dispatch = (
        dispatch_state.get("pass_scope") == PASS_SCOPE_SOURCE_INTAKE_QUALITATIVE
        or dispatch_state.get("source_gate") == SOURCE_INTAKE_EXECUTION_START_SOURCE_GATE
        or dispatch_state.get("package_construction_source_gate")
        == SOURCE_INTAKE_PACKAGE_CONSTRUCTION_SOURCE_GATE
    )
    mixed_source_dispatch = (
        dispatch_state.get("aps_handoff_dispatch_schema_id") == MIXED_SOURCE_APS_HANDOFF_DISPATCH_SCHEMA_ID
        or dispatch_state.get("package_family") == PACKAGE_FAMILY_MIXED_DATASET_DOCUMENT
        or dispatch_state.get("package_construction_source_gate") == SOURCE_WORKBENCH_MIXED_PACKAGE_CONSTRUCTION_FREEZE
    )
    response_schema_id = APS_HANDOFF_DISPATCH_SCHEMA_ID
    if qualitative_aps_dispatch:
        response_schema_id = QUAL_APS_APS_HANDOFF_DISPATCH_SCHEMA_ID
    if source_intake_dispatch:
        response_schema_id = SOURCE_INTAKE_APS_HANDOFF_DISPATCH_SCHEMA_ID
    if mixed_source_dispatch:
        response_schema_id = MIXED_SOURCE_APS_HANDOFF_DISPATCH_SCHEMA_ID
    downstream_unavailable = list(dispatch_state.get("downstream_unavailable") or APS_HANDOFF_DISPATCH_DOWNSTREAM_UNAVAILABLE)
    body = {
        **_base_response(
            response_schema_id,
            request_id=request_id,
        ...
        "next_allowed_actions": (
            ["select_mixed_source_external_export_download_readiness_freeze"]
            if mixed_source_dispatch
            else []
            if (qualitative_aps_dispatch or source_intake_dispatch)
            else ["prepare_external_export_download"]
        ),
        ...
        "authority_rail": _authority_rail(
            ...
            persistence_mode=(
                "durable_mixed_source_aps_handoff_dispatch"
                if mixed_source_dispatch
                else "durable_source_intake_aps_handoff_dispatch"
                if source_intake_dispatch
                else "durable_aps_handoff_dispatch"
            ),
            downstream_unavailable=downstream_unavailable,
    ...
    *,
    ...
    if reconciliation is not None or packages:
        ...
        if not isinstance(commit_summary, dict):
            commit_summary = {}
        commit_authority = commit_summary.get("authority_basis")
        if not isinstance(commit_authority, dict):
            commit_authority = {}
        ...
        package_review_submit_enabled = bool(
            ...
            and (
                ...
                or source_intake_package_construction
            )
        )
        if package_review_submit_enabled and source_intake_package_construction:
            downstream_unavailable = list(SOURCE_INTAKE_PACKAGE_REVIEW_SUBMIT_DOWNSTREAM_UNAVAILABLE)
        elif package_review_submit_enabled and cohort_package_construction:
        ...
        if not isinstance(downstream_unavailable, list):
            downstream_unavailable = (
                list(
                    ...
                    else SOURCE_INTAKE_PACKAGE_REVIEW_SUBMIT_DOWNSTREAM_UNAVAILABLE
        ...
        return {
            ...
            "analysis_run_id": commit_summary.get("analysis_run_id") or commit_authority.get("analysis_run_id"),
            "result_review_record_ref": (
                commit_summary.get("result_review_record_ref")
                or commit_authority.get("result_review_record_ref")
            ),
            ...
            "pass_type": commit_summary.get("pass_type") or commit_authority.get("pass_type"),
            "pass_scope": commit_summary.get("pass_scope") or commit_authority.get("pass_scope"),
            "method": commit_summary.get("method") or commit_authority.get("method"),
            "source_gate": commit_summary.get("source_gate") or commit_authority.get("source_gate"),
            "source_shape": commit_summary.get("source_shape") or commit_authority.get("source_shape"),
            "source_dataset_version_ids": _json_clone(
                commit_summary.get("source_dataset_version_ids")
                or commit_authority.get("source_dataset_version_ids")
                or []
            ),
            "source_intake_record_id": (
                commit_summary.get("source_intake_record_id")
                or commit_authority.get("source_intake_record_id")
            ),
            "candidate_id": commit_summary.get("candidate_id") or commit_authority.get("candidate_id"),
            "output_payload_ref": commit_summary.get("output_payload_ref") or commit_authority.get("output_payload_ref"),
            "output_payload_hash": (
                commit_summary.get("output_payload_hash") or commit_authority.get("output_payload_hash")
            ),
            "package_commit_enabled": False,
    ...
    if source_intake_preview:
        ...
        return {
            **_base_response(
            ...
            "candidate_package_kinds": package_review_candidate_projection(
                package_commit_enabled=True,
                readiness_reason="source-intake package construction is admitted for this boundary",
            ),
            "package_owner_compatibility": {
                "schema_id": "layer3.source_intake_package_owner_compatibility.v1",
                "owner_service": "source_intake_package_construction",
                "assessment_basis": [
                ...
                "materialize_package_entry_callable": True,
                "workbench_package_commit_callable": True,
                "preview_candidate_projection_compatible": True,
                "construction_compatible_with_current_workbench_state": True,
                "missing_owner_service_inputs": [],
...
def _mixed_source_aps_handoff_dispatch(db: Session, payload: dict[str, Any]) -> dict[str, Any]:
    request_id = str(payload.get("client_request_id") or "").strip()
    session_id = str(payload.get("session_id") or "").strip()
    material_preview_id = str(payload.get("material_preview_id") or "").strip()
    material_preview_hash = str(payload.get("material_preview_hash") or "").strip()
    ...
    blocked_payload_fields = mixed_source_aps_handoff_dispatch_blocked_fields(payload)
    if blocked_payload_fields:
        blocked_text = ", ".join(blocked_payload_fields)
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_scope_not_admitted",
            f"Mixed-source APS handoff dispatch request includes non-admitted fields: {blocked_text}.",
            status="invalid",
            blocked_fields=blocked_payload_fields,
            next_allowed_actions=["submit_bounded_mixed_source_aps_handoff_dispatch_request"],
        )
    ...
    if missing:
        raise Layer3WorkbenchError(
            "missing_mixed_source_aps_handoff_dispatch_fields",
            f"Mixed-source APS handoff dispatch request is missing required fields: {', '.join(missing)}.",
            status="invalid",
            blocked_fields=missing,
            next_allowed_actions=["submit_complete_mixed_source_aps_handoff_dispatch_request"],
        )
    if supplied_package_review_state != PACKAGE_REVIEW_APPROVED_STATE:
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_requires_approved_package_review",
            "Mixed-source APS handoff dispatch requires package_review_state to be package_review_approved.",
            status="blocked",
            http_status=409,
            blocked_fields=["package_review_state"],
            next_allowed_actions=["inspect_package_review_submit_state"],
    ...
    if supplied_handoff_export_state != HANDOFF_EXPORT_PREPARED_STATE:
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_requires_prepared_handoff_export",
            "Mixed-source APS handoff dispatch requires handoff_export_state to be handoff_export_prepared.",
            status="blocked",
            http_status=409,
    ...
    if handoff_target != "mixed_source_review_package":
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_target_not_admitted",
            "handoff_target must be mixed_source_review_package for mixed-source APS handoff dispatch.",
            status="invalid",
            blocked_fields=["handoff_target"],
        )
    if export_mode != "reference_envelope_only":
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_export_mode_not_admitted",
            "export_mode must be reference_envelope_only for mixed-source APS handoff dispatch.",
            status="invalid",
            blocked_fields=["export_mode"],
        )
    if aps_handoff_target != MIXED_SOURCE_APS_HANDOFF_TARGET:
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_target_family_not_admitted",
            "aps_handoff_target must be mixed_source_aps_evidence_bundle.",
            status="invalid",
            blocked_fields=["aps_handoff_target"],
        )
    if dispatch_mode != MIXED_SOURCE_APS_HANDOFF_DISPATCH_MODE:
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_mode_not_admitted",
            "dispatch_mode must be server_side_mixed_source_aps_handoff.",
            status="invalid",
            blocked_fields=["dispatch_mode"],
        )
    if operator_decision != MIXED_SOURCE_APS_HANDOFF_DISPATCH_OPERATOR_DECISION:
        raise Layer3WorkbenchError(
            "unsupported_mixed_source_aps_handoff_dispatch_decision",
            "operator_decision must be dispatch_mixed_source_aps_handoff.",
            status="invalid",
            blocked_fields=["operator_decision"],
        )
    ...
    if expected_package_kinds != list(PACKAGE_REVIEW_PREVIEW_CANDIDATE_KINDS):
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_kinds_mismatch",
            "Mixed-source APS handoff dispatch admits exactly canonical_internal, user_facing, and review_facing package kinds.",
            status="conflict",
            http_status=409,
            blocked_fields=["expected_package_kinds"],
        )
    ...
    if preview_body.get("package_review_preview_hash") != supplied_package_preview_hash:
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_preview_mismatch",
            "Mixed-source APS handoff dispatch must reference the current server-recomputed package-review preview hash.",
            status="conflict",
            http_status=409,
            blocked_fields=["package_review_preview_hash"],
            next_allowed_actions=["refresh_package_review_preview"],
    ...
    if preview_body.get("contract_hash") != supplied_contract_hash:
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_contract_mismatch",
            "Mixed-source APS handoff dispatch must reference the current server-recomputed contract hash.",
            status="conflict",
            http_status=409,
            blocked_fields=["contract_hash"],
            next_allowed_actions=["refresh_package_review_preview"],
    ...
    if reconciliation is None:
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_requires_package_construction",
            "Mixed-source APS handoff dispatch requires a P15 mixed-source package construction reconciliation.",
            status="blocked",
            http_status=409,
            blocked_fields=["reconciliation_record_id"],
            next_allowed_actions=["package_construction_commit"],
    ...
    if reconciliation_summary.get("source_gate") != SOURCE_WORKBENCH_MIXED_PACKAGE_CONSTRUCTION_FREEZE:
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_construction_source_gate_mismatch",
            "Mixed-source APS handoff dispatch requires P15 mixed-source package construction authority.",
            status="conflict",
            http_status=409,
            blocked_fields=["reconciliation_record_id"],
        )
    ...
    if not isinstance(commit_summary, dict):
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_non_workbench_package_state",
            "Mixed-source APS handoff dispatch requires workbench package-construction commit provenance.",
            status="blocked",
            http_status=409,
            next_allowed_actions=["inspect_existing_package_state"],
        )
    ...
    if commit_mismatches:
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_construction_mismatch",
            "Stored mixed-source package construction provenance does not match the supplied APS handoff authority.",
            status="conflict",
            http_status=409,
            blocked_fields=commit_mismatches,
        )
    ...
    if supplied_construction_basis_hash != expected_construction_basis_hash:
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_construction_basis_mismatch",
            "Supplied construction_basis_hash does not match the persisted mixed-source package construction.",
            status="conflict",
            http_status=409,
            blocked_fields=["construction_basis_hash"],
    ...
    if unexpected_package_kinds:
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_unexpected_package_state",
            "Mixed-source APS handoff dispatch cannot proceed with unexpected package kinds on the reconciliation.",
            status="blocked",
            http_status=409,
            blocked_fields=["package_kinds"],
            next_allowed_actions=["inspect_existing_package_state"],
    ...
    if (
        ...
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_requires_complete_package_set",
            "Mixed-source APS handoff dispatch requires exactly canonical_internal, user_facing, and review_facing packages.",
            status="blocked",
            http_status=409,
            blocked_fields=["output_package_ids"],
            next_allowed_actions=["inspect_existing_package_state"],
    ...
    if not isinstance(raw_output_package_ids, list):
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_package_ids_invalid",
            "output_package_ids must be a list of the three constructed output package ids.",
            status="invalid",
            blocked_fields=["output_package_ids"],
        )
    expected_package_ids = [package.output_package_id for package in ordered_packages]
    if [str(item or "").strip() for item in raw_output_package_ids] != expected_package_ids:
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_package_ids_mismatch",
            "Supplied output_package_ids do not match the constructed mixed-source package set.",
            status="conflict",
            http_status=409,
            blocked_fields=["output_package_ids"],
    ...
    if not isinstance(raw_payload_hashes, (list, dict)):
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_payload_hashes_invalid",
            "payload_hashes must be either a list of package hashes or a mapping keyed by package kind or package id.",
            status="invalid",
            blocked_fields=["payload_hashes"],
        )
    ...
    if canonical_payload_hashes is None:
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_payload_hashes_mismatch",
            "Supplied payload_hashes do not match the constructed mixed-source package payload hashes.",
            status="conflict",
            http_status=409,
            blocked_fields=["payload_hashes"],
    ...
    package_kinds = [package.package_kind for package in ordered_packages]
    public_payload_refs = [_public_mixed_package_payload_ref(package) for package in ordered_packages]
    public_package_ref_map = {
        package.package_kind: _public_mixed_package_payload_ref(package)
    ...
    if package_review_submit is None:
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_requires_package_review_submit",
            "Mixed-source APS handoff dispatch requires an existing P16 package-review submit state.",
            status="blocked",
            http_status=409,
            blocked_fields=["package_review_submit_record_ref"],
            next_allowed_actions=["submit_mixed_source_package_review"],
    ...
    if list(package_review_submit.get("output_package_ids") or []) != expected_package_ids:
        submit_mismatches.append("output_package_ids")
    if list(package_review_submit.get("package_kinds") or []) != package_kinds:
        submit_mismatches.append("package_kinds")
    if list(package_review_submit.get("payload_hashes") or []) != canonical_payload_hashes:
        submit_mismatches.append("payload_hashes")
    if submit_mismatches:
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_package_review_submit_mismatch",
            "Stored mixed-source package-review submit authority does not match the APS handoff request.",
            status="conflict",
            http_status=409,
            blocked_fields=sorted(set(submit_mismatches)),
        )
    ...
    if prepare_state is None or prepare_state.get("handoff_export_state") != HANDOFF_EXPORT_PREPARED_STATE:
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_requires_prepared_handoff_export",
            "Mixed-source APS handoff dispatch requires recorded handoff_export_prepared state.",
            status="blocked",
            http_status=409,
            blocked_fields=["handoff_export_state"],
            next_allowed_actions=["record_mixed_source_handoff_export_prepare"],
        )
    ...
    if supplied_envelope_ref != envelope_ref:
        prepare_mismatches.append("handoff_export_envelope_ref")
    if list(prepare_state.get("output_package_ids") or []) != expected_package_ids:
        prepare_mismatches.append("output_package_ids")
    if list(prepare_state.get("package_kinds") or []) != package_kinds:
        prepare_mismatches.append("package_kinds")
    if list(prepare_state.get("payload_refs") or []) != public_payload_refs:
    ...
    if not isinstance(envelope, dict):
        prepare_mismatches.append("handoff_export_envelope")
    else:
        if envelope.get("schema_id") != "layer3.mixed_source_handoff_export_envelope.v1":
            prepare_mismatches.append("handoff_export_envelope.schema_id")
        if list(envelope.get("payload_refs") or []) != public_payload_refs:
            prepare_mismatches.append("handoff_export_envelope.payload_refs")
        if list(envelope.get("payload_hashes") or []) != canonical_payload_hashes:
            prepare_mismatches.append("handoff_export_envelope.payload_hashes")
        if envelope.get("handoff_target") != "mixed_source_review_package":
            prepare_mismatches.append("handoff_export_envelope.handoff_target")
        if envelope.get("export_mode") != "reference_envelope_only":
            prepare_mismatches.append("handoff_export_envelope.export_mode")
    if prepare_mismatches:
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_prepare_mismatch",
            "Stored mixed-source handoff/export prepare authority does not match the APS handoff request.",
            status="conflict",
            http_status=409,
    ...
    dispatch_downstream_unavailable = [
        "external_export_download",
        "external_export",
        "download",
        "connector_dispatch",
    ...
    negative_authority_flags = _json_clone(prepare_state.get("negative_authority_flags") or preview_body.get("negative_authority_flags") or {})
    dispatch_basis = {
        "schema_id": "layer3.mixed_source_aps_handoff_dispatch_authority.v1",
        "aps_handoff_dispatch_schema_id": MIXED_SOURCE_APS_HANDOFF_DISPATCH_SCHEMA_ID,
        "package_family": PACKAGE_FAMILY_MIXED_DATASET_DOCUMENT,
        "client_request_id": request_id,
        "session_id": session_id,
        "material_preview_id": material_preview_id,
        ...
        "operator_decision": MIXED_SOURCE_APS_HANDOFF_DISPATCH_OPERATOR_DECISION,
        "decision_notes": decision_notes or None,
    }
    aps_handoff_record_ref = _stable_id("l3-mixed-source-aps-handoff-dispatch", dispatch_basis)
    if existing_dispatch is not None:
        ...
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_already_recorded",
            "This prepared mixed-source package set already has an APS handoff dispatch decision.",
            status="conflict",
            http_status=409,
            blocked_fields=["client_request_id", "operator_decision"],
        )
    if _aps_handoff_package_for_session(db, session_id=session_id) is not None:
        raise Layer3WorkbenchError(
            "mixed_source_aps_handoff_dispatch_existing_aps_package_not_admitted",
            "Mixed-source APS handoff dispatch may not reuse or create selected-pass APS evidence-bundle package rows.",
            status="conflict",
            http_status=409,
            blocked_fields=["session_id"],
        )

    aps_bundle_id = _stable_id(
        "l3-mixed-source-aps-evidence-bundle",
        {**dispatch_basis, "schema_id": "layer3.mixed_source_aps_evidence_bundle_reference_authority.v1"},
    )
    aps_output_package_id = _stable_id(
        "l3-mixed-source-aps-evidence-bundle-ref",
        {**dispatch_basis, "aps_bundle_id": aps_bundle_id},
    )
    aps_bundle_ref = f"layer3://mixed-source-aps-handoff/{aps_bundle_id}"
    recorded_at = _utcnow_iso()
    dispatch_state = {
        ...
        "aps_output_package_id": aps_output_package_id,
        "aps_output_package_kind": MIXED_SOURCE_APS_EVIDENCE_BUNDLE_PACKAGE_KIND,
        "aps_bundle_ref": aps_bundle_ref,
        "aps_bundle_id": aps_bundle_id,
        "aps_schema_id": MIXED_SOURCE_APS_EVIDENCE_BUNDLE_SCHEMA_ID,
        "source_package_refs": public_package_ref_map,
        ...
        "pass_type": None,
        "pass_scope": None,
        "method": "mixed_source_package_lifecycle",
        "source_gate": SOURCE_WORKBENCH_MIXED_PACKAGE_CONSTRUCTION_FREEZE,
        "package_construction_source_gate": SOURCE_WORKBENCH_MIXED_PACKAGE_CONSTRUCTION_FREEZE,
        "source_shape": "dataset_version_plus_aps_content_document",
        "source_dataset_version_ids": [],
        ...
        "provider_public_url_enabled": False,
        "external_export_download_enabled": False,
        "downstream_unavailable": dispatch_downstream_unavailable,
    }
    reconciliation.summary_json = {
        **reconciliation_summary,
...
def _mixed_source_external_export_download_readiness(db: Session, payload: dict[str, Any]) -> dict[str, Any]:
    request_id = str(payload.get("client_request_id") or "").strip()
    session_id = str(payload.get("session_id") or "").strip()
    material_preview_id = str(payload.get("material_preview_id") or "").strip()
    material_preview_hash = str(payload.get("material_preview_hash") or "").strip()
    ...
    blocked_payload_fields = mixed_source_external_export_download_readiness_blocked_fields(payload)
    if blocked_payload_fields:
        blocked_text = ", ".join(blocked_payload_fields)
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_scope_not_admitted",
            f"Mixed-source external export/download readiness request includes non-admitted fields: {blocked_text}.",
            status="invalid",
            blocked_fields=blocked_payload_fields,
            next_allowed_actions=["submit_bounded_mixed_source_external_export_download_readiness_request"],
        )
    ...
    if missing:
        raise Layer3WorkbenchError(
            "missing_mixed_source_external_export_download_readiness_fields",
            f"Mixed-source external export/download readiness request is missing required fields: {', '.join(missing)}.",
            status="invalid",
            blocked_fields=missing,
            next_allowed_actions=["submit_complete_mixed_source_external_export_download_readiness_request"],
        )
    if supplied_package_review_state != PACKAGE_REVIEW_APPROVED_STATE:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_requires_approved_package_review",
            "Mixed-source external export/download readiness requires package_review_state to be package_review_approved.",
            status="blocked",
            http_status=409,
            blocked_fields=["package_review_state"],
            next_allowed_actions=["inspect_package_review_submit_state"],
    ...
    if supplied_handoff_export_state != HANDOFF_EXPORT_PREPARED_STATE:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_requires_prepared_handoff_export",
            "Mixed-source external export/download readiness requires handoff_export_state to be handoff_export_prepared.",
            status="blocked",
            http_status=409,
            blocked_fields=["handoff_export_state"],
            next_allowed_actions=["inspect_handoff_export_prepare_state"],
    ...
    if supplied_aps_handoff_state != APS_HANDOFF_DISPATCHED_STATE:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_requires_dispatched_aps_handoff",
            "Mixed-source external export/download readiness requires aps_handoff_state to be aps_handoff_dispatched.",
            status="blocked",
            http_status=409,
    ...
    if handoff_target != "mixed_source_review_package":
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_target_not_admitted",
            "handoff_target must be mixed_source_review_package for mixed-source external export/download readiness.",
            status="invalid",
            blocked_fields=["handoff_target"],
        )
    if export_mode != "reference_envelope_only":
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_export_mode_not_admitted",
            "export_mode must be reference_envelope_only for mixed-source external export/download readiness.",
            status="invalid",
            blocked_fields=["export_mode"],
        )
    if aps_handoff_target != MIXED_SOURCE_APS_HANDOFF_TARGET:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_target_family_not_admitted",
            "aps_handoff_target must be mixed_source_aps_evidence_bundle.",
            status="invalid",
            blocked_fields=["aps_handoff_target"],
        )
    if dispatch_mode != MIXED_SOURCE_APS_HANDOFF_DISPATCH_MODE:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_dispatch_mode_not_admitted",
            "dispatch_mode must be server_side_mixed_source_aps_handoff.",
            status="invalid",
            blocked_fields=["dispatch_mode"],
        )
    if operator_decision != MIXED_SOURCE_EXTERNAL_EXPORT_DOWNLOAD_READINESS_OPERATOR_DECISION:
        raise Layer3WorkbenchError(
            "unsupported_mixed_source_external_export_download_readiness_decision",
            "operator_decision must be record_mixed_source_external_export_download_readiness.",
            status="invalid",
            blocked_fields=["operator_decision"],
        )
    ...
    if expected_package_kinds != list(PACKAGE_REVIEW_PREVIEW_CANDIDATE_KINDS):
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_kinds_mismatch",
            "Mixed-source external export/download readiness admits exactly canonical_internal, user_facing, and review_facing package kinds.",
            status="conflict",
            http_status=409,
            blocked_fields=["expected_package_kinds"],
        )
    ...
    if preview_body.get("package_review_preview_hash") != supplied_package_preview_hash:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_preview_mismatch",
            "Mixed-source external export/download readiness must reference the current server-recomputed package-review preview hash.",
            status="conflict",
            http_status=409,
            blocked_fields=["package_review_preview_hash"],
            next_allowed_actions=["refresh_package_review_preview"],
    ...
    if preview_body.get("contract_hash") != supplied_contract_hash:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_contract_mismatch",
            "Mixed-source external export/download readiness must reference the current server-recomputed contract hash.",
            status="conflict",
            http_status=409,
            blocked_fields=["contract_hash"],
            next_allowed_actions=["refresh_package_review_preview"],
    ...
    if reconciliation is None:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_requires_package_construction",
            "Mixed-source external export/download readiness requires a P15 mixed-source package construction reconciliation.",
            status="blocked",
            http_status=409,
            blocked_fields=["reconciliation_record_id"],
            next_allowed_actions=["package_construction_commit"],
    ...
    if reconciliation_summary.get("source_gate") != SOURCE_WORKBENCH_MIXED_PACKAGE_CONSTRUCTION_FREEZE:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_construction_source_gate_mismatch",
            "Mixed-source external export/download readiness requires P15 mixed-source package construction authority.",
            status="conflict",
            http_status=409,
            blocked_fields=["reconciliation_record_id"],
        )
    ...
    if not isinstance(commit_summary, dict):
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_non_workbench_package_state",
            "Mixed-source external export/download readiness requires workbench package-construction commit provenance.",
            status="blocked",
            http_status=409,
            next_allowed_actions=["inspect_existing_package_state"],
        )
    ...
    if commit_mismatches:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_construction_mismatch",
            "Stored mixed-source package construction provenance does not match the supplied external export/download readiness authority.",
            status="conflict",
            http_status=409,
            blocked_fields=commit_mismatches,
        )
    ...
    if supplied_construction_basis_hash != expected_construction_basis_hash:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_construction_basis_mismatch",
            "Supplied construction_basis_hash does not match the persisted mixed-source package construction.",
            status="conflict",
            http_status=409,
            blocked_fields=["construction_basis_hash"],
    ...
    if unexpected_package_kinds:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_unexpected_package_state",
            "Mixed-source external export/download readiness cannot proceed with unexpected package kinds on the reconciliation.",
            status="blocked",
            http_status=409,
            blocked_fields=["package_kinds"],
            next_allowed_actions=["inspect_existing_package_state"],
    ...
    if (
        ...
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_requires_complete_package_set",
            "Mixed-source external export/download readiness requires exactly canonical_internal, user_facing, and review_facing packages.",
            status="blocked",
            http_status=409,
            blocked_fields=["output_package_ids"],
            next_allowed_actions=["inspect_existing_package_state"],
    ...
    if not isinstance(raw_output_package_ids, list):
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_package_ids_invalid",
            "output_package_ids must be a list of the three constructed output package ids.",
            status="invalid",
            blocked_fields=["output_package_ids"],
        )
    ...
    if [str(item or "").strip() for item in raw_output_package_ids] != expected_package_ids:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_package_ids_mismatch",
            "Supplied output_package_ids do not match the constructed mixed-source package set.",
            status="conflict",
            http_status=409,
            blocked_fields=["output_package_ids"],
    ...
    if not isinstance(raw_payload_hashes, (list, dict)):
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_payload_hashes_invalid",
            "payload_hashes must be either a list of package hashes or a mapping keyed by package kind or package id.",
            status="invalid",
            blocked_fields=["payload_hashes"],
        )
    ...
    if canonical_payload_hashes is None:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_payload_hashes_mismatch",
            "Supplied payload_hashes do not match the constructed mixed-source package payload hashes.",
            status="conflict",
            http_status=409,
            blocked_fields=["payload_hashes"],
    ...
    if package_review_submit is None:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_requires_package_review_submit",
            "Mixed-source external export/download readiness requires an existing P16 package-review submit state.",
            status="blocked",
            http_status=409,
            blocked_fields=["package_review_submit_record_ref"],
            next_allowed_actions=["submit_mixed_source_package_review"],
    ...
    if submit_mismatches:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_package_review_submit_mismatch",
            "Stored mixed-source package-review submit authority does not match the external export/download readiness request.",
            status="conflict",
            http_status=409,
            blocked_fields=sorted(set(submit_mismatches)),
        )
    ...
    if prepare_state is None or prepare_state.get("handoff_export_state") != HANDOFF_EXPORT_PREPARED_STATE:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_requires_prepared_handoff_export",
            "Mixed-source external export/download readiness requires recorded handoff_export_prepared state.",
            status="blocked",
            http_status=409,
            blocked_fields=["handoff_export_state"],
            next_allowed_actions=["record_mixed_source_handoff_export_prepare"],
    ...
    if prepare_mismatches:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_prepare_mismatch",
            "Stored mixed-source handoff/export prepare authority does not match the external export/download readiness request.",
            status="conflict",
            http_status=409,
            blocked_fields=sorted(set(prepare_mismatches)),
        )

    # P18 dispatch precondition check
    recorded_dispatch = _aps_handoff_dispatch_from_reconciliation(reconciliation)
    if recorded_dispatch is None:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_requires_aps_handoff_dispatch",
            "Mixed-source external export/download readiness requires a recorded P18 APS handoff dispatch state.",
            status="blocked",
            http_status=409,
            blocked_fields=["aps_handoff_record_ref"],
            next_allowed_actions=["record_mixed_source_aps_handoff_dispatch"],
        )
    if recorded_dispatch.get("aps_handoff_state") != APS_HANDOFF_DISPATCHED_STATE:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_requires_dispatched_aps_handoff",
            "Mixed-source external export/download readiness requires aps_handoff_state to be aps_handoff_dispatched on the recorded dispatch.",
            status="blocked",
            http_status=409,
            blocked_fields=["aps_handoff_state"],
            next_allowed_actions=["inspect_aps_handoff_dispatch_state"],
        )
    if recorded_dispatch.get("aps_handoff_record_ref") != supplied_aps_handoff_record_ref:
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_aps_handoff_mismatch",
            "Supplied aps_handoff_record_ref does not match the recorded P18 APS handoff dispatch.",
            status="conflict",
            http_status=409,
            blocked_fields=["aps_handoff_record_ref"],
        )
    ...
    negative_authority_flags = {
        **_json_clone(prepare_state.get("negative_authority_flags") or preview_body.get("negative_authority_flags") or {}),
        "external_export_enabled": False,
        "download_enabled": False,
    ...
    readiness_downstream_unavailable = [
        "external_export_download",
        "external_export",
        "download",
        "download_url",
    ...
    readiness_basis = {
        "schema_id": "layer3.mixed_source_external_export_download_readiness_authority.v1",
        "external_export_download_readiness_schema_id": MIXED_SOURCE_EXTERNAL_EXPORT_DOWNLOAD_READINESS_SCHEMA_ID,
        "readiness_schema_id": MIXED_SOURCE_EXTERNAL_EXPORT_DOWNLOAD_READINESS_SCHEMA_ID,
        "package_family": PACKAGE_FAMILY_MIXED_DATASET_DOCUMENT,
        "client_request_id": request_id,
        "session_id": session_id,
        "material_preview_id": material_preview_id,
    ...
    external_export_download_readiness_record_ref = _stable_id(
        "l3-mixed-source-external-export-download-readiness", readiness_basis
    )

    existing_readiness = _mixed_source_external_export_download_readiness_from_reconciliation(reconciliation)
    if existing_readiness is not None:
        if (
            existing_readiness.get("external_export_download_readiness_record_ref") == external_export_download_readiness_record_ref
            and existing_readiness.get("client_request_id") == request_id
        ):
            readiness_id = str(existing_readiness.get("readiness_id") or "")
            external_export_download_readiness_ref = f"layer3://mixed-source-external-export/{readiness_id}"
            return {
                **_base_response(
                    MIXED_SOURCE_EXTERNAL_EXPORT_DOWNLOAD_READINESS_SCHEMA_ID,
                    request_id=request_id,
                ...
                "readiness_schema_id": MIXED_SOURCE_EXTERNAL_EXPORT_DOWNLOAD_READINESS_SCHEMA_ID,
                "external_export_download_readiness_schema_id": MIXED_SOURCE_EXTERNAL_EXPORT_DOWNLOAD_READINESS_SCHEMA_ID,
                "external_export_download_readiness_state": existing_readiness.get("external_export_download_readiness_state"),
                "external_export_download_readiness_record_ref": external_export_download_readiness_record_ref,
                "external_export_download_readiness_ref": external_export_download_readiness_ref,
                "package_family": PACKAGE_FAMILY_MIXED_DATASET_DOCUMENT,
                "aps_handoff_record_ref": supplied_aps_handoff_record_ref,
                "aps_handoff_state": APS_HANDOFF_DISPATCHED_STATE,
                "prepare_record_ref": supplied_prepare_ref,
                ...
                "delivery_enabled": False,
                "external_export_download_enabled": False,
                "downstream_unavailable": readiness_downstream_unavailable,
                "next_allowed_actions": [],
                "next_state": "mixed_source_external_export_download_ready",
                "authority_rail": _authority_rail(
                    session_id=session_id,
                    current_gate="package",
                    persistence_mode="durable_mixed_source_external_export_download_readiness",
                    downstream_unavailable=readiness_downstream_unavailable,
                    execution_enabled=False,
                    package_review_enabled=False,
                ),
        ...
        raise Layer3WorkbenchError(
            "mixed_source_external_export_download_readiness_already_recorded",
            "This prepared mixed-source package set already has an external export/download readiness decision.",
            status="conflict",
            http_status=409,
            blocked_fields=["client_request_id", "operator_decision"],
        )
    ...
    readiness_id = _stable_id(
        "l3-mixed-source-external-export-download-readiness-ref",
        {**readiness_basis, "external_export_download_readiness_record_ref": external_export_download_readiness_record_ref},
    )
    external_export_download_readiness_ref = f"layer3://mixed-source-external-export/{readiness_id}"
    recorded_at = _utcnow_iso()
    readiness_state = {
    ...
    reconciliation.summary_json = {
        **reconciliation_summary,
        "external_export_download_readiness": readiness_state,
    }
    session.summary_json = {
        **_json_clone(session.summary_json or {}),
        "external_export_download_readiness": {
            "schema_id": MIXED_SOURCE_EXTERNAL_EXPORT_DOWNLOAD_READINESS_STATE_SCHEMA_ID,
            "external_export_download_readiness_schema_id": MIXED_SOURCE_EXTERNAL_EXPORT_DOWNLOAD_READINESS_SCHEMA_ID,
            "readiness_schema_id": MIXED_SOURCE_EXTERNAL_EXPORT_DOWNLOAD_READINESS_SCHEMA_ID,
            "external_export_download_readiness_record_ref": external_export_download_readiness_record_ref,
...
def mixed_source_external_export_download_readiness(db: Session, payload: dict[str, Any]) -> dict[str, Any]:
    request_id = str(payload.get("client_request_id") or "").strip()
    if not request_id:
        raise Layer3WorkbenchError(
            "client_request_id_required",
            "client_request_id is required for mixed-source external export/download readiness.",
            status="invalid",
            blocked_fields=["client_request_id"],
            next_allowed_actions=["submit_idempotent_mixed_source_external_export_download_readiness_request"],
        )
...
def aps_handoff_dispatch(db: Session, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    mixed_material_authority_fields_present = any(
        field in payload
        for field in (
            "material_preview_id",
            "material_preview_hash",
    ...
    if mixed_material_authority_fields_present:
        return _mixed_source_aps_handoff_dispatch(db, payload)
    ...
    source_intake_dispatch = _source_intake_result_review_source_admitted(
        status_body=status_body,
        output_metadata_summary=output_metadata_summary,
    )
    if status_body.get("engine_family") == ENGINE_FAMILY_SOURCE_INTAKE_QUALITATIVE_PREVIEW and not source_intake_dispatch:
        raise Layer3WorkbenchError(
            "source_intake_aps_handoff_dispatch_not_admitted",
            "APS handoff dispatch is admitted only for exact source-intake result/status authority.",
            status="blocked",
            http_status=409,
            next_allowed_actions=["inspect_execution_result_status"],
        )
    ...
    if source_intake_dispatch:
        if supplied_analysis_run_id:
            raise Layer3WorkbenchError(
                "source_intake_aps_handoff_dispatch_analysis_run_not_admitted",
                "Source-intake APS handoff dispatch must not provide analysis_run_id.",
                status="invalid",
                blocked_fields=["analysis_run_id"],
            )
    elif status_body.get("pass_type") == PASS_TYPE_ASSOCIATED_COHORT:
    ...
    if source_intake_dispatch:
        source_intake_submit_mismatches = [
            field
            for field, expected in {
                "pass_type": PASS_TYPE_SINGLE_ITEM,
                "pass_scope": PASS_SCOPE_SOURCE_INTAKE_QUALITATIVE,
        ...
        submit_schema_id = str(
            package_review_submit.get("package_review_submit_schema_id")
            or package_review_submit.get("schema_id")
            or ""
        )
        if submit_schema_id != SOURCE_INTAKE_PACKAGE_REVIEW_SUBMIT_SCHEMA_ID:
            source_intake_submit_mismatches.append("package_review_submit_schema_id")
        if list(package_review_submit.get("source_dataset_version_ids") or []) != []:
            source_intake_submit_mismatches.append("source_dataset_version_ids")
        if list(package_review_submit.get("payload_refs") or []) != canonical_payload_refs:
            source_intake_submit_mismatches.append("payload_refs")
        if source_intake_submit_mismatches:
            raise Layer3WorkbenchError(
                "source_intake_aps_handoff_dispatch_not_admitted",
                "Source-intake APS handoff dispatch requires exact approved source-intake package-review submit authority.",
                status="blocked",
                http_status=409,
    ...
    if source_intake_prepare_mismatches:
        raise Layer3WorkbenchError(
            "source_intake_aps_handoff_dispatch_not_admitted",
            "Source-intake APS handoff dispatch requires exact prepared source-intake handoff/export authority.",
            status="blocked",
            http_status=409,
    ...
    if source_intake_dispatch:
        dispatch_state.update(
            {
                "source_intake_record_id": prepare_state.get("source_intake_record_id"),
                "candidate_id": prepare_state.get("candidate_id"),
    ...
    session.summary_json = {
        **_json_clone(session.summary_json or {}),
        "aps_handoff_dispatch": {
            ...
            "aps_bundle_ref": aps_package.payload_ref,
            "aps_bundle_id": str(aps_summary.get("bundle_id") or ""),
            "source_intake_record_id": prepare_state.get("source_intake_record_id") if source_intake_dispatch else None,
            "candidate_id": prepare_state.get("candidate_id") if source_intake_dispatch else None,
            "output_payload_ref": prepare_state.get("output_payload_ref") if source_intake_dispatch else None,
            "output_payload_hash": prepare_state.get("output_payload_hash") if source_intake_dispatch else None,
            "external_export_enabled": False,
            "download_enabled": False,
            "connector_dispatch_enabled": False,
            "downstream_unavailable": list(APS_HANDOFF_DISPATCH_DOWNSTREAM_UNAVAILABLE),
    ...
    *,
    ...
    if recorded_submit is not None:
        ...
        return {
            ...
            "source_intake_record_id": recorded_submit.get("source_intake_record_id"),
            "candidate_id": recorded_submit.get("candidate_id"),
            "output_payload_ref": recorded_submit.get("output_payload_ref"),
            "output_payload_hash": recorded_submit.get("output_payload_hash"),
            "package_review_submit_enabled": False,
            "handoff_enabled": False,
            "export_enabled": False,
            "downstream_unavailable": list(downstream_unavailable),
    ...
    return {
        ...
        or commit_summary.get("authority_basis_hash"),
        "package_construction_source_gate": reconciliation_summary.get("source_gate"),
        "analysis_run_id": commit_summary.get("analysis_run_id"),
        "result_review_record_ref": commit_summary.get("result_review_record_ref"),
        "pass_type": commit_summary.get("pass_type"),
        "pass_scope": commit_summary.get("pass_scope"),
        "method": commit_summary.get("method"),
        "source_gate": commit_summary.get("source_gate"),
        "source_shape": commit_summary.get("source_shape"),
        "source_dataset_version_ids": _json_clone(commit_summary.get("source_dataset_version_ids") or []),
        "source_intake_record_id": commit_summary.get("source_intake_record_id"),
        "candidate_id": commit_summary.get("candidate_id"),
        "output_payload_ref": commit_summary.get("output_payload_ref"),
        "output_payload_hash": commit_summary.get("output_payload_hash"),
        "package_review_submit_enabled": True,
        "handoff_enabled": False,
        "export_enabled": False,
...
def _handoff_export_prepare_summary(
    ...
    *,
    ...
    prepare_submit_schema_id = (
        SOURCE_INTAKE_PACKAGE_REVIEW_SUBMIT_SCHEMA_ID
        if package_construction_source_gate == SOURCE_INTAKE_PACKAGE_CONSTRUCTION_SOURCE_GATE
        else QUAL_APS_PACKAGE_REVIEW_SUBMIT_SCHEMA_ID
...
def _aps_handoff_dispatch_summary(
    ...
    *,
    ...
    source_intake_prepare_source = (
        handoff_export_prepare_state.get("pass_scope") == PASS_SCOPE_SOURCE_INTAKE_QUALITATIVE
    ...
    if source_intake_prepare_source and not _source_intake_aps_dispatch_prepare_state_admitted(
        ...
        return {
            ...
            "available": False,
            "state": APS_HANDOFF_UNAVAILABLE_STATE,
            "blocked_reason": "source_intake_aps_handoff_dispatch_not_admitted",
            "reconciliation_record_id": reconciliation_record_id,
            "prepare_record_ref": prepare_record_ref,
            "handoff_export_envelope_ref": envelope_ref,
            "aps_handoff_target": "aps_evidence_bundle",
...
def _connector_local_destination_receipt_history_item(
    row: L3ConnectorLocalDestinationReceipt,
) -> dict[str, Any]:
    return {
        "schema_id": CONNECTOR_LOCAL_DESTINATION_RECEIPT_LIFECYCLE_SCHEMA_ID,
        ...
        "audit_authority": "durable_connector_local_destination_receipt_row",
    }


def _connector_local_destination_receipt_summary_history_item(
    ...
    *,
    ...
    return {
        ...
        "connector_local_destination_receipt_state": summary.get("connector_local_destination_receipt_state"),
        "reconciliation_record_id": reconciliation_record_id,
        ...
        "destination_target": summary.get("destination_target"),
        ...
        "audit_authority": "connector_local_destination_receipt_reconciliation_summary",
    }


def _connector_local_destination_receipt_failure_projection(
    *,
    ...
    return [
        {
            ...
            "projected_error_code": blocked_reason or "connector_local_destination_receipt_not_ready",
            "operator_status": "blocked_until_existing_connector_dispatch_and_external_export_download_readiness",
        },
        {
            "case": "stale_authority",
            "projected_error_code": "external_export_download_delivery_authority_mismatch",
            "operator_status": "reject_first_write_when_readiness_no_longer_matches_connector_basis",
        },
        {
            "case": "wrong_session_or_pass",
            "projected_error_code": "connector_local_destination_receipt_pass_run_session_mismatch",
            "operator_status": "reject_payload_outside_current_session_pass_authority",
        },
        {
            "case": "wrong_artifact",
            "projected_error_code": "external_export_download_delivery_source_artifact_mismatch",
            "operator_status": "reject_source_artifact_ref_hash_or_size_drift",
        },
        {
            "case": "wrong_basis",
            "projected_error_code": "connector_local_destination_receipt_connector_dispatch_record_ref_mismatch",
            "operator_status": "reject_connector_or_external_readiness_basis_drift",
        },
        {
            "case": "same_key_same_payload_replay",
            "projected_error_code": None,
            "operator_status": "return_already_recorded_without_revalidating_stale_readiness",
        },
        {
            "case": "same_key_different_payload_conflict",
            "projected_error_code": "connector_local_destination_receipt_client_request_conflict",
        ...
        },
        {
            ...
            "projected_error_code": "connector_local_destination_receipt_already_recorded",
            "operator_status": "reject_second_receipt_for_same_authority_basis",
        },
        {
            "case": "current_status",
            "projected_error_code": blocked_reason,
            "operator_status": current_state or "connector_local_destination_receipt_unavailable",
        },
    ]


def _with_connector_local_destination_receipt_lifecycle(
    status: dict[str, Any],
    *,
    history_rows: list[L3ConnectorLocalDestinationReceipt],
    current_state: str | None,
    blocked_reason: str | None,
    recorded_summary: dict[str, Any] | None = None,
) -> dict[str, Any]:
    history = [_connector_local_destination_receipt_history_item(row) for row in history_rows]
    if not history and recorded_summary:
        history = [
            _connector_local_destination_receipt_summary_history_item(
                recorded_summary,
    ...
    lifecycle = {
        ...
        "audit_trail_authority": "durable_receipt_row_and_reconciliation_summary_projection",
        ...
        "failure_state_projection": _connector_local_destination_receipt_failure_projection(
...
def _connector_local_destination_receipt_summary(
    ...
    *,
    ...
    if not isinstance(external_export_download_state, dict):
        external_export_download_state = {}
    else:
        external_export_download_state = dict(external_export_download_state)
    ...
    if not isinstance(reconciliation_summary, dict):
        reconciliation_summary = {}
    recorded_external_export_download_state = reconciliation_summary.get("external_export_download_prepare")
    if (
        isinstance(recorded_external_export_download_state, dict)
        and recorded_external_export_download_state.get("schema_id") == EXTERNAL_EXPORT_DOWNLOAD_PREPARE_STATE_SCHEMA_ID
        and (
            external_export_download_state.get("external_export_download_state")
            != EXTERNAL_EXPORT_DOWNLOAD_PREPARED_STATE
            or not external_export_download_state.get("external_export_download_record_ref")
        )
    ):
        external_export_download_state = dict(recorded_external_export_download_state)
        reconciliation_record_id = str(
            external_export_download_state.get("reconciliation_record_id") or reconciliation_record_id
        ).strip()
    ...
    history_rows = (
        ...
        .filter(L3ConnectorLocalDestinationReceipt.session_id == session_id)
        .order_by(L3ConnectorLocalDestinationReceipt.created_at.desc())
        .all()
    )
    if receipt_row is not None:
        return _with_connector_local_destination_receipt_lifecycle(
            {
            ...
            current_state=receipt_row.receipt_state,
            blocked_reason=None,
        )
    if recorded_summary.get("schema_id") == CONNECTOR_LOCAL_DESTINATION_RECEIPT_STATE_SCHEMA_ID:
        return _with_connector_local_destination_receipt_lifecycle(
            ...
            history_rows=history_rows,
            current_state=recorded_summary.get("connector_local_destination_receipt_state"),
            blocked_reason=None,
    ...
    if prepared and connector_recorded:
        return _with_connector_local_destination_receipt_lifecycle(
            ...
            history_rows=history_rows,
            current_state=CONNECTOR_LOCAL_DESTINATION_RECEIPT_READY_STATE,
            blocked_reason=None,
        )
    blocked_reason = (
...
def _external_local_export_history_item(
    row: L3ExternalLocalExportReceipt,
    *,
    audit_events: list[dict[str, Any]],
) -> dict[str, Any]:
    return {
        "schema_id": EXTERNAL_LOCAL_EXPORT_LIFECYCLE_SCHEMA_ID,
        ...
        "source_outbox_artifact_ref": "artifact://external-local-export-source-outbox-redacted",
        ...
        "audit_authority": "durable_external_local_export_receipt_row",
    }


def _external_local_export_audit_item(row: L3ExternalLocalExportAuditEvent) -> dict[str, Any]:
    return {
        "schema_id": EXTERNAL_LOCAL_EXPORT_AUDIT_SCHEMA_ID,
        "external_local_export_audit_event_id": row.external_local_export_audit_event_id,
        "external_local_export_receipt_id": row.external_local_export_receipt_id,
        "event_type": row.event_type,
        "event_status": row.event_status,
        "request_id": row.request_id,
        "authority_basis_hash": row.authority_basis_hash,
        "reason_code": row.reason_code,
        "event_payload": row.event_payload_json,
        "created_at": row.created_at.isoformat() if row.created_at else None,
    }


def _external_local_export_failure_projection(
    *,
    current_state: str | None,
    blocked_reason: str | None,
) -> list[dict[str, Any]]:
    return [
        {
            "case": "not_ready",
            "operator_status": blocked_reason or "await_server_owned_local_outbox_write",
            "projected_error_code": "external_local_export_requires_outbox_write",
            "active": current_state == EXTERNAL_LOCAL_EXPORT_NOT_READY_STATE,
        },
        {
            "case": "stale_authority",
            "operator_status": "conflict",
            "projected_error_code": EXTERNAL_LOCAL_EXPORT_STALE_AUTHORITY_STATE,
            "active": current_state == EXTERNAL_LOCAL_EXPORT_STALE_AUTHORITY_STATE,
        },
        {
            "case": "wrong_session_or_pass",
            "operator_status": "conflict",
            "projected_error_code": "external_local_export_pass_run_mismatch",
            "active": False,
        },
        {
            "case": "wrong_receipt_chain",
            "operator_status": "conflict",
            "projected_error_code": "external_local_export_requires_receipt_chain",
            "active": False,
        },
        {
            "case": "missing_provider_private_handoff_when_applicable",
            "operator_status": "blocked",
            "projected_error_code": "external_local_export_requires_provider_private_handoff",
            "active": False,
        },
        {
            "case": "same_key_same_payload_replay",
            "operator_status": "already_recorded",
            "projected_error_code": None,
            "active": current_state == EXTERNAL_LOCAL_EXPORT_REPLAY_STATE,
        },
        {
            "case": "same_key_different_payload_conflict",
            "operator_status": "conflict",
            "projected_error_code": "external_local_export_client_request_conflict",
            "active": current_state == EXTERNAL_LOCAL_EXPORT_CONFLICT_STATE,
        },
        {
            "case": "same_basis_different_client_request_id",
            "operator_status": "already_recorded",
        ...
        {
            "case": "target_write_conflict",
            "operator_status": "conflict",
            "projected_error_code": "external_local_export_existing_output_conflict",
            "active": False,
        },
        {
            "case": "write_failed",
            "operator_status": "failed",
            "projected_error_code": EXTERNAL_LOCAL_EXPORT_FAILED_STATE,
            "active": current_state == EXTERNAL_LOCAL_EXPORT_FAILED_STATE,
        },
    ]


def _with_external_local_export_lifecycle(
    status: dict[str, Any],
    *,
    history_rows: list[L3ExternalLocalExportReceipt],
    audit_rows: list[L3ExternalLocalExportAuditEvent],
    current_state: str | None,
    blocked_reason: str | None,
) -> dict[str, Any]:
    audit_history_by_receipt: dict[str, list[dict[str, Any]]] = {}
    audit_history = [_external_local_export_audit_item(row) for row in audit_rows]
    for item in audit_history:
        receipt_id = str(item.get("external_local_export_receipt_id") or "")
        audit_history_by_receipt.setdefault(receipt_id, []).append(item)
    history = [
        _external_local_export_history_item(
            row,
            audit_events=audit_history_by_receipt.get(row.external_local_export_receipt_id, []),
        )
        for row in history_rows
    ]
    latest_receipt = history[0] if history else None
    ...
    lifecycle = {
        "schema_id": EXTERNAL_LOCAL_EXPORT_LIFECYCLE_SCHEMA_ID,
        "surface_mode": "read_only_external_local_export_status_history",
        "history_listing_authority": "durable_external_local_export_receipt_rows",
        "audit_trail_authority": "durable_external_local_export_audit_event_rows",
        "history_count": len(history),
        "external_local_export_history": history,
        "latest_external_local_export_receipt": latest_receipt,
        "audit_event_history_count": len(audit_history),
        "audit_event_history": audit_history,
        "latest_audit_event": latest_audit,
        "idempotency_policy": {
            "client_request_id_unique": True,
            "authority_basis_hash_unique": True,
            "external_artifact_ref_unique": True,
            "same_key_same_payload_replay": "already_recorded",
            "same_key_different_payload_conflict": "external_local_export_client_request_conflict",
            "same_basis_different_client_request_id": "return_existing_status",
            "duplicate_target_identical_output": "return_existing_status",
            "duplicate_target_conflicting_output": "external_local_export_existing_output_conflict",
        },
        "retry_policy": {
            "retry_fields_admitted": False,
            "rerun_fields_admitted": False,
        ...
        "failure_state_projection": _external_local_export_failure_projection(
            current_state=current_state,
            blocked_reason=blocked_reason,
        ),
    }
    return {
        **status,
        "lifecycle_status_surface": lifecycle,
        "external_local_export_history": history,
        "external_local_export_history_count": len(history),
        "latest_external_local_export_receipt": latest_receipt,
        "audit_event_history": audit_history,
        "audit_event_history_count": len(audit_history),
        "latest_audit_event": latest_audit,
...
def _external_local_export_summary(
    ...
    *,
    session_id: str,
    server_owned_local_outbox_write_state: dict[str, Any],
    local_outbox_provider_private_handoff_state: dict[str, Any],
) -> dict[str, Any]:
    if not isinstance(server_owned_local_outbox_write_state, dict):
        server_owned_local_outbox_write_state = {}
    if not isinstance(local_outbox_provider_private_handoff_state, dict):
        local_outbox_provider_private_handoff_state = {}
    write_receipt_id = str(
        server_owned_local_outbox_write_state.get("server_owned_local_outbox_write_receipt_id") or ""
    ).strip()
    export_query = db.query(L3ExternalLocalExportReceipt).filter(
        L3ExternalLocalExportReceipt.session_id == session_id
    )
    if write_receipt_id:
        export_query = export_query.filter(
            L3ExternalLocalExportReceipt.server_owned_local_outbox_write_receipt_id == write_receipt_id
        )
    export_row = export_query.order_by(L3ExternalLocalExportReceipt.created_at.desc()).first()
    history_rows = (
        db.query(L3ExternalLocalExportReceipt)
        .filter(L3ExternalLocalExportReceipt.session_id == session_id)
        .order_by(L3ExternalLocalExportReceipt.created_at.desc())
        .all()
    )
    receipt_ids = [row.external_local_export_receipt_id for row in history_rows]
    audit_rows = []
    if receipt_ids:
        audit_rows = (
            db.query(L3ExternalLocalExportAuditEvent)
            .filter(L3ExternalLocalExportAuditEvent.external_local_export_receipt_id.in_(receipt_ids))
            .order_by(L3ExternalLocalExportAuditEvent.created_at.desc())
            .all()
        )
    if export_row is not None:
        return _with_external_local_export_lifecycle(
            {
                "schema_id": EXTERNAL_LOCAL_EXPORT_STATUS_SCHEMA_ID,
                "available": False,
                "state": export_row.export_state,
                "blocked_reason": None,
                "session_id": export_row.session_id,
                "pass_run_id": export_row.pass_run_id,
                "reconciliation_record_id": export_row.reconciliation_record_id,
                "external_local_export_receipt_id": export_row.external_local_export_receipt_id,
                "external_local_export_state": export_row.export_state,
                "server_owned_local_outbox_write_receipt_id": export_row.server_owned_local_outbox_write_receipt_id,
                "server_owned_local_outbox_target_receipt_id": export_row.server_owned_local_outbox_target_receipt_id,
                "connector_local_destination_receipt_id": export_row.connector_local_destination_receipt_id,
                "provider_private_handoff_receipt_id": export_row.provider_private_handoff_receipt_id,
                "connector_dispatch_record_ref": export_row.connector_dispatch_record_ref,
                "external_export_download_record_ref": export_row.external_export_download_record_ref,
                "target_identity": export_row.target_identity,
                "target_class": export_row.target_class,
                "dispatch_mode": export_row.dispatch_mode,
                "operator_decision": EXTERNAL_LOCAL_EXPORT_OPERATOR_DECISION,
                "redacted_destination_label": export_row.redacted_destination_label,
                "external_artifact_ref": export_row.external_artifact_ref,
                "external_manifest_ref": export_row.external_manifest_ref,
                "external_artifact_hash": export_row.external_artifact_hash,
                "external_artifact_size_bytes": export_row.external_artifact_size_bytes,
                "external_manifest_hash": export_row.external_manifest_hash,
                "external_manifest_size_bytes": export_row.external_manifest_size_bytes,
                "source_outbox_artifact_ref": "artifact://external-local-export-source-outbox-redacted",
                "source_outbox_artifact_hash": export_row.source_outbox_artifact_hash,
                "source_outbox_artifact_size_bytes": export_row.source_outbox_artifact_size_bytes,
                "authority_basis_hash": export_row.authority_basis_hash,
                "idempotency_key": export_row.idempotency_key,
                ...
                "generic_downstream_dispatch_enabled": False,
                "status_surface_mode": "read_only_server_session_summary_projection",
                "response_authority": "durable_external_local_export_receipt_row",
                "downstream_unavailable": list(EXTERNAL_LOCAL_EXPORT_DOWNSTREAM_UNAVAILABLE),
            },
            history_rows=history_rows,
            audit_rows=audit_rows,
            current_state=export_row.export_state,
            blocked_reason=None,
        )
    write_recorded = (
        server_owned_local_outbox_write_state.get("state") == SERVER_OWNED_LOCAL_OUTBOX_WRITE_RECORDED_STATE
    ...
    provider_receipt_id = str(
        local_outbox_provider_private_handoff_state.get("provider_private_handoff_receipt_id") or ""
    ).strip()
    if write_recorded:
        return _with_external_local_export_lifecycle(
            {
                "schema_id": EXTERNAL_LOCAL_EXPORT_STATUS_SCHEMA_ID,
                "available": True,
                "state": EXTERNAL_LOCAL_EXPORT_READY_STATE,
                "blocked_reason": None,
                "reconciliation_record_id": server_owned_local_outbox_write_state.get("reconciliation_record_id"),
                "server_owned_local_outbox_write_receipt_id": write_receipt_id,
                "server_owned_local_outbox_target_receipt_id": server_owned_local_outbox_write_state.get(
                ...
                "provider_private_handoff_receipt_id": provider_receipt_id or None,
                "provider_private_handoff_state": local_outbox_provider_private_handoff_state.get("state"),
                "connector_dispatch_record_ref": server_owned_local_outbox_write_state.get(
                    "connector_dispatch_record_ref"
                ),
                "external_export_download_record_ref": server_owned_local_outbox_write_state.get(
                ...
                "source_outbox_artifact_ref": "artifact://external-local-export-source-outbox-redacted",
                "source_outbox_artifact_hash": server_owned_local_outbox_write_state.get("outbox_artifact_hash"),
                "source_outbox_artifact_size_bytes": server_owned_local_outbox_write_state.get(
                    "outbox_artifact_size_bytes"
                ),
                "server_configured_external_local_export_write_enabled": True,
                "server_configured_external_local_export_write_performed": False,
                "external_destination_write_enabled": True,
                "operator_destination_path_enabled": False,
                "real_connector_invocation_enabled": False,
                "connector_run_created": False,
            ...
            history_rows=history_rows,
            audit_rows=audit_rows,
            current_state=EXTERNAL_LOCAL_EXPORT_READY_STATE,
            blocked_reason=None,
        )
    return _with_external_local_export_lifecycle(
        {
            "schema_id": EXTERNAL_LOCAL_EXPORT_STATUS_SCHEMA_ID,
            "available": False,
            "state": EXTERNAL_LOCAL_EXPORT_NOT_READY_STATE,
            "blocked_reason": "write_server_owned_local_outbox",
            "target_identity": EXTERNAL_LOCAL_EXPORT_TARGET_IDENTITY,
            "target_class": EXTERNAL_LOCAL_EXPORT_TARGET_CLASS,
            "dispatch_mode": EXTERNAL_LOCAL_EXPORT_DISPATCH_MODE,
            "operator_decision": EXTERNAL_LOCAL_EXPORT_OPERATOR_DECISION,
            "server_configured_external_local_export_write_enabled": False,
            "server_configured_external_local_export_write_performed": False,
            "external_destination_write_enabled": True,
            "operator_destination_path_enabled": False,
            "real_connector_invocation_enabled": False,
            ...
            "generic_downstream_dispatch_enabled": False,
            "status_surface_mode": "read_only_server_session_summary_projection",
            "response_authority": "existing_session_summary_without_external_local_export_receipt",
            "downstream_unavailable": list(EXTERNAL_LOCAL_EXPORT_DOWNSTREAM_UNAVAILABLE),
        },
        history_rows=history_rows,
        audit_rows=audit_rows,
        current_state=EXTERNAL_LOCAL_EXPORT_NOT_READY_STATE,
        blocked_reason="write_server_owned_local_outbox",
    )
...
def session_summary(db: Session, session_id: str) -> dict[str, Any]:
    ...
    external_local_export_state = _external_local_export_summary(
        db,
        session_id=session_id,
        server_owned_local_outbox_write_state=server_owned_local_outbox_write_state,
