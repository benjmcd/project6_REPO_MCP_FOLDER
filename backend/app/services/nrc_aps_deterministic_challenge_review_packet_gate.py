...
def _owner_run_id_from_artifact_payload(payload: dict[str, Any]) -> str | None:
    source_challenge = dict(payload.get("source_deterministic_challenge_artifact") or {})
...
def _artifact_scope_for_run_id(run_id: str) -> str:
    return contract.safe_path_token(f"run_{review_packet._safe_scope_token(run_id)}")
...
def validate_deterministic_challenge_review_packet_gate(*, run_ids: list[str] | None = None, limit: int = 50, report_path: str | Path = DEFAULT_REPORT_PATH, require_runs: bool = True) -> dict[str, Any]:
    ...
    for row in run_rows:
        ...
        for artifact_path in sorted(
            Path(settings.connector_reports_dir).glob(f"{scope}_*_aps_deterministic_challenge_review_packet_v1.json")
        ):
        ...
        for failure_path in sorted(
            Path(settings.connector_reports_dir).glob(f"{scope}_*_aps_deterministic_challenge_review_packet_failure_v1.json")
        ):
        ...
        checks.append(
            {
                "run_id": run_id,
                "deterministic_challenge_review_packet_refs": [str(path) for path, _payload in matched_artifacts],
