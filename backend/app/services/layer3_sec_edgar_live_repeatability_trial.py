...
SCHEMA_ID = "layer3.sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial.v1"
REQUEST_SCHEMA_ID = (
    "layer3.sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_request.v1"
)
SCHEMA_VERSION = 1
TRIAL_MODE = "append_only_trial_receipt_over_original_and_repeat_live_downstream_status_authority_without_sec_fetch_or_processing_execution"
OPERATOR_DECISION = "record_sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial"
TRIAL_RECEIPT_PREFIX = "sec-edgar-text-table-live-source-artifact-downstream-operator-repeatability-trial"
TRIAL_RECEIPT_DIR = "layer3-sec-edgar-live-repeatability-trial"
TRIAL_ACCEPTED_STATE = (
    "sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_accepted"
)
TRIAL_BLOCKED_STATE = (
    "sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_blocked"
)
ACCEPTED_DISPOSITIONS = {"no_regression_observed", "delta_reviewed_no_regression"}
BLOCKED_DISPOSITION = "regression_detected_blocked"
REDACTION_POLICY_ID = (
    "sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_redaction_v1"
)
...
def record_sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial(
    ...
    if disposition not in ACCEPTED_DISPOSITIONS and disposition != BLOCKED_DISPOSITION:
        _blocked(
            "sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_disposition_not_admitted",
            "The SEC EDGAR live downstream repeatability trial disposition is not admitted.",
            blocked_fields=["operator_repeatability_disposition"],
        )
    if request.get("operator_confirmation") is not True:
        _blocked(
            "sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_operator_confirmation_missing",
            "operator_confirmation=true is required before recording a SEC EDGAR live downstream repeatability trial.",
            http_status=409,
    ...
    if authority_mismatches:
        _blocked(
            ...
            "The original and repeat SEC EDGAR live downstream status projections are not bound to the same authority chain.",
            http_status=409,
    ...
    if proof_hash_comparison != "match" or coverage_step_set_comparison != "match":
        _blocked(
            "sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_proof_or_coverage_mismatch",
            "The live repeatability trial requires matching live downstream proof hash and coverage step set.",
            http_status=409,
            blocked_fields=["proof_hash", "coverage"],
        )
    if disposition == "no_regression_observed" and operator_status_hash_comparison != "match":
        _blocked(
            "sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_disposition_contradicts_delta",
    ...
    response = {
        ...
        "coverage_step_set_comparison": coverage_step_set_comparison,
        "trial_authority": {
            "source": "server_revalidated_sec_edgar_live_source_artifact_downstream_operator_status_projections",
            "original_status_available": original_status.get("status") == "available",
            "repeat_status_available": repeat_status.get("status") == "available",
            "live_source_artifact_authority_bound": bool(original_status.get("live_source_artifact_receipt_hash")),
            "source_acquisition_authority_bound": bool(original_status.get("source_acquisition_receipt_hash")),
            "live_material_bridge_authority_bound": bool(
                original_status.get("live_source_artifact_material_bridge_receipt_hash")
            ),
            "browser_supplied_local_authority_rejected": True,
            "browser_supplied_raw_url_rejected": True,
            "browser_supplied_sec_url_rejected": True,
        ...
        "fail_closed_behavior": {
            ...
            "mismatched_live_source_artifact_receipt_blocks_trial": True,
            "mismatched_source_acquisition_receipt_blocks_trial": True,
            "mismatched_live_material_bridge_blocks_trial": True,
        ...
        "next_allowed_actions": [
            "inspect the recorded SEC EDGAR live downstream repeatability trial receipt",
    ...
    if _contains_forbidden_output_ref(response):
        _blocked(
            "sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_raw_authority_exposed",
            "SEC EDGAR live downstream repeatability trial would expose raw path, URL, token, or artifact-byte authority.",
            http_status=409,
        )
    return response
...
def _load_operator_status(
    *,
    ...
    if not isinstance(status_request, Mapping):
        _blocked(
            f"sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_{prefix}_status_request_missing",
            "The repeatability trial requires a structured SEC EDGAR live downstream operator-status request.",
            http_status=409,
    ...
    try:
        ...
        _blocked(
            f"sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_{prefix}_status_invalid",
            "The repeatability trial could not revalidate the requested SEC EDGAR live downstream operator status.",
            http_status=exc.http_status,
            blocked_fields=list(exc.blocked_fields),
        )
    if status.get("status") != "available" or status.get("operator_status_state") != "available":
        _blocked(
            f"sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_{prefix}_status_not_available",
            "The repeatability trial requires available original and repeat live downstream operator-status projections.",
            http_status=409,
            blocked_fields=[f"{prefix}_operator_status_request"],
        )
    if status.get("proof_available") is not True or not status.get("proof_hash"):
        _blocked(
            f"sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_{prefix}_proof_missing",
            "The repeatability trial requires live downstream proof to be available for both status projections.",
            http_status=409,
            blocked_fields=[f"{prefix}_operator_status_request"],
        )
    if str(status.get("operator_status_hash") or "") != expected_status_hash:
        _blocked(
            f"sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_stale_{prefix}_operator_status_hash",
            "The supplied SEC EDGAR live downstream operator-status hash is stale or contradictory.",
            http_status=409,
    ...
    *,
    ...
    if target.exists():
        ...
        if existing.get("trial_receipt_hash") != receipt_hash:
            _blocked(
                "sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_receipt_conflict",
                "A live repeatability trial already exists for this original/repeat authority pair.",
                http_status=409,
            )
    ...
    try:
        ...
        _blocked(
            "sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_receipt_write_failed",
            "SEC EDGAR live downstream repeatability trial receipt could not be recorded.",
            http_status=409,
...
def _read_receipt(path: Path) -> dict[str, Any]:
    try:
        ...
        _blocked(
            "sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_receipt_unreadable",
            "SEC EDGAR live downstream repeatability trial receipt could not be read.",
            http_status=409,
            blocked_fields=[exc.__class__.__name__],
        )
    if not isinstance(receipt, dict):
        _blocked(
            "sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_receipt_invalid",
            "SEC EDGAR live downstream repeatability trial receipts must be JSON objects.",
            http_status=409,
        )
    return receipt
...
def _normalise_request(fields: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if blocked or nested_blocked:
        _blocked(
            ...
            "SEC EDGAR live downstream repeatability trial does not admit caller paths, URLs, bytes, credentials, commands, process state, connector, model, browser, source-expansion, or frontend authority.",
            blocked_fields=[*blocked, *nested_blocked],
        )
    unknown = sorted(set(request) - ALLOWED_FIELDS)
    if unknown:
        _blocked(
            "sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_unknown_field",
            "SEC EDGAR live downstream repeatability trial fields are intentionally scoped.",
            blocked_fields=unknown,
        )
    schema_id = str(request.get("schema_id") or REQUEST_SCHEMA_ID).strip()
    if schema_id != REQUEST_SCHEMA_ID:
        _blocked(
            "sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_schema_not_admitted",
            "SEC EDGAR live downstream repeatability trial requires the admitted request schema.",
...
def _required(fields: Mapping[str, Any], key: str) -> str:
    ...
    if not value:
        _blocked(
            "sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_required_field_missing",
            "A required SEC EDGAR live downstream repeatability trial field is missing or empty.",
            blocked_fields=[key],
        )
    return value
...
def _required_hash(fields: Mapping[str, Any], key: str) -> str:
    ...
    if not _is_sha256(value):
        _blocked(
            "sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_hash_invalid",
            "SEC EDGAR live downstream repeatability trial hash fields must be SHA-256 hex strings.",
            blocked_fields=[key],
        )
    return value
...
def _require_exact(fields: Mapping[str, Any], key: str, expected: str) -> None:
    if _required(fields, key) != expected:
        _blocked(
            f"sec_edgar_text_table_live_source_artifact_downstream_operator_repeatability_trial_{key}_not_admitted",
            "SEC EDGAR live downstream repeatability trial request does not match the admitted runtime contract.",
            blocked_fields=[key],
        )
...
def _negative_invariants() -> dict[str, bool]:
    return {
        ...
        "gate_b_mutation_admitted_in_trial": False,
