...
SCHEMA_ID = "layer3.sec_edgar_text_table_live_source_artifact_downstream_operator_status.v1"
REQUEST_SCHEMA_ID = "layer3.sec_edgar_text_table_live_source_artifact_downstream_operator_status_request.v1"
SCHEMA_VERSION = 1
...
STATUS_PROJECTION_PREFIX = "sec-edgar-text-table-live-source-artifact-downstream-operator-status"
...
def inspect_sec_edgar_text_table_live_source_artifact_downstream_operator_status(
    ...
    proof_request = request.get("live_downstream_proof_request")
    ...
    if proof_request is None:
        if expected_proof_hash:
            return _blocked_status(
                ...
                code="sec_edgar_text_table_live_source_artifact_downstream_operator_status_ambiguous_proof_authority",
                message="Expected proof hash was supplied without live downstream proof authority to revalidate.",
                expected_proof_hash=expected_proof_hash,
                blocked_fields=["live_downstream_proof_request", "expected_proof_hash"],
            )
    ...
    if not isinstance(proof_request, Mapping):
        return _blocked_status(
            request_id=request_id,
            code="sec_edgar_text_table_live_source_artifact_downstream_operator_status_proof_request_invalid",
            message="Live downstream proof authority must be supplied as a structured proof request.",
            blocked_fields=["live_downstream_proof_request"],
        )
    if not expected_proof_hash:
        return _blocked_status(
            request_id=request_id,
            code="sec_edgar_text_table_live_source_artifact_downstream_operator_status_expected_proof_hash_missing",
            message="Expected proof hash is required when live downstream proof authority is supplied.",
            blocked_fields=["expected_proof_hash"],
        )
    if not _is_sha256(expected_proof_hash):
        return _blocked_status(
            request_id=request_id,
            code="sec_edgar_text_table_live_source_artifact_downstream_operator_status_expected_proof_hash_invalid",
    ...
    if str(proof.get("proof_hash") or "") != expected_proof_hash:
        return _blocked_status(
            request_id=request_id,
            code="sec_edgar_text_table_live_source_artifact_downstream_operator_status_proof_hash_mismatch",
            message="Revalidated live downstream proof hash does not match the expected proof hash.",
            expected_proof_hash=expected_proof_hash,
            proof=proof,
            blocked_fields=["expected_proof_hash", "live_downstream_proof_request"],
            next_allowed_actions=["refresh_sec_edgar_text_table_live_source_artifact_downstream_proof"],
        )
    ...
    if _contains_forbidden_output_ref(response):
        return _blocked_status(
            request_id=request_id,
            code="sec_edgar_text_table_live_source_artifact_downstream_operator_status_raw_authority_exposed",
            message="SEC EDGAR live downstream status projection would expose raw path, URL, token, or artifact-byte authority.",
            expected_proof_hash=expected_proof_hash,
            proof=proof,
            blocked_fields=["live_downstream_proof_request"],
        )
...
def _not_recorded_status(*, request_id: str) -> dict[str, Any]:
    ...
    return _status_response(
        ...
        blocked_reasons=[],
        proof_summary={},
        next_allowed_actions=["record_sec_edgar_text_table_live_source_artifact_downstream_proof_before_status_available"],
    )
...
def _available_status(*, request_id: str, expected_proof_hash: str, proof: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return _status_response(
        ...
        next_allowed_actions=[
            "use_sec_edgar_text_table_live_source_artifact_downstream_status_as_operator_visible_e2e_evidence",
            "select_sec_edgar_text_table_live_source_artifact_downstream_rendered_status_projection",
        ],
    )
...
def _blocked_status(
    *,
    ...
    return _status_response(
        ...
        or ["repair_or_refresh_sec_edgar_text_table_live_source_artifact_downstream_proof_authority"],
    )
    ...
    return {
        **status_input,
        ...
        "status_projection": {
            ...
            "live_source_artifact_authority_bound": bool(status_input.get("live_source_artifact_receipt_hash")),
            "live_material_bridge_authority_bound": bool(
                status_input.get("live_source_artifact_material_bridge_receipt_hash")
            ),
        },
        "blocked_reasons": blocked_reasons,
...
def _status_input(
    *,
    ...
    return {
        ...
        "authority_envelope_hash": str(proof.get("authority_envelope_hash") or ""),
        ...
        "source_acquisition_receipt_hash": str(proof.get("source_acquisition_receipt_hash") or ""),
        "live_source_artifact_material_bridge_receipt_hash": str(
            proof.get("live_source_artifact_material_bridge_receipt_hash") or ""
        ),
        "material_bridge_receipt_hash": str(proof.get("material_bridge_receipt_hash") or ""),
        ...
        "downstream_proof_hash": str(proof.get("downstream_proof_hash") or ""),
...
def _normalise_request(fields: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if unknown:
        _blocked(
            "sec_edgar_text_table_live_source_artifact_downstream_operator_status_unknown_field",
            "SEC EDGAR live downstream status fields are intentionally scoped.",
            blocked_fields=unknown,
        )
    schema_id = str(request.get("schema_id") or REQUEST_SCHEMA_ID).strip()
    if schema_id != REQUEST_SCHEMA_ID:
        _blocked(
            "sec_edgar_text_table_live_source_artifact_downstream_operator_status_schema_not_admitted",
            "SEC EDGAR live downstream status requires the admitted request schema.",
...
def _required(fields: Mapping[str, Any], key: str) -> str:
    ...
    if not value:
        _blocked(
            "sec_edgar_text_table_live_source_artifact_downstream_operator_status_required_field_missing",
            "A required SEC EDGAR live downstream status field is missing or empty.",
            blocked_fields=[key],
        )
    return value
...
def _require_exact(fields: Mapping[str, Any], key: str, expected: str) -> None:
    if _required(fields, key) != expected:
        _blocked(
            f"sec_edgar_text_table_live_source_artifact_downstream_operator_status_{key}_not_admitted",
            "SEC EDGAR live downstream status request does not match the admitted runtime contract.",
            blocked_fields=[key],
        )
