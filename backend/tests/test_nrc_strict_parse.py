...
@pytest.mark.skipif(
    not _GUARDED_CHILD,
    reason="network guard self-probe runs only inside the guarded child",
)
...
@pytest.mark.skipif(
    _GUARDED_CHILD,
    reason="the guarded child must not recursively launch itself",
)
def test_full_strict_subset_passes_in_fresh_guarded_child() -> None:
    test_file = Path(__file__).resolve()
    backend = test_file.parents[1]
    ...
    assert skipped is not None, completed.stdout
    assert (int(passed.group(1)), int(skipped.group(1))) == (29, 1)
