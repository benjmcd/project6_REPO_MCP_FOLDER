"""AST-level drift guard for router.py operator identity gating.

ALL routes (any HTTP method) in router.py must:
  ...
  3. Pass the ``access=`` keyword argument with value "read" or "write".

No exemptions — every route in this module is a sensitive surface.
"""
...
SOURCE_FILE = BACKEND / "app" / "api" / "router.py"

_VALID_ACCESS_VALUES = {"read", "write"}


# ---------------------------------------------------------------------------
# AST helpers (mirror the pattern from test_review_nrc_aps_operator_identity_drift_guard.py)
# ---------------------------------------------------------------------------


def _is_router_route(decorator: ast.expr) -> bool:
    """Return True if the decorator is any api_router.get/post/put/patch/delete call."""
...
def test_all_routes_have_wired_identity_seam() -> None:
    """Every route handler in router.py must gate on operator identity."""
    ...
    for node in ast.walk(tree):
        ...
        fname = node.name
        lineno = node.lineno
        label = f"router.py:{lineno} {fname}"
    ...
    if violations:
        ...
        raise AssertionError(
            f"{len(violations)} route(s) in router.py missing wired identity seam:\n{formatted}"
        )
...
def test_all_routes_declare_access_keyword() -> None:
    ...
    if violations:
        formatted = "\n".join(f"  - {v}" for v in violations)
        raise AssertionError(
            f"{len(violations)} route(s) in router.py with invalid or missing access= declaration:\n{formatted}"
        )
