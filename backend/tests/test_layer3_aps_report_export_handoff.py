...
def test_evidence_report_export_gate_dedupes_raw_and_fallback_candidates_for_same_run(
    tmp_path: Path,
) -> None:
    original_storage_dir = settings.storage_dir
    settings.storage_dir = str(tmp_path)
    try:
        ...
        gate_report = aps_report_export_gate_module.validate_evidence_report_export_gate(
            run_ids=None,
            limit=10,
            report_path=tmp_path / "evidence_report_export_gate_dedupe_scope.json",
            require_runs=True,
        )
