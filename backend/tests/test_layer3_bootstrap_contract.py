...
def test_layer3_bootstrap_contract_is_shared() -> None:
    ...
    assert direct_body["execution_readiness"]["sec_edgar_text_table_source_acquisition_authority_admitted"] is True
    ...
    assert (
        direct_body["execution_readiness"]["sec_edgar_text_table_live_source_artifact_downstream_proof_admitted"]
        is True
    )
    ...
    assert direct_body["execution_readiness"]["sec_edgar_html_inline_xbrl_downstream_operator_status_admitted"] is True
    assert direct_body["execution_readiness"]["sec_edgar_html_inline_xbrl_downstream_operator_status_endpoint"] == (
        "/api/v1/layer3/source/sec-edgar/html-inline-xbrl/downstream-proof/status"
    )
    assert (
        direct_body["execution_readiness"][
    ...
    assert (
        direct_body["execution_readiness"]["candidate_b_full_corpus_operator_workflow_completion_monitor_admitted"]
        is True
    )
    assert direct_body["execution_readiness"]["candidate_b_full_corpus_operator_workflow_completion_monitor_endpoint"] == (
        "/api/v1/layer3/source/ingestion/candidate-b/full-corpus/operator-workflow/completion/monitor"
    )
    assert (
        direct_body["execution_readiness"]["candidate_b_full_corpus_operator_repeatability_checkpoint_admitted"]
        is True
    )
