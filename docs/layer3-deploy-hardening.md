...
- In proxy-owned non-local posture, route-level operator-identity presence is enforced before service logic on the Layer 3 core workbench, handoff, package, source-ingestion, and source/sec-edgar POST surfaces, plus selected sensitive core-workbench/handoff/source-ingestion/source-sec-edgar GET status/read surfaces. The public metadata GETs `/bootstrap`, `/readiness`, and `/authority-matrix` remain explicitly exempt.
...
3. **Auth owner:** keep `AUTH_OWNER=proxy`; the app requires proxy-owned posture. Route-level operator-identity PRESENCE is now enforced in-app for the Layer 3 core-workbench/handoff/package/source_ingestion/source-sec-edgar POST surface and selected sensitive core-workbench/handoff/source_ingestion/source-sec-edgar GET status/read routes: fail-closed 409/401 under proxy posture, inert under local default. This is identity-presence enforcement only, NOT role/authorization policy. Public metadata GETs (`/bootstrap`, `/readiness`, `/authority-matrix`) stay open; StaticFiles wrapping, remaining GET surfaces, and reverse-proxy authn/authz remain deployment-owned unless separately governed.
...
5. **Workspace/groups header (required) and optional email metadata:** the proxy MUST supply the configured `PROXY_GROUPS_HEADER` (default `X-Forwarded-Groups`) on every request to the Layer 3 core-workbench/handoff/package/source_ingestion/source-sec-edgar POST surface and selected sensitive core-workbench/handoff/source_ingestion/source-sec-edgar GET status/read routes; the in-app identity seam derives workspace authority from it and rejects requests without it (401). Configure `PROXY_EMAIL_HEADER` only if the proxy can supply trustworthy values. The proxy must strip or overwrite any client-supplied values for both headers before forwarding.
