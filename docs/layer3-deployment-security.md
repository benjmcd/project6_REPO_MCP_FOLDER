...
- In proxy-owned non-local posture, the route-level operator-identity seam now fails closed before service logic on Layer 3 core workbench, handoff, package, source-ingestion, and source/sec-edgar POST routes, plus selected sensitive core-workbench/handoff/source-ingestion/source-sec-edgar GET status/read routes. The seam is inert under the local default profile. The public metadata GETs `/bootstrap`, `/readiness`, and `/authority-matrix` remain explicitly exempt.
...
2. Reverse-proxy authentication and authorization enforcement for `/api/v1`, `/review/*`, and any file-serving surface; the app requires proxy-owned posture. Route-level operator-identity PRESENCE is now enforced in-app for the Layer 3 core-workbench/handoff/package/source_ingestion/source-sec-edgar POST surface and selected sensitive core-workbench/handoff/source_ingestion/source-sec-edgar GET status/read routes: fail-closed under proxy posture, inert under local default. This is identity-presence enforcement only, NOT role/authorization policy. SEC XBRL routes were already enforced via doc 1352. Public metadata GETs stay open; StaticFiles wrapping, remaining GET surfaces, and reverse-proxy authn/authz remain deployment-owned.
