...
The selected final 0.3.0 profile claims the public_connectors overlay for public/anonymous connectors and the sec_xbrl_offline overlay for offline/replay SEC XBRL proof only. ScienceBase public/MCS, Senate LDA anonymous metadata, World Bank Indicators anonymous metadata, and connector run observability are supported for operator-workflow + local-deployment under the local_expert base. SEC XBRL value-bearing support remains simulation/offline-replay on already-acquired operator-supplied evidence, and bounded SEC live source-artifact acquisition remains explicit-default-off.
