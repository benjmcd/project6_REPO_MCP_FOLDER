...
- Decision: (c). Send-capable paths require the current definition/grant
  ...
  gap, orphan, partial addition, or drop/relabel fails. Campaign armings, log
  seals, and both connector-run seal events bind the introduction
  revision/digest. Historical validation accepts no archive/index path from the
