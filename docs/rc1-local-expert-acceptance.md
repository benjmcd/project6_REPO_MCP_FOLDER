...
| Local profile operational acceptance | `scripts/local_profile_acceptance.py` and `backend/tests/test_release_local_profile_operational_acceptance.py` prove install/run, restart-survival, backup/restore, and hidden-state refusal. The RC1 capstone runner verifies the test is collected without re-running the heavy live-process backup/restore path and, in CI, records the upstream `backend-layer3-api` dependency result before passing this criterion. |
