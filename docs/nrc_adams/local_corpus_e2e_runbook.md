...
The runtime classifies existing fact-authority inventory into redacted statement candidate groups without re-fetching SEC content, reparsing retained HTML/iXBRL, reading raw values into operator projection, resolving taxonomies, or claiming final audited statement semantics. PR #1901 is now merged into current main; the next exact posture is downstream product/inspection selection.
...
Current main now includes the SEC HTML/iXBRL package-review preview runtime. The next slice should select package construction commit over package-review preview receipts, while keeping review submit, handoff/export, delivery, taxonomy/CompanyFacts, provider, connector, RAG/model, full mockup, and frontend durable authority separate.
...
This selection freezes the SEC HTML/iXBRL package construction commit slice over existing package-review preview receipts. The future runtime should commit redacted server-owned package payload artifacts and construction receipts while keeping review submit, handoff/export, delivery, taxonomy/CompanyFacts, provider, connector, RAG/model, full mockup, and frontend durable authority separate.
...
The SEC HTML/iXBRL package-construction runtime now commits package-review preview receipts into server-owned package construction authority with redacted payload artifacts. Package-review submit, handoff/export, delivery, taxonomy/CompanyFacts, provider, connector, RAG/model, full mockup, and frontend durable authority remain separate future slices.
...
Current main now includes the SEC HTML/iXBRL package-construction commit runtime. The next slice should select package-review submit over package-construction receipts, while keeping handoff/export, delivery, taxonomy/CompanyFacts, provider, connector, RAG/model, full mockup, and frontend durable authority separate.
...
This selection freezes package-review submit over current-main SEC HTML/iXBRL package-construction receipts. The future runtime should record a bounded operator decision and review-submit receipt while keeping handoff/export, delivery, taxonomy/CompanyFacts, provider, connector, RAG/model, full mockup, and frontend durable authority separate.
...
PR #1920 landed the governed SEC/EDGAR real-company path on current main. The current-main path satisfies the selected objective without admitting provider writes, connector dispatch, RAG/model runtime, full mockup activation, frontend-only durable authority, Candidate B PDF-only routing for SEC filings, or unauthorized source/parser expansion.
