...
This document is the canonical live-repo status surface for the NRC ADAMS APS stack. Last updated April 20, 2026 to reflect the bounded Layer 3 APS export-derived context-packet, multisource, evidence-report-export-package, package-derived-context, context-dossier, and deterministic-insight handoff slices now landed on current `main`, the landed export-package first shared-consumer freeze, package-derived-context freeze, context-dossier freeze, and deterministic-insight continuation freeze now all landed on current `main`, the merged malformed-scoped candidate-discovery closeout now also landed on current `main`, the merged deterministic-gate exact-run hardening now also landed on current `main`, and the current open PR `#128` carrying the read-only deterministic-challenge continuation freeze beyond that landed deterministic-insight boundary, on top of the earlier bounded evidence-bundle, citation-pack, evidence-report, and evidence-report-export handoff slices landed on current `main`, the Deterministic Challenge Review Packet v1 closeout, and the narrow Tier2 diagnostics-write closeout.
...
10. current open PR `#128` now carries the read-only `19_GATED_APS_DETERMINISTIC_CHALLENGE_FREEZE.md` freeze selecting `deterministic_challenge_artifact` as the next deterministic continuation beyond the landed deterministic-insight handoff; that open freeze still does not admit implementation, challenge-review-packet fan-out, validate-only expansion, route/UI, runtime DB, or schema widening by itself
11. the next safe move is to land current open PR `#128`, then open the bounded handoff lane rooted in `backend/app/services/layer3_aps_deterministic_challenge_artifact_handoff.py` and `backend/tests/test_layer3_aps_deterministic_challenge_artifact_handoff.py`
12. rerun `.\project6.ps1 -Action prove-nrc-aps-document-processing -RequireOcr` when OCR/corpus behavior changes, and treat `.\project6.ps1 -Action validate-nrc-aps-content-index` plus `.\project6.ps1 -Action validate-nrc-aps-evidence-bundle` as the validate-only refresh path for those checked-in reports
