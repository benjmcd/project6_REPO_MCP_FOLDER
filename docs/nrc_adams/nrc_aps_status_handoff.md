...
8. `38_L3_WB_EXECUTION_SELECTION_FREEZE.md` and `39_L3_WB_EXECUTION_SELECTION_API_AND_STATE_CONTRACT.md` add planning-only execution-selection governance for a future selected/not-started `L3PassRun` shell boundary after approved-plan and preview-hash validation. They do not admit `AnalysisRun`, analysis execution, results/package/handoff artifacts, approved-plan supersession, runtime DB/schema widening, source-breadth expansion, local upload ingestion, or full mockup activation.
