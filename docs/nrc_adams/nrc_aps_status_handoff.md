...
6. the next justified move is a fresh read-only freeze for `context_dossier`, because the live shared graph still leaves it as the next unresolved later shared APS family before deterministic fan-out
