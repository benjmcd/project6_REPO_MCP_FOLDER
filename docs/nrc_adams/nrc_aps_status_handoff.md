...
19. no later post-validate-only freeze has yet landed on current `main`; if continuation resumes beyond the now-landed dedicated validate-only runtime/report-ref boundary, first settle a fresh read-only freeze rather than widening directly into validate-only top-chain expansion, promotion, retrieval cutover, route/UI, runtime DB, or schema scope
