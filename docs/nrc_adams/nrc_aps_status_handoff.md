...
8. open PR `#124` now carries the read-only `18_GATED_APS_DETERMINISTIC_INSIGHT_FREEZE.md` freeze selecting `deterministic_insight_artifact` as the next deterministic continuation beyond the landed dossier boundary; that open freeze has not landed on current `main` yet and must land before any later deterministic, review-packet, route/UI, runtime DB, or schema widening
