...
6. current `main` also includes the merged deferred-scope prep packet rooted in `24_L3_WB_FREEZE.md`, `25_L3_QUAL1_FREEZE.md`, `26_L3_WB_INPUTS.md`, and `27_L3_QUAL1_INPUTS.md` from PR `#165`, `#168`, and `#172`; current `main` also includes `28_L3_WB_FIRST_SLICE_FREEZE.md` from PR `#178` and `29_L3_WB_FIRST_SLICE_API_AND_STATE_CONTRACT.md` from PR `#182` as the governing first-slice scope/API contract, plus the PR `#184` implementation and PR `#185` through PR `#189` closeout/correction passes that make `/review/layer3` and `/api/v1/layer3/...` live only for the bounded first-slice workbench without reopening or extending the settled later APS family packet. When present, `30_L3_WB_PLAN_PREVIEW_FREEZE.md` and `31_L3_WB_PLAN_PREVIEW_API_AND_STATE_CONTRACT.md` define only the next planning-only plan-preview slice after explicit Gate C typing commit; they still leave execution, results, package review, handoff, runtime DB writes, schema widening, qualitative/hybrid/RAG/vector execution, and LLM planning out.
