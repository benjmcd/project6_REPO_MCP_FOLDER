# 07 Phase1A Implementation Local Decisions REV2

## Correction Note

This revision regenerates the Phase 1A local-freeze decisions from the allowed source set only.

Corrected from the prior freeze doc:
- source hygiene is now explicit and clean,
- the decisions below were rederived from allowed planning, curated repo-root, and baseline artifact sources only,
- no hidden memory, session, or profile-state source was used.

Retained unchanged:
- exact owner-module recommendation,
- exact migration posture,
- exact proof/test placement recommendation,
- exact internal entrypoint posture,
- exact recommended command order,
- exact stop/escalation posture.

Material effect on the later write-enabled session:
- no material change to tranche scope,
- no material change to the recommended implementation-local defaults.

Evidence basis: `A|next_milestone_plans/Layer3_execution_freeze/07_PHASE1A_IMPLEMENTATION_LOCAL_DECISIONS.md|artifact|correction target`; `P|layer3_primary_planningdocs/12_LAYER3_ROADMAP_PHASES_AND_OPEN_QUESTIONS.md|Can be decided implementation-locally once the pack is accepted|137-141`; `A|next_milestone_plans/Layer3_execution_handoff/06_PHASE1A_CODEWRITING_HANDOFF.md|artifact|sections 5-8`
...
6. current freeze-pack artifacts
7. all other artifacts
...
1. `Settled from source evidence`
   ...
   `Evidence:` `P|layer3_primary_planningdocs/03_LAYER3_SESSION_LEDGER_AND_WORKSPACE_MODEL.md|Frozen decisions in scope|25-30`; `P|layer3_primary_planningdocs/03_LAYER3_SESSION_LEDGER_AND_WORKSPACE_MODEL.md|Design thesis and canonical write order|45-89`; `R|docs/nrc_adams/nrc_aps_status_handoff.md|Schema/migration authority|142-145`; `A|next_milestone_plans/Layer3_execution_handoff/04_PHASE1A_FILE_TOUCH_AND_OWNER_MATRIX.md|artifact|models owner row`
...
2. `Settled from source evidence`
   ...
   `Evidence:` `P|layer3_primary_planningdocs/08_LAYER3_UI_WORKBENCH_AND_API_SURFACE.md|Route-family posture|146-155`; `P|layer3_primary_planningdocs/12_LAYER3_ROADMAP_PHASES_AND_OPEN_QUESTIONS.md|Explicit out-of-scope items for the first slice|123-129`; `R|backend/app/api/router.py|api_router include_router review_nrc_aps and market_data routers|93-97`; `R|backend/app/api/review_nrc_aps.py|router and first review routes|38-52`; `A|next_milestone_plans/Layer3_execution_handoff/04_PHASE1A_FILE_TOUCH_AND_OWNER_MATRIX.md|artifact|rows for backend/main.py, backend/app/api/router.py, backend/app/api/review_nrc_aps.py`
...
3. `Settled from source evidence`
   ...
   `Evidence:` `P|layer3_primary_planningdocs/09_LAYER3_PERSISTENCE_RUNTIME_AND_SECURITY_BOUNDARIES.md|Runtime DB rule|76-83`; `P|layer3_primary_planningdocs/06_LAYER3_ENGINE_MAP_AND_EXISTING_REPO_REUSE.md|Reuse anti-patterns|189-196`; `R|backend/app/services/review_nrc_aps_document_trace.py|_resolve_safe_runtime_path|169-180`; `A|next_milestone_plans/Layer3_execution_handoff/05_PHASE1A_PROOF_RUNBOOK_AND_STOP_CONDITIONS.md|artifact|preflight rule 4 and stop condition 5`
...
5. `Settled from source evidence`
   ...
   `Evidence:` `P|layer3_primary_planningdocs/03_LAYER3_SESSION_LEDGER_AND_WORKSPACE_MODEL.md|Design thesis and canonical write order|45-89`; `R|docs/nrc_adams/nrc_aps_status_handoff.md|Schema/migration authority|142-145`; `R|backend/main.py|_run_migrations and _initialize_database|19-38`
...
2. `Recommended implementation-local choice.`
   ...
   `Evidence:` `P|layer3_primary_planningdocs/08_LAYER3_UI_WORKBENCH_AND_API_SURFACE.md|Route-family posture|146-155`; `A|next_milestone_plans/Layer3_execution_handoff/04_PHASE1A_FILE_TOUCH_AND_OWNER_MATRIX.md|artifact|conditional api.py row`; `A|next_milestone_plans/Layer3_execution_handoff/06_PHASE1A_CODEWRITING_HANDOFF.md|artifact|sections 5 and 7`
...
4. `Recommended implementation-local choice.`
   ...
   `Evidence:` `P|layer3_primary_planningdocs/11_LAYER3_VALIDATION_PROOF_AND_DECISION_GATES.md|Frozen decisions in scope and explicit non-goals|18-27`; `R|docs/nrc_adams/nrc_aps_status_handoff.md|Schema/migration authority|142-145`; `A|next_milestone_plans/Layer3_execution_handoff/05_PHASE1A_PROOF_RUNBOOK_AND_STOP_CONDITIONS.md|artifact|escalation trigger 2`
...
5. `Recommended implementation-local choice.`
   ...
   `Evidence:` `R|docs/analyst_insight/analyst_insight_status_handoff.md|Current proof basis|62-68`; `R|docs/nrc_adams/nrc_aps_status_handoff.md|Current proof freshness and remaining gaps|36-37`; `P|layer3_primary_planningdocs/11_LAYER3_VALIDATION_PROOF_AND_DECISION_GATES.md|Minimum proof outputs per slice|129-134`
...
3. `Still unresolved / escalate.`
   ...
   `Evidence:` `R|backend/main.py|_initialize_database DB_INIT_MODE path|31-38`; `P|layer3_primary_planningdocs/12_LAYER3_ROADMAP_PHASES_AND_OPEN_QUESTIONS.md|Reopen triggers|148-153`
