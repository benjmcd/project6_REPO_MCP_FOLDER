...
- no later post-validate-only freeze has yet landed on current `main`; if continuation resumes beyond that now-landed dedicated validate-only boundary, first settle a fresh read-only freeze instead of widening directly into validate-only top-chain expansion, promotion, retrieval cutover, route/UI, runtime DB, or schema scope
