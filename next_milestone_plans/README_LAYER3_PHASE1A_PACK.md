...
And to the Layer 3 workbench external export/download delivery UI freeze packet, which is planning-only by itself and governs the rendered `/review/layer3` download control separately implemented by PR `#282` over the already-live PR `#278` same-origin delivery endpoint:
