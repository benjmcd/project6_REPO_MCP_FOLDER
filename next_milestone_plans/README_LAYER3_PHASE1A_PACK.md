...
- no later post-validate-only freeze has yet landed on current `main`; this branch now carries the read-only `23_GATED_APS_PROMOTION_FREEZE.md` freeze, which selects promotion as the first later APS family beyond that landed dedicated validate-only boundary while keeping retrieval cutover later
