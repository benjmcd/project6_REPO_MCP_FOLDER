...
Doc `154` is the current raw-ingestion materialization implementation-entry freeze. It selects `raw_mixed_existing_source_materialization_entry` for a later `POST /api/v1/layer3/source/mixed-corpus/materialize` boundary that may materialize only deterministic `dataset_version` and `aps_content_document` source authority from a server-owned, hash-checked manifest, while leaving the existing seed route no-write and seed-only. It is planning/control only and does not add runtime behavior, routes, DTOs, services, models, migrations, file writes, source adapter registry behavior, local upload, local-directory ingestion, web connector retrieval, RAG/vector retrieval, provider/public URLs, connector/destination dispatch, package mutation/reconstruction, hidden LLM planning, full mockup activation, auth/security behavior, rendered controls, frontend-only durable authority, or theme behavior.
