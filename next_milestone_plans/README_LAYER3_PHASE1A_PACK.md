...
Docs `151`/`152` define the planning-only `qual_aps_rendered_downstream_existing_controls_only` UI freeze/contract. They select only a future rendered `/review/layer3` activation over already-live qualitative APS backend/API package/downstream steps, require server-authoritative state and headed/headless Chromium theme proof, and keep backend changes, source expansion, provider/public URLs, signed URLs, connector/destination dispatch, RAG/vector behavior, hidden LLM planning, package mutation/reconstruction, full mockup activation, and auth/security behavior blocked.
