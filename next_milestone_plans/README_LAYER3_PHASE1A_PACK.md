...
PR `#243` is the merged backend-only implementation for that packet. It adds `POST /api/v1/layer3/package/review/submit`, records one operator decision in existing reconciliation/session JSON, verifies the constructed package ids and payload hashes, and keeps package rows, package payload refs/hashes, handoff/export, rendered UI behavior on current `main`, schema/runtime/source widening, and full mockup activation out until separately admitted. PR `#245` is the merged bounded rendered UI implementation that renders the package construction commit and package-review submit controls over the already-live PR `#238` and PR `#243` endpoints on current `main`.
