...
- current `main` now also includes the landed read-only `23_GATED_APS_PROMOTION_FREEZE.md` freeze from PR `#145`; it selects promotion as the first later APS family beyond that landed dedicated validate-only boundary while keeping retrieval cutover later
