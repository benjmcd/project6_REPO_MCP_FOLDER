...
- docs `100`/`101` are the current-main PR `#485` rendered-control settlement boundary: the generic `/review/layer3` delivery form already exists from the earlier single-item UI path, so the next implementation must either make associated-cohort delivery UI activation explicit and server-authoritative or keep the control disabled for associated-cohort readiness
- the readiness, delivery, and UI-settlement governance do not admit public/signed URLs, connector dispatch, generic downstream dispatch, destination selection, package payload copy/rewrite/reconstruction, schema/runtime/source widening, retry/recovery/rerun expansion, pass-entry changes, broader UI, qualitative/hybrid/RAG/vector execution, or full mockup activation
...
Read docs `100`/`101` as current-main PR `#485` planning/control over existing rendered delivery UI code, not as runtime activation:
- the generic delivery form and same-origin attachment submission path already exist from PR `#282`/`#285`/`#286`
- associated-cohort activation still requires an explicit server-authoritative UI gate because PR `#483` proved backend/API delivery only
- if `browser_download_enabled` remains `false` and no replacement server gate is added, associated-cohort delivery must render unavailable
- a later implementation must prove headed and headless browser behavior before treating associated-cohort rendered delivery as live
