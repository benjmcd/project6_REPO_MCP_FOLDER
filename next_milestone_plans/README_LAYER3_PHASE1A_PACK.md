...
Docs `147`/`148` define the planning/control implementation-entry freeze for future `qual_aps_aps_handoff_dispatch_entry` over an already prepared qualitative APS internal handoff/export envelope. They select reuse of the APS handoff dispatch route family and require persisted qualitative prepare authority. This keeps the live `qualitative_aps_aps_handoff_dispatch_not_admitted` blocker until a separate runtime pass. They allow only the future APS owner-service handoff package row/artifact and dispatch state needed for APS evidence-bundle handoff, and they continue to block external export/download, connector/destination dispatch, provider/public URLs, rendered controls/theme behavior, source expansion, RAG/vector, hidden LLM, full mockup, auth/security, model/migration, package mutation/reconstruction, and package supersession.
