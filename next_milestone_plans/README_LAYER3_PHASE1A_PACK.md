...
Read docs `94`/`95` as branch-local planning-only governance by themselves:
...
- current code still blocks associated-cohort APS dispatch with `associated_cohort_aps_handoff_dispatch_not_admitted`
...
- a later implementation must prove exact PR `#432`/`#438`/`#443`/`#447`/`#451`/`#456`/`#460`/`#462` authority before narrowing that blocker
- the freeze does not admit external export/download, connector dispatch, generic downstream dispatch, destination selection, package payload copy/rewrite/reconstruction, schema/runtime/source widening, retry/recovery, pass-entry changes, broader UI, qualitative/hybrid/RAG/vector execution, or full mockup activation
