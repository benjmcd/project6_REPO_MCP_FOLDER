...
The next implementation pass should not repeat the APS-derived dataset selection, source-family surfacing, selected-material trace-detail, SEC HTML/iXBRL reconciliation, or server-owned raw mixed trace-labeling slices. Remaining work is:

1. Broader Phase P10 trace/detail surfacing for refused artifacts or mixed source families beyond the server-owned raw mixed materialization sentinel, only as those families become backed by server authority and need more than candidate-panel and selected-material ledger detail.
