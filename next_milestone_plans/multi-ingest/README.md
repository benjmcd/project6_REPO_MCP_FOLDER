...
This lane began as a planning/spec/design pack and now also includes Phase P1, Phase P2, Phase P3, Phase P4, Phase P4.5, Phase P5, Phase P6, bounded Phase P10A source/test/UI changes, bounded Phase P7 XLSX parser/materialization changes, bounded Phase P7.5 generic table bridge orchestration, bounded Phase P8 JSON recordset parser/materialization, bounded Phase P9 plain-text SEC/EDGAR filing parser/materialization, bounded Phase P10B workbench source-family surfacing, bounded Phase P10C selected-material dataset trace/detail surfacing, and bounded Phase P10D selected APS content-document trace/detail surfacing. It still does not change schema, migrations, Layer 3 typing rules, parser behavior, Candidate B behavior, or mixed package semantics. The API changes are limited to material-preview accepting explicit `dataset_version_ids` and `aps_content_document_ids`, read-only Layer 3 candidate-list endpoints for APS-derived `DatasetVersion` rows and indexed APS content documents, source-family metadata for admitted/deferred/refused APS table families, selected-material source trace metadata for APS-derived `DatasetVersion` rows and selected `ApsContentDocument` rows, and connector run/report refs exposing the generic table dataset bridge when explicitly enabled.
...
| XML/HTML | Explicitly refused today | Declared/sniffed/extension XML/HTML are refused; inline XBRL/XML/HTML filing documents are not admitted by Phase P9 |
| SEC/EDGAR filings/submissions | Bounded parser/materialization implemented for complete submission text files with plain document text and simple delimited `<TABLE>` blocks | Partial; SEC/EDGAR complete submission signatures are sniffed as `application/x-sec-edgar-submission`, metadata/sections become document units, extracted delimited table blocks can materialize through the generic table bridge, and only admitted forms such as `10-K`, `10-Q`, and `8-K` are allowed by default. HTML/XML/inline XBRL filings, unsupported forms, ambiguous tables, archive-member orchestration, schema changes, new Layer 3 source semantics, and richer financial statement semantics remain fail-closed or deferred. |
...
2. A follow-up SEC/EDGAR parser slice for HTML/XML/inline XBRL only after a separate section/table/statement contract is specified.
3. Mixed qualitative-plus-table package semantics only after the existing SEC/EDGAR mixed parser contract is connected to governed package semantics.
