...
This lane began as a planning/spec/design pack and now also includes Phase P1, Phase P2, Phase P3, Phase P4, Phase P4.5, Phase P5, Phase P6, bounded Phase P10A source/test/UI changes, bounded Phase P7 XLSX parser/materialization changes, bounded Phase P7.5 generic table bridge orchestration, bounded Phase P8 JSON recordset parser/materialization, bounded Phase P9 plain-text SEC/EDGAR filing parser/materialization, bounded Phase P10B workbench source-family surfacing, and bounded Phase P10C selected-material trace/detail surfacing. It still does not change schema, migrations, or Layer 3 typing rules. The API changes are limited to material-preview accepting explicit `dataset_version_ids`, a read-only Layer 3 candidate-list endpoint that projects APS-derived `DatasetVersion` rows from existing `DatasetSourceProvenance` authority, source-family metadata for admitted/deferred/refused APS table families, selected-material source trace metadata for APS-derived `DatasetVersion` rows, and connector run/report refs exposing the generic table dataset bridge when explicitly enabled.
...
1. Broader Phase P10 trace/detail surfacing for document/refused/mixed source families only as those families become backed by server authority and need more than candidate-panel and selected-material ledger detail.
