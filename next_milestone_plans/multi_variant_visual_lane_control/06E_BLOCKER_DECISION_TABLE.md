...
It is using the achieved M5 barrier closure, the achieved M6A workbench record and pinned report artifact in `05J`, the frozen approved target record in `05L`, the frozen `03AA` + `05H` direct-admission packet, the achieved `05M` implementation record, and the merged-main closure/handoff in `05N` as the current closure state.
