...
| Runtime-root coexistence mechanism | IMPLEMENTATION REQUIRED | `review_nrc_aps_runtime_roots.py`; `backend/tests/test_review_nrc_aps_api.py` | Behavior requirement is frozen, but exact coexistence mechanism for experiments is not. Must produce exact experiment root naming/placement rule |
...
| Runtime DB binding / discovery semantics | IMPLEMENTATION REQUIRED | `backend/app/api/review_nrc_aps.py`; `backend/tests/test_review_nrc_aps_runtime_db.py` | Review API directly depends on runtime DB binding. Must produce exact no-change rule for run->binding resolution and session isolation |
