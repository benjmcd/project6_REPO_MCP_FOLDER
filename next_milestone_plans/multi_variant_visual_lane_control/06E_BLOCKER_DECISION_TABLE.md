...
| Exact approved M6B target naming | BOUNDED RESIDUAL | `00D`; `03AA`; `05H`; `03AB`; `05I` | The direct-admission packet is frozen, but no one selector value has yet been explicitly approved and recorded with evidence refs. The immediate next lane is the M6A workbench, not direct admission |
...
It is using the achieved M5 barrier closure, the frozen `03AB` + `05I` workbench packet as immediate input, and the frozen `03AA` + `05H` direct-admission packet as later input.
