...
| Broader T5/T6 acceptance gate | IMPLEMENTATION REQUIRED | `tests/test_api.py`; `tests/test_nrc_aps_document_corpus.py`; `tests/test_run_nrc_aps_local_corpus_e2e.py`; `06C`; `05D` | Full T1-T8 bootstrap acceptance still requires the broader root-side context/API/corpus bundles to pass. Current failures include DB/test-isolation issues in `tests/test_api.py` and expectation drift in `tests/test_nrc_aps_document_corpus.py`, both outside the localized MVVLC selector path but still blocking milestone acceptance |
...
| Acceptance harness operability gap | BOUNDED RESIDUAL | `06C`; `06E`; `backend/tests/review_nrc_aps_runtime_fixture.py` | The earlier T7/T8 harness defects are resolved. The remaining acceptance gap is no longer backend harness operability; it is broader root-side T5/T6 instability plus the still-unexecuted `06I` performance gate. Carry that explicitly instead of calling full bootstrap acceptance green |
