...
This document still records the pack-prepared sequencing logic, but the canonical merged authority worktree has already moved past the original pre-separation state:

- shared whole-document PageEvidence extraction now exists in `worktrees/pageevidence-main-merge/backend/app/services/nrc_aps_page_evidence.py`
- Candidate A whole-document projection now exists as a separate helper in that same file
- `nrc_aps_document_processing.py` remained untouched by that change
- that landed change matches the pack's service-layer structural-separation objective and leaves runner/report adaptation as later work if reopened

---

## Recommended pass order

### Pass 1 — cleanup, lifecycle hardening, and structural separation

Goals:
- explicit `fitz` document close / cleanup semantics
- no behavior drift
- no artifact meaning change
- separate shared evidence extraction from candidate projection inside the service-layer owner boundary
...
### Pass 2 — runner/report adaptation with compatibility bridge

Goals:
- adapt runner/report surfaces to the separated evidence/projection structure where needed
- preserve current admitted Candidate A behavior and pinned artifact meaning
...
Live-status note:
- the canonical merged authority worktree has already landed the service-layer separation step
- any reopened work should start from the post-Pass 1 / pre-runner-adaptation state rather than treating separation itself as still pending
