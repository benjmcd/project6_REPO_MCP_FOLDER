...
## Current live status and before-state topology

### Current live status

The canonical merged authority worktree has already landed the service-layer structural separation for whole-document PageEvidence handling.

Current live topology therefore includes:

1. `nrc_aps_page_evidence.py` now contains separate whole-document extraction and Candidate A projection helpers plus a compatibility wrapper.
2. `run_nrc_aps_page_evidence_workbench.py` still consumes PageEvidence service output and emits a workbench artifact.
3. `nrc_aps_document_processing.py` still contains the admitted Candidate A seam-local helper and remained untouched by the landed service-layer split.
4. Hidden-consumer surfaces downstream of processing still exist across content index, models/schemas, retrieval, evidence bundles, review, and report/export chains.
5. Candidate A is already admitted; `baseline` remains the default for the current horizon.

### Before-state A — historical fused baseline for topology comparison

The historical pre-separation topology was:
...
This before-state should now be treated as the comparison baseline for before/after blast-radius judgments, not as the literal current live shape.
...
- extraction logic
- candidate identity in projected/compatibility outputs if still present
- projection logic placement and compatibility-wrapper behavior
