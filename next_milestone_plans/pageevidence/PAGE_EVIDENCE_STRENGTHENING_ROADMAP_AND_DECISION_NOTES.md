...
This roadmap remains useful as a root-local planning/control artifact, but the canonical merged authority worktree has already landed the service-layer whole-document shared-evidence / Candidate A projection split that this pack should treat as Pass 1 structural separation.

As of the current repo state:

- `worktrees/pageevidence-main-merge/backend/app/services/nrc_aps_page_evidence.py` contains separate whole-document extraction and projection helpers
- the compatibility wrapper remains in place for the existing whole-document call path
...
- shared evidence / candidate projection separation inside the service-layer owner boundary
- no production touch to `nrc_aps_document_processing.py`

#### Pass 2
- runner/report adaptation to the separated structure
- compatibility bridge if required
- preserve current admitted Candidate A behavior and artifact meaning
- no production touch to `nrc_aps_document_processing.py` unless a separate escalation is approved

Current status:
- the service-layer structural-separation objective is now materially satisfied in the merged authority worktree
- any future reopen should begin from the post-Pass 1 / pre-runner-adaptation state
...
Question:
- Can runner/report adaptation and any compatibility bridge be completed without hidden-consumer drift or admitted Candidate A behavior change?
...
- the service-layer separation question has already been answered "yes" in the merged authority worktree
- future use of this gate is for runner/report adaptation, artifact compatibility handling, or another deeper outward-contract change
...
2. post-cleanup, pre-merged-structural-separation
3. post-merged-structural-separation, pre-runner-adaptation
4. post-runner-adaptation, pre-field-enrichment
5. post-field-enrichment, pre-evaluation expansion
