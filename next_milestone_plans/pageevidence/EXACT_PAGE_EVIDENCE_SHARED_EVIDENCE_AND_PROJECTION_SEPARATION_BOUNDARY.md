...
- this file belongs to the root-local planning branch copy of the PageEvidence pack
- its fused-state assumptions are planning assumptions carried by this branch copy, not claims that the current stronger merged-main worktree still has that exact internal shape

---

## Current live anchors and branch-local assumptions this lane is built on
...
4. The canonical merged authority worktree has already landed service-layer separation between shared whole-document extraction and Candidate A projection.
5. This root-local branch copy was drafted against an earlier state where PageEvidence still fused raw evidence extraction with projected visual-page classification.
6. The current projected class logic is permissive enough that dense-text pages with nonzero drawing/image counts may still be projected as visual.
