...
Current committed `main` facts:
- there is no dedicated `tests/test_nrc_aps_candidate_b_opendataloader_compare.py`
- `tests/test_nrc_aps_candidate_b_opendataloader.py` is a helper-focused unit test only
- that unit test does **not** generate raw outputs or write the proof/compare/retention artifacts

Current committed artifact posture:
- the durable Candidate B proof/compare/retention artifacts already in `tests/reports/` are historical workbench outputs
- review them as committed evidence, not as output generated by the validate-only pytest surface
