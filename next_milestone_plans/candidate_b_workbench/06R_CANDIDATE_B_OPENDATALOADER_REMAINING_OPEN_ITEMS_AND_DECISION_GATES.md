...
- The Phase 1 implementation admits Candidate B only as `document_processing_engine="candidate_b_opendataloader_pdf"` on the existing NRC APS run-submit flow.
- This does not revise the shipped bundle-scoped compare + Candidate B Trace surface, does not make Candidate B visible in the normal review run selector, and does not widen routes, schemas, models, migrations, document-trace parity, or persistence semantics.
...
- the current admitted scope is Phase 1 processing-engine admission only; selector visibility and runtime classification remain follow-on decisions
...
- runtime admission exists only for the opt-in processing engine path
- no normal run-selector entry

### Hard rule
Do not drift from Phase 1 processing-engine admission into selector admission, Workbench Compare integration, Candidate B Trace parity, document-trace parity, routes, schemas, models, migrations, or widened persistence by incidental follow-on edits.
