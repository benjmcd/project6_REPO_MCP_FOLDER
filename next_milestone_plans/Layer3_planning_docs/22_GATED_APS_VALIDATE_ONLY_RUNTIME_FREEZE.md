...
- this branch now carries the read-only `23_GATED_APS_PROMOTION_FREEZE.md` freeze selecting promotion as the first later APS family beyond that landed validate-only boundary while keeping retrieval cutover later
...
- no later post-validate-only freeze has yet landed on current `main`, and this branch now carries the read-only `23_GATED_APS_PROMOTION_FREEZE.md` freeze selecting promotion while keeping retrieval cutover later
