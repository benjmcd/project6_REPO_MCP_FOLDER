...
- the later post-validate-only freeze `23_GATED_APS_PROMOTION_FREEZE.md` has now landed on current `main`
...
- current `main` now also includes the landed read-only `23_GATED_APS_PROMOTION_FREEZE.md` freeze from PR `#145`, which selected promotion while keeping retrieval cutover later during the freeze decision itself
- live repo truth now also shows the existing promotion governance family already sufficient on current `main`, while retrieval cutover already exists there as a separate validate-only parity-proof family
- this freeze therefore remains the governing carried-forward boundary for the landed dedicated validate-only runtime/report-ref lane, but no further later APS family lane is currently justified by default
