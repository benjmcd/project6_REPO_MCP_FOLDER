...
acceptance:
  ...
  - id: no_regression
    check: full test_sec_xbrl*.py and test_layer3_api.py pass at >= baseline counts
      ...
      target-selection validation and git diff --check pass; l3-progress-check reports
      no new value-reveal/1353 findings beyond the pre-existing unrelated 558/561/562
      source-intake sync findings.
```
...
$env:DATABASE_URL = "sqlite:///$repo/backend/test_method_aware.db"
$env:STORAGE_DIR = Join-Path (Get-Location) "app\storage_test_runtime"
...
python tools/l3-progress-check.py   # expected current failure: unrelated pre-existing 558/561/562 sync findings only
git diff --check
```
