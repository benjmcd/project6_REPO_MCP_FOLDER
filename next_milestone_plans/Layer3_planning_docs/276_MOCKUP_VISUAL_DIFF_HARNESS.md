...
The current thresholds are calibration thresholds for a working deterministic harness: `normalizedMeanDeltaMax: 0.19` and `highDeltaRatioMax: 0.305` at a `360 x 220` comparison size after the bounded PDF-location selector refinement in `277_MOCKUP_PIXEL_REFINEMENT.md` and threshold tightening in `278_MOCKUP_THRESHOLD_TIGHTENING.md`. They are intentionally not a pixel-perfect threshold. A later pixel-refinement pass may tighten `MOCKUP_VISUAL_DIFF_LIMITS` only after the rendered theme is deliberately adjusted toward the repo-local frames.
