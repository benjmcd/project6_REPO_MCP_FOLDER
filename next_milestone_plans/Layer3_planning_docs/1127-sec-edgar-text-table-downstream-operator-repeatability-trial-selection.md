...
original_downstream_proof_hash_required: true
repeat_downstream_proof_hash_required: true
...
mismatched_dataset_version_must_reject: true
...
This selection admits the smallest SEC EDGAR text/table downstream repeatability trial after the current downstream chain closeout. The trial is not a SEC fetcher, parser, process runner, model runtime, connector dispatcher, or frontend authority surface. It should compare two server-owned downstream operator-status projections for the same DatasetVersion material authority and downstream proof chain, then write an append-only trial receipt only if both projections are current, `available`, redacted, and bound to the same envelope, bridge, Gate B, selection, material snapshot, and coverage authority.

The browser may supply only opaque receipt/status identifiers, hashes, fixed operator decision fields, and disposition fields needed to identify the original and repeat status projections. The server must reload and revalidate all proof/status authority before deciding the trial. SEC network acquisition, XML/HTML/inline XBRL parsing, raw filing URL authority, provider writes, connector dispatch, RAG/model runtime, full mockup activation, browser storage, and frontend durable authority remain blocked.

## Next Runtime Requirements

The next runtime implementation must:

1. Add a server endpoint for `record_sec_edgar_text_table_downstream_operator_repeatability_trial`.
2. Load and revalidate the original SEC EDGAR downstream operator-status projection.
3. Load and revalidate the repeat SEC EDGAR downstream operator-status projection.
4. Require both projections to be `available`.
5. Require the same DatasetVersion id/hash, materialization receipt hash, authority envelope hash, bridge receipt hash, material preview hash, Gate B decision/session, selection manifest, material snapshot hash, coverage evidence hash, and negative-invariant hash.
6. Compare operator-status hashes, proof hashes, coverage step sets, and downstream receipt/hash bindings.
...
8. Reject stale, missing, mismatched, raw-leaking, non-available, browser-owned, path-owned, URL-owned, or artifact-byte-owned authority.
...
- Is this endpoint a SEC/EDGAR corpus processor? Recommended answer: no. It is a repeatability comparator and receipt writer over two already server-owned downstream status projections.
- Does it admit SEC network fetches, XML/HTML/inline XBRL parsing, or raw filing URLs? Recommended answer: no.
- Can the browser provide paths, URLs, commands, stdout/stderr, artifact bytes, or process state to prove repeatability? Recommended answer: no. It can identify server receipts/statuses only; the server reloads authority.
