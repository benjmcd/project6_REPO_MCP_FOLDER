...
implemented_request_model: Layer3SecEdgarHtmlInlineXbrlFactMaterialDownstreamProofRequest
implemented_response_model: Layer3SecEdgarHtmlInlineXbrlFactMaterialDownstreamProofResponse
...
This runtime records a bounded SEC EDGAR HTML/iXBRL fact-material downstream Layer 3 proof over already-existing server authority. It re-reads the parser receipt, fact-authority receipt, fact-material bridge receipt, committed Gate B session, selection manifest, and committed DatasetVersion material snapshot before accepting downstream coverage evidence.

The runtime does not fetch SEC content, run submissions lookup, reparse or rematerialize retained content, reconstruct fact values, resolve taxonomy, classify facts into statements, mutate Gate B, write provider objects, dispatch connectors, add RAG/model runtime, activate full mockup behavior, or accept raw URL/path/artifact-byte/frontend/browser authority.
