...
- Latest committed branch head before this plan-preview DTO boundary hardening: `ed557fdd`.
...
- `python -m pytest $files -q`, where `$files` is the local `backend/tests/test_layer3*.py` set, passed with `261 passed, 4 warnings`.
