...
- Current merged-main authority after PR #558: `project6-origin/main` at `5831ff2f`.
- This recheck is current-main proof through the bounded PR #558 proof-hardening merge. It does not claim any deferred broad capability beyond the admitted Layer 3 surfaces listed here.
- `python -m pytest $files -q`, where `$files` is the local `backend/tests/test_layer3*.py` set, passed with `287 passed, 4 warnings`.
