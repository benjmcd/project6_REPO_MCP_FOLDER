...
- Current merged-main authority after PR #562: `project6-origin/main` at `369e4131`.
- This recheck is current-main proof through the bounded PR #562 response-contract extraction merge. It does not claim any deferred broad capability beyond the admitted Layer 3 surfaces listed here.
- `python -m pytest $files -q`, where `$files` is the local `backend/tests/test_layer3_*.py` set, passed with `288 passed, 4 warnings`.
