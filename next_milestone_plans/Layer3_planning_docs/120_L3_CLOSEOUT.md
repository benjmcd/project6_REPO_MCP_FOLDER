...
| Approved plan correction lifecycle | Planning/control freeze only | `135_APPROVED_PLAN_CORRECTION_FREEZE.md`; `backend/app/services/layer3_readiness_contract.py`; `backend/app/services/layer3_plan_flow_contract.py`; `tools/l3-progress-check.py` | No approved-plan cancellation, reopening, replacement, deletion, or supersession is live. Not route/API changes, not `L3AnalysisPlan` mutation, not `L3PassRun` or `AnalysisRun` mutation, not package/handoff/export, not connector/destination dispatch, not source widening, not broad qualitative/hybrid/RAG, not full mockup activation, and not auth/security behavior. |
