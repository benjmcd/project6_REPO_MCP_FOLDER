...
Branch-local status: `134_PLAN_REVISION_RECOVERY_ENTRY_FREEZE.md` adds implementation-entry freeze scope for `plan_revision_recovery_preview_refresh_entry` only. It selects the later route/service/DTO/schema-id shape but still admits no runtime recovery or live state/action change.
