...
| Service extraction to reduce workbench risk | Implemented narrowly | `backend/app/services/layer3_plan_revision_state.py`; `backend/app/services/layer3_gate_b_state.py`; `backend/app/services/layer3_source_boundary.py`; `backend/app/services/layer3_response_contract.py`; related tests including `backend/tests/test_layer3_response_contract.py`; Response envelope extraction keeps the shared response schema/version helper outside the workbench without changing emitted envelopes | No broad `layer3_workbench.py` rewrite and no behavior change beyond extracted ownership. |
