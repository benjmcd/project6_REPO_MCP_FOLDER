...
| Plan revision recovery lifecycle | Planning/control only | `132_PLAN_REVISION_RECOVERY_FREEZE.md`; `133_PLAN_REVISION_RECOVERY_CONTRACT.md`; `backend/app/services/layer3_state_model_contract.py` keeps `plan_rejected` and `plan_revision_requested` terminal with `allowed_next_actions: []` until a later implementation-entry freeze admits recovery | Not runtime recovery, not approved-plan supersession, not execution, not package/handoff/export, not connector/destination dispatch, not source widening, not broad qualitative/hybrid/RAG, not full mockup activation, and not auth/security behavior. |
