...
Doc `135_APPROVED_PLAN_CORRECTION_FREEZE.md` is planning/control only for `approved_plan_correction_lifecycle`. It records that approved-plan cancellation, reopening, replacement, deletion, and supersession remain not admitted after PR `#599`; a later implementation-entry freeze must choose exactly one bounded correction mode and prove server authority before any runtime route, DTO, state write, UI behavior, package behavior, execution behavior, connector/destination behavior, source widening, broad qualitative/hybrid/RAG behavior, full mockup activation, or authentication/security work can begin.

Doc `136_APPROVED_PLAN_CANCEL_ENTRY_FREEZE.md` is implementation-entry only for the future `approved_plan_cancel_without_replacement` mode. It selects one future route/service/DTO/schema-id shape for cancelling the current approved plan without creating a replacement, but it admits no runtime behavior by itself. Approved-plan supersession, replacement, reopening, deletion, `L3AnalysisPlan` creation, `L3PassRun` or `AnalysisRun` creation/mutation, package/handoff/export behavior, connector/destination dispatch, source/schema/runtime widening, broad qualitative/hybrid/RAG behavior, full mockup activation, and authentication/security work remain blocked until a later runtime PR proves them separately.
