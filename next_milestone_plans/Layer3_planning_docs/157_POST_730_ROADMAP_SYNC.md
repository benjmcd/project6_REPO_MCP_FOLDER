...
- raw mixed source-authority materialization through `POST /api/v1/layer3/source/mixed-corpus/materialize`, creating only deterministic admitted `dataset_version` and `aps_content_document` source-authority rows from server-owned, hash-checked refs, writing no files, and starting no Layer 3 flow;
