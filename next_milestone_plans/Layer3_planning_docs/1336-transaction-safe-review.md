...
The proof-capability diagnostic now calls the isolated orchestrator with
`single_transaction=True` and blocks readiness if the isolated response does not
prove both `single_transaction_claimed=true` and
