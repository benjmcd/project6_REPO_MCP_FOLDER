...
Status: planning/control implementation-entry freeze for the first raw mixed-corpus source-authority materialization pass after `153_SOURCE_BREADTH_FREEZE.md`.

This artifact does not implement raw ingestion. It selects the exact first materialization boundary that a later runtime branch may implement without widening source classes beyond current main.
...
The first later implementation may add only a server-owned manifest-to-source-authority materialization boundary for the current admitted source classes:
...
The selected future runtime surface is:
...
The future request may include only:
...
The future implementation may read:
...
The future implementation must write no files. Source payload files must already exist under server-owned storage-root refs and must be hash checked before row materialization.

The future implementation must create no `L3Session`, `L3Descriptor`, `L3SelectionManifest`, `L3MaterialSnapshot`, `L3TypingRecord`, `L3AnalysisPlan`, `L3PassRun`, execution/result/package/handoff/export state, connector dispatch/destination write, provider/public URL, signed URL, vector index, mockup state, or auth/security state.
