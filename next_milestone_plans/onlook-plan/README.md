...
- the repo-local integrity helper is `tools/check-onlook.ps1`; it verifies the preserved clones or tree-equivalent restored clones, required env files, preserved patch archives, can optionally rerun the bounded repo validations with `-RunValidation`, treats line-ending-only drift in the known runtime-generated files as non-blocking rather than misclassifying it as a semantic source edit, and now accepts the default same-origin sandbox config from `onlook-ui/.env.example` without requiring `onlook-ui/.env.local`
...
- the current-project first gate now resolves its default pair and proof contract through `tools/onlook-active-pair.json`; when that file points at tracked proof such as `tools/onlook-proof.json`, the default no-arg gate is repo-portable as long as helper fingerprints and runtime-clone provenance still match
- the current-project first gate now prefers a real `CSB_API_KEY` in `ext-onlook-fix/apps/web/client/.env.local`, and the local startup wrapper no longer requires a real parent-shell `CSB_API_KEY` when that file-based key is present
...
- the default duplicate-prep path now works without `-CopyLocalEnv` because the same-origin sandbox config comes from `onlook-ui/.env.example`; `-CopyLocalEnv` is only for intentional localhost review-API override
