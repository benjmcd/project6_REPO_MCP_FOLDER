...
- Workbench slice state is tracked separately below as 11 structured records: 6 planning-only records and 5 current-main live bounded implementation records, including 1 read-only implementation-readiness proof record. This keeps the settled later APS family decision from being confused with workbench readiness, and keeps PRs `#184`, `#194`, `#199`, `#205`, `#207`, `#212`, and `#213` plus their governing freeze/contract docs from remaining only prose-bound in this board.
...
- Current decision state: the `34`/`35` plan-revision contract is now implemented only as revision-control for the current server-backed preview before approval; PR `#212` added the planning-only execution-readiness packet, and PR `#213` implements only read-only readiness/hash/state proof surfaces against that packet.
