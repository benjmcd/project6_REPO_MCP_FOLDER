...
Current `main` now includes docs `75`/`76` plus PR `#417` as the bounded single-item `descriptive_summary` Gate C admission tranche. Associated-cohort admission, UI, schema/runtime/source, package/handoff/export, connector dispatch, qualitative/hybrid/RAG/vector behavior, and full mockup activation remain blocked.
...
- Current selected method boundary: current `main` includes docs `72`/`73` from PR `#402` as governance for the existing `descriptive_summary` recommendation label and PR `#411` as the bounded lower-level analysis API implementation. Docs `70`/`71` remain current-methods `AnalysisMethod` registry governance by themselves, PR `#316` separately implements the initial registry support guardrail, and PR `#411` adds only `descriptive_summary` registry membership plus deterministic lower-level execution. Gate C pass-entry, UI/source/runtime/schema widening, package/handoff/export, connector dispatch, qualitative/hybrid/RAG/vector expansion, and full mockup activation remain deferred. Public or signed URLs, connector dispatch, destination selection, generic downstream dispatch, package mutation/reconstruction, additional row families, schema/runtime/source widening, qualitative/hybrid/RAG/vector expansion, and full mockup activation remain deferred until separately governed and implemented.
...
| Descriptive summary lower-level implementation | merged live lower-level analysis API method | `72_L3_DESCRIPTIVE_SUMMARY_FREEZE.md`, `73_L3_DESCRIPTIVE_SUMMARY_CONTRACT.md`, `74_L3_DEFERRED_IMPLEMENTATION_PLAYBOOK.md` | PR `#411` | Implements `descriptive_summary` only in `backend/app/services/analysis.py` and `tests/test_api.py`: registry membership, fallback execution for datasets outside starter time-series assumptions, deterministic JSON artifact output, and assumption/caveat rows. It preserves Layer 3 Gate C fail-closed behavior and does not widen UI, schema/runtime/source, package/handoff/export, connector dispatch, qualitative/hybrid/RAG/vector, or full mockup behavior. |
| Descriptive summary Gate C admission | branch-local planning-only admission boundary | `75_L3_DESCRIPTIVE_SUMMARY_GATEC_ADMISSION_FREEZE.md`, `76_L3_DESCRIPTIVE_SUMMARY_GATEC_ADMISSION_CONTRACT.md` | pending | Freezes only a future single-item Gate C admission implementation boundary for the already-live lower-level `descriptive_summary` method. It does not implement pass-entry behavior, admit associated cohorts, widen UI/schema/runtime/source, activate package/handoff/export, connector dispatch, qualitative/hybrid/RAG/vector behavior, or full mockup behavior. |
...
The current method focus after PR `#411` is narrower than broader deferred work: docs `72`/`73` govern, and PR `#411` implements, only lower-level deterministic `descriptive_summary` analysis API support. It does not select Gate C admission, UI, schema/runtime/source, package/handoff/export, connector dispatch, qualitative/hybrid/RAG/vector, or full mockup work.
