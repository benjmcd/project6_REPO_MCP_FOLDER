...
- docs `68`/`69` as planning-only rendered `/review/layer3` download-control governance over the PR `#278` endpoint, plus PR `#282` as the bounded rendered UI implementation. Docs `68`/`69` do not implement UI behavior by themselves; PR `#282` renders only the server-gated same-origin delivery control and does not admit public/signed URLs, connector dispatch, destination selection, generic downstream dispatch, package mutation/reconstruction, additional rows, schema/runtime/source widening, or full mockup activation
