...
- PR `#258` docs `58`/`59` as APS handoff dispatch governance after `handoff_export_prepared`, PR `#260` as the bounded backend/API implementation into the existing `aps_evidence_bundle_handoff` owner-service family, PR `#261` as fail-closed authority hardening, PR `#263` as APS package-row allowlist hardening, docs `60`/`61` as planning-only rendered APS dispatch UI governance, and PR `#266` as the bounded rendered APS dispatch UI implementation. Docs `60`/`61` still do not make UI behavior live by themselves, and external export/download, connector dispatch, destination selection, package mutation/reconstruction, schema/runtime/source widening, and full mockup activation remain out
