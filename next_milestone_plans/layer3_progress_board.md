...
| Descriptive summary cohort requirements | planning-only requirements gate | `77_COHORT_REQS.md` | pending | Records the decisions required before any future associated-cohort `descriptive_summary` freeze: cohort data shape, method-selection rule, execution surface, provenance manifest, failure behavior, and proof expectations. Associated-cohort behavior and selected-pass workbench cohort execution remain blocked. |
