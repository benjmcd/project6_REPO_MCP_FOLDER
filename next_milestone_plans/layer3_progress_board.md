...
- if present before merge, `32_L3_WB_PLAN_APPROVAL_FREEZE.md` and `33_L3_WB_PLAN_APPROVAL_API_AND_STATE_CONTRACT.md` are branch-local planning-only docs for operator plan approval plus durable `L3AnalysisPlan` formation; they do not make plan approval live, do not admit `L3PassRun` creation, and do not admit execution/results/package/handoff, runtime snapshot DB writes, schema widening, qualitative/hybrid/RAG/vector execution, or hidden LLM planning
...
- The candidate third-slice plan-approval docs, when present, freeze only a future operator approval and approved-plan persistence boundary; they require a new narrow owner-service helper because current `materialize_pass_entry(...)` is execution-bearing
