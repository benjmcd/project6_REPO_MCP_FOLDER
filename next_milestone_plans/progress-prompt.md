...
- if a branch checkout adds `/api/v1/layer3/readiness`, explicit preview identity/hash metadata, or approval/revision serialization checks, classify that branch as read-only implementation-readiness proof only until GitHub and current `main` confirm it; do not treat it as execution, result/package/handoff, source-breadth, schema, or runtime DB expansion
