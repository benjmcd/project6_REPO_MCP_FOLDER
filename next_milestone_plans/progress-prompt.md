...
- if `next_milestone_plans/Layer3_planning_docs/27_L3_QUAL1_INPUTS.md` is present in the current checkout, preserve it as a branch-local planning-only companion input doc for the deferred qualitative single-item breadth axis rather than promoting it into merged milestones, packet-reopen evidence, or an active lane
...
- if `next_milestone_plans/Layer3_planning_docs/27_L3_QUAL1_INPUTS.md` is present in the current checkout, keep it adjacent to `25_L3_QUAL1_FREEZE.md` as branch-local deferred-scope companion prep rather than treating it as a milestone, merged-main fact, or active-lane marker
