...
- if the checkout matches current `main` after PR `#189`, preserve PRs `#185` through `#189` as post-PR184 status/cohesion/explicit-Gate-C-typing/review-feedback closeouts; keep the first-slice route/API scope unchanged while carrying forward response-envelope, blocked Gate B error, Gate C authority-rail count/source-context, and tracked-PR metadata corrections
- if `next_milestone_plans/Layer3_planning_docs/30_L3_WB_PLAN_PREVIEW_FREEZE.md` and `next_milestone_plans/Layer3_planning_docs/31_L3_WB_PLAN_PREVIEW_API_AND_STATE_CONTRACT.md` are present in the current checkout, preserve them as a planning-only second-slice plan-preview packet for the future workbench route family; do not promote them into live behavior, merged milestone count changes, packet-reopen evidence, schema widening, runtime snapshot DB writes, downstream execution/package scope, or full mockup/broader-workbench activation
...
- if `next_milestone_plans/Layer3_planning_docs/30_L3_WB_PLAN_PREVIEW_FREEZE.md` and `next_milestone_plans/Layer3_planning_docs/31_L3_WB_PLAN_PREVIEW_API_AND_STATE_CONTRACT.md` are present in the current checkout, keep them adjacent to `28_L3_WB_FIRST_SLICE_FREEZE.md` and `29_L3_WB_FIRST_SLICE_API_AND_STATE_CONTRACT.md` as the planning-only second-slice plan-preview packet; they do not make plan preview live without a later implementation PR and they do not admit execution/results/package/handoff, runtime DB/schema widening, qualitative/hybrid/RAG/vector execution, or LLM planning
...
- if refreshing from a checkout that matches current `main` after PR `#189`, keep PRs `#181` through `#189` in the tracked GitHub PR set where applicable, refresh the snapshot base and seed-checkout fields to the artifact refresh that includes PR `#189`, and do not treat the closeout/correction PRs as new merged milestones
...
- current `main` now also includes the post-PR184 status/cohesion/explicit-Gate-C-typing/review-feedback closeouts from PRs `#185` through `#189`; these preserve the bounded first-slice live scope while tightening response-envelope, Gate B blocked-error, Gate C authority-rail, and PR-tracking metadata
- when present in the checkout, `30_L3_WB_PLAN_PREVIEW_FREEZE.md` and `31_L3_WB_PLAN_PREVIEW_API_AND_STATE_CONTRACT.md` are planning-only second-slice plan-preview prep for the future workbench route family; they should be preserved without treating plan preview, downstream execution, results, package review, handoff, qualitative/hybrid/RAG/vector execution, runtime DB writes, schema widening, or LLM planning as live
