...
- preserve `layer3_workbench_current_decision` as the workbench-only decision surface: current `main` has first-slice shell/API, read-only plan preview, approval-only plan persistence, and pre-approval plan-revision control; no workbench behavior beyond that is selected without a later freeze/API-state contract
- preserve `layer3_workbench_slices` as the structured register for workbench deferred prep, first-slice contract/implementation, plan-preview contract/implementation, plan-approval contract/implementation, plan-revision contract, and plan-revision implementation; do not collapse these into the APS milestone table or change the 29 merged APS milestone count
