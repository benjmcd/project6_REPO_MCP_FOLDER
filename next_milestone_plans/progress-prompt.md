...
- preserve `layer3_workbench_current_decision` as the workbench-only decision surface: current `main` has first-slice shell/API, read-only plan preview, and approval-only plan persistence, and the next behavior may be selected only as planning freeze/API-state governance without making it live
- preserve `layer3_workbench_slices` as the structured register for workbench deferred prep, first-slice contract/implementation, plan-preview contract/implementation, plan-approval contract/implementation, and any later planning-only revision-control contract; do not collapse these into the APS milestone table or change the 29 merged APS milestone count
...
- if refreshing from a checkout that matches current `main` after PR `#202`, keep PRs `#200`, `#201`, and `#202` in the tracked GitHub PR set and merged-history wording, refresh the snapshot base to the PR `#202` merge commit used for the artifact refresh, and do not treat those docs/control syncs as new runtime implementation slices
- if `34_L3_WB_PLAN_REVISION_FREEZE.md` and `35_L3_WB_PLAN_REVISION_API_AND_STATE_CONTRACT.md` are present, keep them adjacent to the `32`/`33` plan-approval packet as the PR `#203` fourth-slice planning-only revision-control packet; do not describe plan rejection/revision as live until a later implementation PR proves it, and do not treat it as approved-plan reopening/supersession, pass-run creation, analysis execution, results/package/handoff activation, runtime DB/schema widening, or qualitative/hybrid/RAG/vector/LLM-planning admission
