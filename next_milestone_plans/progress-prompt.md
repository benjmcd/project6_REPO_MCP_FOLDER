...
- preserve `layer3_workbench_current_decision` as the workbench-only decision surface: current `main` has first-slice shell/API, read-only plan preview, approval-only plan persistence, and pre-approval plan-revision control hardened by PR `#207`; no workbench behavior beyond that is selected without a later freeze/API-state contract
- preserve `layer3_workbench_slices` as the structured register for workbench deferred prep, first-slice contract/implementation, plan-preview contract/implementation, plan-approval contract/implementation, plan-revision contract, and plan-revision implementation; PR `#207` belongs to the plan-revision implementation record as hardening rather than a new slice; do not collapse these into the APS milestone table or change the 29 merged APS milestone count
...
- if `34_L3_WB_PLAN_REVISION_FREEZE.md` and `35_L3_WB_PLAN_REVISION_API_AND_STATE_CONTRACT.md` are present, keep them adjacent to the `32`/`33` plan-approval packet as the PR `#203` fourth-slice planning-only revision-control packet; after PR `#205`, describe only pre-approval plan rejection/revision-control as live; after PR `#207`, describe backend row-locking and shared UI in-flight locking as hardening of that bounded slice, not as new functional scope; do not treat it as approved-plan reopening/supersession, pass-run creation, analysis execution, results/package/handoff activation, runtime DB/schema widening, or qualitative/hybrid/RAG/vector/LLM-planning admission
