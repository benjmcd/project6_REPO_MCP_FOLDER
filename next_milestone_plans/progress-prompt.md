...
- if a future branch carries additional readiness, execution, source, results, package, handoff, or correction behavior beyond PR `#213`, keep that future behavior branch-local until GitHub and current `main` both confirm it
- if a branch checkout based before PR `#213` contains `/api/v1/layer3/readiness`, explicit preview identity/hash metadata, or approval/revision serialization checks, classify that branch as read-only implementation-readiness proof until GitHub and current `main` confirm it; if current `main` already includes PR `#213`, classify only additional behavior beyond that merged proof as branch-local; do not treat either case as execution, result/package/handoff, source-breadth, schema, or runtime DB expansion
