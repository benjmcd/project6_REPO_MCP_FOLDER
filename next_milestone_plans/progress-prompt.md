...
- if `next_milestone_plans/Layer3_planning_docs/28_L3_WB_FIRST_SLICE_FREEZE.md` is present in the current checkout, preserve it as planning-only first-slice setup for the deferred future workbench route family rather than promoting it into merged milestones, packet-reopen evidence, a live `/review/layer3` route claim, or a live `/api/v1/layer3/...` API claim
- if `next_milestone_plans/Layer3_planning_docs/29_L3_WB_FIRST_SLICE_API_AND_STATE_CONTRACT.md` is present in the current checkout, preserve it as planning-only endpoint/state contract detail for the deferred first-slice workbench target rather than promoting it into merged milestones, active implementation scope, live route/API claims, schema widening, runtime snapshot DB writes, or downstream execution/package scope; if the checkout matches current `main` after PR `#182`, keep it as landed current-main planning-only history
...
- if the current checkout matches current `main` after PR `#178`, preserve the merged planning-only `28_L3_WB_FIRST_SLICE_FREEZE.md` first-slice setup target as landed current-main history without changing settled packet counts, current focus, or live route/API claims
...
- if `next_milestone_plans/Layer3_planning_docs/28_L3_WB_FIRST_SLICE_FREEZE.md` is present in the current checkout, keep it adjacent to `24_L3_WB_FREEZE.md` and `26_L3_WB_INPUTS.md` as planning-only first-slice setup rather than treating it as a milestone, active-lane marker, live route claim, or live API claim
- if `next_milestone_plans/Layer3_planning_docs/29_L3_WB_FIRST_SLICE_API_AND_STATE_CONTRACT.md` is present in the current checkout, keep it adjacent to `28_L3_WB_FIRST_SLICE_FREEZE.md` as the planning-only endpoint/state companion for a later implementation pass, not as route/API activation
...
- current `main` now also includes the merged planning-only `28_L3_WB_FIRST_SLICE_FREEZE.md` first-slice setup target from PR `#178`; preserve it without changing the settled packet state, merged milestone count, current focus, or route/API live-state claims
- current `main` now also includes the merged planning-only `29_L3_WB_FIRST_SLICE_API_AND_STATE_CONTRACT.md` endpoint/state companion from PR `#182`; preserve it without changing the settled packet state, merged milestone count, current focus, route/API live-state claims, schema/runtime widening posture, or downstream execution/package/handoff scope
