...
- preserve PR `#198` as the merged planning-only `32`/`33` plan-approval freeze/API-state packet; preserve PR `#199`, when merged, as the bounded approval-only implementation of that packet, not as pass-run creation, analysis execution, execution/results/package/handoff activation, runtime DB/schema widening, or qualitative/hybrid/RAG/vector/LLM-planning admission
...
- PR `#199`, when merged, implements the `32`/`33` plan-approval boundary with a narrow owner-service helper while keeping current `materialize_pass_entry(...)` execution-bearing and outside the workbench approval path
