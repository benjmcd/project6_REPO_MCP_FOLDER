...
- if refreshing from a checkout that matches current `main` after PR `#194`, keep PRs `#181` through `#194` in the tracked GitHub PR set where applicable, refresh the snapshot base and seed-checkout fields to the artifact refresh that includes PR `#194`, and do not treat the closeout/correction, metadata-sync, or read-only plan-preview implementation PRs as new merged milestones
