...
- if the current checkout also contains `backend/app/services/nrc_aps_validate_only_gates_contract.py`, `backend/app/services/nrc_aps_validate_only_gates.py`, and `backend/app/services/nrc_aps_validate_only_gates_gate.py`, render the dedicated validate-only runtime/report-ref implementation lane as `branch_only` unless GitHub confirms an open PR or merged state
