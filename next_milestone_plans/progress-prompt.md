...
- if refreshing from a checkout that matches current `main` after PR `#192`, keep PRs `#181` through `#192` in the tracked GitHub PR set where applicable, refresh the snapshot base and seed-checkout fields to the artifact refresh that includes PR `#192`, and do not treat the closeout/correction or metadata-sync PRs as new merged milestones
...
- current `main` also includes the post-PR191 progress-metadata sync from PR `#192`; preserve it as metadata/status alignment only, not as route/API scope expansion or downstream activation
