...
- if `36_L3_WB_EXECUTION_READINESS_FREEZE.md`, `37_L3_WB_STATE_HASH_IDEMPOTENCY_CONTRACT.md`, and `next_milestone_plans/layer3_workbench_proof_manifest.json` are present on a branch checkout, keep them adjacent to the `34`/`35` plan-revision packet as branch-local execution-readiness planning only; they define proof/readiness, state, preview-hash, idempotency, concurrency, revision-recovery, approved-plan-correction, output-taxonomy, and source-breadth gates before execution, but they do not select execution, change route/API/UI behavior, reopen approved plans, create pass runs, enable results/package/handoff, widen runtime DB/schema, or admit qualitative/hybrid/RAG/vector/LLM planning
