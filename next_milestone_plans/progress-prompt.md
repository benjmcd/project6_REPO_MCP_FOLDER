...
- if refreshing from a checkout that matches current `main` after PR `#191`, keep `30_L3_WB_PLAN_PREVIEW_FREEZE.md` and `31_L3_WB_PLAN_PREVIEW_API_AND_STATE_CONTRACT.md` adjacent to `28_L3_WB_FIRST_SLICE_FREEZE.md` and `29_L3_WB_FIRST_SLICE_API_AND_STATE_CONTRACT.md` as the merged planning-only second-slice plan-preview packet; they do not make plan preview live without a later implementation PR and they do not admit execution/results/package/handoff, runtime DB/schema widening, qualitative/hybrid/RAG/vector execution, or LLM planning
...
- current `main` now also includes the merged planning-only `30_L3_WB_PLAN_PREVIEW_FREEZE.md` and `31_L3_WB_PLAN_PREVIEW_API_AND_STATE_CONTRACT.md` second-slice plan-preview prep from PR `#191` for the future workbench route family; they should be preserved without treating plan preview, downstream execution, results, package review, handoff, qualitative/hybrid/RAG/vector execution, runtime DB writes, schema widening, or LLM planning as live
