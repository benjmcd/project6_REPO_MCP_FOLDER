...
- `planning_only_descriptive_summary_governance`: amber or other clearly current-main planning-only method-governance state; it must not look like live `descriptive_summary` support, `ANALYSIS_METHOD_REGISTRY` membership, recommendation or execution behavior change, Gate C pass-entry change, source/schema/runtime widening, UI activation, qualitative/hybrid/RAG/vector expansion, or full mockup activation
