...
- `planning_only_aps_handoff_dispatch_freeze`: amber-brown or other clearly planning-only APS handoff dispatch governance state; it must not look like a live endpoint, live UI control, external export/download, connector dispatch, destination selection, package mutation/reconstruction, source/schema/runtime widening, execution selection/start UI expansion, or full mockup activation
