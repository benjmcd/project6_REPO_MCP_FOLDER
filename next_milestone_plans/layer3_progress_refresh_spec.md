...
- if `next_milestone_plans/Layer3_planning_docs/32_L3_WB_PLAN_APPROVAL_FREEZE.md` and `next_milestone_plans/Layer3_planning_docs/33_L3_WB_PLAN_APPROVAL_API_AND_STATE_CONTRACT.md` are present before their GitHub PR is merged, preserve them as branch-local planning-only third-slice inputs; if GitHub later confirms merge, preserve them as planning-only plan-approval freeze docs and not as active implementation, execution/results/package/handoff activation, runtime DB/schema widening, or qualitative/hybrid/RAG/vector/LLM planning admission
