...
- if `64_L3_WB_EXTERNAL_EXPORT_DOWNLOAD_READINESS_UI_FREEZE.md` and `65_L3_WB_EXTERNAL_EXPORT_DOWNLOAD_READINESS_UI_STATE_CONTRACT.md` are present, classify them as planning-only rendered `/review/layer3` external export/download readiness UI governance over the already-live PR `#269` backend/API readiness endpoint; do not treat those docs alone as live UI behavior, browser download route/control, public or signed URL generation, file streaming, connector dispatch, destination selection, generic downstream dispatch, package mutation/reconstruction, additional reconciliation/package/artifact rows, `AnalysisArtifact`, source/schema/runtime widening, qualitative/hybrid/RAG/vector execution, or full mockup activation
- if current `main` includes merged PR `#275`, classify the `/review/layer3` external export/download readiness panel and one `prepare_external_export_download` submit action as current-main live bounded rendered readiness UI over PR `#269`; do not treat it as browser download route/control, public or signed URL generation, file streaming, connector dispatch, destination selection, generic downstream dispatch, package mutation/reconstruction, additional reconciliation/package/artifact rows, `AnalysisArtifact`, source/schema/runtime widening, qualitative/hybrid/RAG/vector execution, or full mockup activation
...
- if a future branch implements `/api/v1/layer3/handoff/export/download/deliver` or equivalent external export/download delivery behavior beyond docs `66`/`67`, classify it as branch-local until GitHub and current `main` both confirm it; preserve same-origin delivery over the existing validated APS evidence-bundle handoff artifact and do not treat it as public/signed URL generation, connector dispatch, destination selection, generic downstream dispatch, rendered download UI activation, package mutation/reconstruction, additional row family, schema/runtime/source widening, or full mockup activation
