...
- current `main` now also includes that fresh read-only decision in `22_GATED_APS_VALIDATE_ONLY_RUNTIME_FREEZE.md` from PR `#140`; if continuation resumes beyond the landed generic boundary, do not widen directly into implementation, promotion, retrieval cutover, route or UI, runtime DB, or schema scope without first following the implementation boundary settled by that freeze
