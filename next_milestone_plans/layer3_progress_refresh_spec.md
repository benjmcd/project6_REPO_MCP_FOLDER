...
- if refreshing from the branch that carries open docs-only closeout PR `#141`, keep that follow-up distinct from the already-landed PR `#140` freeze
- once that open docs-only closeout merges, current `main` still points next to the write-enabled implementation boundary settled by `22_GATED_APS_VALIDATE_ONLY_RUNTIME_FREEZE.md`; do not widen directly into implementation, promotion, retrieval cutover, route or UI, runtime DB, or schema scope without first following that freeze
