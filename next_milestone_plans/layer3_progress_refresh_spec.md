...
- preserve PR `#198` as the planning-only plan-approval freeze packet; preserve PR `#199`, when merged, as the bounded approval-only implementation that persists `L3AnalysisPlan` after server-backed preview and still does not admit `L3PassRun`, analysis execution, execution/results/package/handoff activation, runtime DB/schema widening, or qualitative/hybrid/RAG/vector/LLM planning
