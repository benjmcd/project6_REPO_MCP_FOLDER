...
- if `68_L3_WB_EXTERNAL_EXPORT_DOWNLOAD_DELIVERY_UI_FREEZE.md` and `69_L3_WB_EXTERNAL_EXPORT_DOWNLOAD_DELIVERY_UI_STATE_CONTRACT.md` are present only on a branch, classify them as branch-local planning-only rendered `/review/layer3` download-control governance over the PR `#278` endpoint; do not treat those docs alone as live UI behavior, public or signed URL generation, connector dispatch, destination selection, generic downstream dispatch, package mutation/reconstruction, additional row family, `AnalysisArtifact`, schema/runtime/source widening, qualitative/hybrid/RAG/vector expansion, or full mockup activation
- if a future branch implements rendered download controls or delivery behavior beyond PR `#278`, classify it as branch-local until GitHub and current `main` both confirm it; preserve same-origin delivery over the existing validated APS evidence-bundle handoff artifact and do not treat it as public/signed URL generation, connector dispatch, destination selection, generic downstream dispatch, package mutation/reconstruction, additional row family, schema/runtime/source widening, or full mockup activation
