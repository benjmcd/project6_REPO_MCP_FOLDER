...
- if a future branch implements `/api/v1/layer3/handoff/export/download/prepare` or equivalent external export/download readiness behavior, classify it as branch-local until GitHub and current `main` both confirm it; preserve the docs `62`/`63` reference-only descriptor boundary over the existing APS evidence-bundle handoff artifact and do not treat it as browser download, public/signed URL generation, connector dispatch, destination selection, package mutation/reconstruction, additional reconciliation/package/artifact rows, `AnalysisArtifact`, source/schema/runtime widening, rendered UI activation, or full mockup activation
