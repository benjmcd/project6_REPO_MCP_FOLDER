...
- if continuation resumes beyond this landed boundary, start with a fresh read-only decision on whether to admit a dedicated validate-only family-specific report-ref/runtime lane rather than widening directly into promotion, retrieval cutover, route or UI, runtime DB, or schema scope
