...
Current `main` includes docs `72_L3_DESCRIPTIVE_SUMMARY_FREEZE.md` and `73_L3_DESCRIPTIVE_SUMMARY_CONTRACT.md` from PR `#402` as descriptive-summary governance, PR `#411` as bounded lower-level `descriptive_summary` analysis API support, docs `75_L3_DESCRIPTIVE_SUMMARY_GATEC_ADMISSION_FREEZE.md` and `76_L3_DESCRIPTIVE_SUMMARY_GATEC_ADMISSION_CONTRACT.md` as single-item Gate C admission governance, PR `#417` as bounded single-item Gate C admission implementation, and `77_COHORT_REQS.md` as associated-cohort requirements governance. PR `#423` adds docs `78_COHORT_FREEZE.md` and `79_COHORT_CONTRACT.md` as service-materialize associated-cohort freeze/contract governance. PR `#424` implements explicit service-owned associated-cohort `descriptive_summary` materialization only through `materialize_pass_entry(...)` when `L3AnalysisSet.formation_basis_json["requested_method_name"] == "descriptive_summary"`. PR `#425` hardens that path so absent, non-string, whitespace-padded, malformed, or otherwise non-exact requested-method metadata preserves the default cohort behavior instead of silently selecting `descriptive_summary`. PR `#428` syncs the cohort planning docs to the PR `#424`/`#425` live state, PR `#429` syncs cohort service proof surfaces, PR `#430` adds docs `80_COHORT_EXECUTION_FREEZE.md` and `81_COHORT_EXECUTION_CONTRACT.md` as selected-pass associated-cohort execution-start/result-status governance, PR `#432` implements that bounded backend/API path, PR `#433` records post-PR432 proof-state docs/control sync only, PR `#434` lands docs `82_COHORT_RESULT_REVIEW_FREEZE.md` and `83_COHORT_RESULT_REVIEW_CONTRACT.md` as planning/control selected-pass associated-cohort result-review governance, PR `#435` corrects post-PR434 count metadata only, PR `#436` syncs cohort-review lineage metadata only, PR `#438` implements the exact bounded backend/API associated-cohort result-review path, PR `#441` adds docs `84_COHORT_RESULT_REVIEW_UI_FREEZE.md` and `85_COHORT_RESULT_REVIEW_UI_CONTRACT.md` as planning-only associated-cohort result-review UI governance, and PR `#443` implements only the exact rendered `/review/layer3` associated-cohort result-review presentation/control path over that authority. Do not treat docs `77`/`78`/`79` or PR `#424`/`#425` as selected-pass workbench cohort breadth, API/UI activation, schema/runtime/source widening, package/handoff/export behavior, connector dispatch, qualitative/hybrid/RAG/vector behavior, full mockup activation, or broader associated-cohort behavior beyond the exact service path. Do not treat docs `80`/`81`, PR `#432`, docs `82`/`83`, PR `#438`, docs `84`/`85`, or PR `#443` as package, handoff, export, schema/runtime/source, connector, qualitative/hybrid/RAG/vector, retry/recovery, pass-entry, broader UI, or full mockup behavior; docs `82`/`83` do not make result review live by themselves, and docs `84`/`85` do not make UI behavior live by themselves. If a future checkout carries revisions beyond docs `77`/`78`/`79`/`80`/`81`/`82`/`83`/`84`/`85` or PR `#443`, preserve those revisions as branch-local until GitHub and current `main` both confirm them.
...
- if a future branch carries additional changes beyond docs `75`/`76`, docs `77`/`78`/`79`, docs `80`/`81`, docs `82`/`83`, docs `84`/`85`, docs `86`/`87`, PR `#417`, PR `#424`/`#425`, PR `#432`, PR `#433`, PR `#434`, PR `#435`, PR `#436`, PR `#438`, PR `#441`, PR `#443`, or PR `#447`, classify them as branch-local until GitHub and current `main` both confirm them
