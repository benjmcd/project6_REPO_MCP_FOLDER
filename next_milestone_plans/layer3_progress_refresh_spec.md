...
- if `34_L3_WB_PLAN_REVISION_FREEZE.md` and `35_L3_WB_PLAN_REVISION_API_AND_STATE_CONTRACT.md` are present, preserve them as the PR `#203` planning-only fourth-slice revision-control freeze/API-state packet; after PR `#205`, mark only pre-approval plan rejection/revision-control live, and after PR `#207`, treat backend row-locking and shared UI in-flight locking as hardening of that same bounded slice rather than a new functional slice; do not let it imply approved-plan reopening/supersession, execution, manifests, results/package/handoff, runtime DB/schema widening, or qualitative/hybrid/RAG/vector/LLM planning
