...
- if a branch checkout adds `/api/v1/layer3/readiness`, explicit preview identity/hash metadata, or approval/revision serialization checks, preserve it as branch-local read-only implementation-readiness proof until GitHub and current `main` both confirm it; do not treat it as execution selection or downstream activation
