...
test('Layer 3 workbench renders Candidate B default-promotion status contract without route calls', async ({ page }) => {
  ...
  await expect(page.locator('#candidate-b-broader-scope-selector-activation-form')).toHaveAttribute(
    'data-rendered-mode',
    'rendered_candidate_b_broader_eligible_corpus_default_scope_selector_activation_status_control',
  );
  await expect(page.locator('#candidate-b-broader-scope-selector-activation-form')).toHaveAttribute('data-frontend-durable-authority', 'false');
  await expect(page.locator('#candidate-b-broader-scope-selector-activation-submit')).toBeDisabled();
  await expect(page.locator('#candidate-b-broader-scope-activation-consumption-form')).toHaveAttribute(
    'data-rendered-mode',
    'rendered_candidate_b_broader_eligible_corpus_default_scope_activation_receipt_consumption_status_control',
  );
  await expect(page.locator('#candidate-b-broader-scope-activation-consumption-form')).toHaveAttribute('data-frontend-durable-authority', 'false');
  await expect(page.locator('#candidate-b-broader-scope-activation-consumption-submit')).toBeDisabled();
  await expect(page.locator('#candidate-b-broader-scope-consumption-receipt-use-form')).toHaveAttribute(
    'data-rendered-mode',
    'rendered_candidate_b_broader_eligible_corpus_default_scope_consumption_receipt_use_status_control',
  );
  await expect(page.locator('#candidate-b-broader-scope-consumption-receipt-use-form')).toHaveAttribute('data-frontend-durable-authority', 'false');
...
test('Layer 3 workbench records Candidate B broader eligible-corpus runtime status through rendered control', async ({ page, request }) => {
  ...
  const panel = page.locator('#candidate-b-default-promotion-status-panel');
  await expect(panel).toBeVisible();
  const form = page.locator('#candidate-b-broader-scope-runtime-form');
  ...
  await expect(panel).toContainText('candidate_b_broader_scope_activation_receipt_consumption_selected');
  await expect(panel).toContainText('consumption receipt status: recorded');
  await expect(panel).toContainText('consumption receipt id: cb-broader-scope-activation-consumption-rendered-proof');
  await expect(panel).toContainText('consumption authority source: redacted_candidate_b_broader_scope_selector_activation_receipt');
  await expect(panel).toContainText('activation receipt reloaded: true');
  await expect(panel).toContainText('activation receipt binding verified: true');
  await expect(panel).toContainText('selector-use status revalidated: true');
  await expect(panel).toContainText('default scope consumption enabled: true');
  ...
  await expect(panel).toContainText('candidate_b_broader_scope_consumption_receipt_use_selected');
  await expect(panel).toContainText('use receipt status: recorded');
  await expect(panel).toContainText('use receipt id: cb-broader-scope-consumption-receipt-use-rendered-proof');
  await expect(panel).toContainText('use authority source: redacted_candidate_b_broader_scope_activation_consumption_receipt');
  await expect(panel).toContainText('consumption receipt reloaded: true');
  await expect(panel).toContainText('consumption receipt binding verified: true');
  await expect(panel).toContainText('activation receipt binding verified: true');
  await expect(panel).toContainText('selector-use status revalidated: true');
  await expect(panel).toContainText('default scope use enabled: true');
  await expect(panel).toContainText('default scope application scope: consumed_receipt_bound_selected_classes_only');
  await expect(panel).toContainText('default scope mutation performed: false');
  await expect(panel).toContainText('raw local path exposed: false');
  await expect(panel).toContainText('raw URL exposed: false');
...
test('Layer 3 workbench records Candidate B workflow queue state through rendered append-only control', async ({ page }) => {
  ...
  const returnedStatusRequest = {
    client_request_id: 'candidate-b-rendered-queue-state-status-request',
    status_mode: 'candidate_b_full_corpus_operator_workflow_status_v1',
    operator_decision: 'inspect_candidate_b_full_corpus_operator_workflow_status',
    operator_workflow_receipt_id: 'cb-full-corpus-operator-run-queue-state-rendered-proof',
    baseline_run_id: 'baseline-rendered-queue-state-proof',
    candidate_a_run_id: 'candidate-a-rendered-queue-state-proof',
    candidate_b_run_id: 'candidate-b-rendered-queue-state-proof',
    bridge_receipt_id: 'cb-runtime-l3-rendered-queue-state-proof',
    downstream_proof_id: 'cb-runtime-downstream-proof-rendered-queue-state-proof',
  };
  await page.route('**/api/v1/layer3/source/ingestion/candidate-b/full-corpus/operator-workflow/history', async (route) => {
    await route.fulfill({
      status: 200,
  ...
  await page.route('**/api/v1/layer3/source/ingestion/candidate-b/full-corpus/operator-workflow/history', async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        next_allowed_actions: [
          'inspect a selected workflow-run row through the returned status request',
          'record append-only execution-boundary authority after retry terminal projection',
        ],
        history_rows: [
          {
            operator_workflow_receipt_id: returnedStatusRequest.operator_workflow_receipt_id,
            operator_workflow_receipt_hash: '9'.repeat(64),
            source_operator_workflow_receipt_id: 'cb-full-corpus-operator-source-execution-boundary-rendered-proof',
            source_operator_workflow_receipt_hash: 'a'.repeat(64),
            authority_basis_hash: 'b'.repeat(64),
            runtime_root_lifecycle_receipt_id: 'cb-full-corpus-runtime-roots-execution-boundary-rendered-proof',
            baseline_run_id: returnedStatusRequest.baseline_run_id,
            candidate_a_run_id: returnedStatusRequest.candidate_a_run_id,
            candidate_b_run_id: returnedStatusRequest.candidate_b_run_id,
            compare_target_set_hash: 'c'.repeat(64),
            bridge_receipt_id: returnedStatusRequest.bridge_receipt_id,
            downstream_proof_id: returnedStatusRequest.downstream_proof_id,
            material_relative_name: 'text/target-execution-boundary-00001.md',
            run_state: 'proven',
            status_endpoint: '/api/v1/layer3/source/ingestion/candidate-b/full-corpus/operator-workflow/status',
            status_request: returnedStatusRequest,
            raw_local_path_exposed: false,
        ...
        history_rows: [
          {
            operator_workflow_receipt_id: returnedStatusRequest.operator_workflow_receipt_id,
            operator_workflow_receipt_hash: '9'.repeat(64),
            source_operator_workflow_receipt_id: 'cb-full-corpus-operator-source-process-execution-rendered-proof',
            source_operator_workflow_receipt_hash: 'a'.repeat(64),
            authority_basis_hash: 'b'.repeat(64),
            runtime_root_lifecycle_receipt_id: 'cb-full-corpus-runtime-roots-process-execution-rendered-proof',
            baseline_run_id: returnedStatusRequest.baseline_run_id,
            candidate_a_run_id: returnedStatusRequest.candidate_a_run_id,
            candidate_b_run_id: returnedStatusRequest.candidate_b_run_id,
            compare_target_set_hash: 'c'.repeat(64),
            bridge_receipt_id: returnedStatusRequest.bridge_receipt_id,
            downstream_proof_id: returnedStatusRequest.downstream_proof_id,
            material_relative_name: 'text/target-process-execution-00001.md',
            run_state: 'proven',
            status_endpoint: '/api/v1/layer3/source/ingestion/candidate-b/full-corpus/operator-workflow/status',
            status_request: returnedStatusRequest,
            raw_local_path_exposed: false,
        ...
        history_rows: [
          {
            operator_workflow_receipt_id: returnedStatusRequest.operator_workflow_receipt_id,
            operator_workflow_receipt_hash: '9'.repeat(64),
            source_operator_workflow_receipt_id: 'cb-full-corpus-operator-source-process-completion-result-rendered-proof',
            source_operator_workflow_receipt_hash: 'a'.repeat(64),
            authority_basis_hash: 'b'.repeat(64),
            runtime_root_lifecycle_receipt_id: 'cb-full-corpus-runtime-roots-process-completion-result-rendered-proof',
            baseline_run_id: returnedStatusRequest.baseline_run_id,
            candidate_a_run_id: returnedStatusRequest.candidate_a_run_id,
            candidate_b_run_id: returnedStatusRequest.candidate_b_run_id,
            compare_target_set_hash: 'c'.repeat(64),
            bridge_receipt_id: returnedStatusRequest.bridge_receipt_id,
            downstream_proof_id: returnedStatusRequest.downstream_proof_id,
            material_relative_name: 'text/target-process-completion-result-00001.md',
            run_state: 'proven',
            status_endpoint: '/api/v1/layer3/source/ingestion/candidate-b/full-corpus/operator-workflow/status',
            status_request: returnedStatusRequest,
            raw_local_path_exposed: false,
...
test('Layer 3 workbench records Candidate B bundle downstream proof from rendered coverage evidence', async ({ page, request }) => {
  ...
  expect(JSON.stringify(proof)).not.toContain('C:\\');
  await expect(panel).toContainText('candidate_b_bundle_downstream_proof_recorded');
  await expect(panel).toContainText('candidate_b_layer3_downstream_e2e_proven');
  await expect(panel).toContainText('visual lane enabled: false');
  await expect(panel).not.toContainText('http://');
  await expect(panel).not.toContainText('https://');
  await expect(panel).not.toContainText('file://');
  await expect(panel).not.toContainText('C:\\');
...
test('Layer 3 workbench records Candidate B runtime visual-lane downstream proof from rendered status evidence', async ({ page, request }) => {
  const setup = await expectJson(await request.post('/__test/layer3/candidate-b-realistic-readiness-audit'));
  const requiredCoverage = [
    'source_directory_scan',
    'material_preview',
...
test('Layer 3 workbench runs Candidate B readiness audit from rendered closure evidence', async ({ page, request }) => {
  const setup = await expectJson(await request.post('/__test/layer3/candidate-b-realistic-readiness-audit'));
  const requiredCoverage = [
    'source_directory_scan',
    'material_preview',
  ...
  expect(apiRequests.filter((apiRequest) => (
    ...
    || apiRequest.path.includes('/source/ingestion/candidate-b/default-promotion/operator-status')
    || apiRequest.path.includes('/source/ingestion/candidate-b/default-promotion/closure-evidence')
    || apiRequest.path.includes('/source/ingestion/candidate-b/default-promotion/readiness-audit')
  ))).toEqual([
    { method: 'POST', path: '/api/v1/layer3/source/ingestion/candidate-b/bundle/downstream-proof' },
    { method: 'POST', path: '/api/v1/layer3/source/ingestion/candidate-b/visual-lane/status' },
    { method: 'POST', path: '/api/v1/layer3/source/ingestion/candidate-b/runtime/downstream-proof' },
    { method: 'POST', path: '/api/v1/layer3/source/ingestion/candidate-b/default-promotion/operator-status' },
    { method: 'POST', path: '/api/v1/layer3/source/ingestion/candidate-b/default-promotion/closure-evidence' },
    { method: 'POST', path: '/api/v1/layer3/source/ingestion/candidate-b/default-promotion/readiness-audit' },
  ]);
});

test('Layer 3 workbench records and inspects Candidate B final proof from rendered readiness audit', async ({ page, request }) => {
  const setup = await expectJson(await request.post('/__test/layer3/candidate-b-realistic-readiness-audit'));
  const requiredCoverage = [
    'source_directory_scan',
    'material_preview',
  ...
  expect(apiRequests.filter((apiRequest) => (
    ...
    || apiRequest.path.includes('/source/ingestion/candidate-b/default-promotion/closure-evidence')
    || apiRequest.path.includes('/source/ingestion/candidate-b/default-promotion/readiness-audit')
    || apiRequest.path.includes('/source/ingestion/candidate-b/default-promotion/final-proof')
  ))).toEqual([
    { method: 'POST', path: '/api/v1/layer3/source/ingestion/candidate-b/bundle/downstream-proof' },
    { method: 'POST', path: '/api/v1/layer3/source/ingestion/candidate-b/visual-lane/status' },
    { method: 'POST', path: '/api/v1/layer3/source/ingestion/candidate-b/runtime/downstream-proof' },
...
test('Layer 3 mockup activation readiness dashboard classifies next-phase journeys from bootstrap authority', async ({ page }, testInfo) => {
  ...
  await expect(panel).toContainText('pdf_location_read_only_live_projection_contract');
  await expect(panel).toContainText('layer3.pdf_location_projection.v1');
  await expect(panel).toContainText('#mockup-pdf-location-projection');
  await expect(panel).toContainText('1 status projection and 6 negative boundaries');
  await expect(panel).toContainText('sublayers_3a_3b_read_only_live_projection_contract');
