...
>> Status note (2026-06-22): Layer 3 front-door truth is anchored to current `project6-origin/main`, [README.md](README.md), and [config/support_matrix.yaml](config/support_matrix.yaml). The selected profile is `base=local_expert`, `overlays=["public_connectors","sec_xbrl_offline"]`; role authorization remains opt-in/inert under the local default, 3C deterministic method/product flows and public connector workflows are supported for local operator use, model/agent egress is default-deny, SEC-XBRL value-bearing support is simulation/offline-replay only, and SEC live network egress/value reveal/controlled reveal submit remain default-off. Do not infer production-ready SEC value reveal, real provider delivery, live network, model egress, or default-on behavior from these surfaces.
