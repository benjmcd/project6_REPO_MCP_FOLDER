...
>> Status note (2026-06-18): Layer 3 live-main front-door truth is anchored to `project6-origin/main` `f679f64f`. The API surface is identity-gated by default with local `AUTH_OWNER=none` usability, role authorization remains opt-in/inert by default, 3C deterministic method/product flows are live behind server-owned authority, model/agent egress is default-deny, SEC-XBRL live network/value reveal/controlled reveal submit remain default-off, and CI now covers the PostgreSQL 3C golden path plus SEC-XBRL test families. Do not infer production-ready SEC value reveal, real provider delivery, live network, model egress, or default-on behavior from these surfaces.
